...
Панель управления LTT полностью отделена от бэкенда и общается с ним через единую точку входа — файл `api.php`. Все запросы передают параметр `?action=X`, который маршрутизируется внутри PHP-скрипта.
