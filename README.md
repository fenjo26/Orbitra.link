...
Postbacks that explain themselves: common status words map out of the box, and a one-click tester shows exactly where a conversion gets stuck.

### Added

- 🗺️ **Built-in status aliases** — `approved`, `confirmed`, `accepted`, `converted` → `sale`; `declined`, `refused`, `cancelled`, `canceled` → `rejected`; they fire only as the last resort before `custom`, so your own conversion types and `{type}_status` parameters always win (previously a BIGO/Dr. Cash-style "approved" landed in `custom`: no revenue, no S2S to the source)
- 🧪 **Postback tester (campaign editor → S2S Postbacks)** — one click fires a real postback at your own server with a throwaway click and shows a verdict for every stage: recording with the mapped status, statuses-filter match per configured postback, a live probe delivery (HTTP code + timing), unresolved macros like `{external_id}`/`{sub_id_N}` that would ship to the source as literal text, and queue-worker health (ping, last error, cron); cleans up after itself, audit line stays in the incoming log
- 🎛️ **S2S postbacks editor rebuilt** — each postback is a card with a segmented GET/POST control, an "insert macro" menu that drops `{subid}`, `{status}`, `{payout}`, `{sub_id_1..5}`… at the caret, and statuses as toggle chips (built-ins plus any custom status added inline) instead of a raw comma field; the stored value is unchanged
- 🩹 **Update self-heals the queue worker** — a missing worker cron is installed automatically after an in-panel update, and a silent worker reports its last error right in the update output; the cron log path is unified to `var/logs/postback_queue.log`

### Previous Highlights (v1.5.10)

- 🩹 **Postback hotfix for PHP < 8.4** — v1.5.9 rejected every incoming postback on PHP 8.1–8.3 (raw `BEGIN IMMEDIATE` invisible to `inTransaction()`, php bug #81227); `orbitraPostbackTransactionActive()` asks SQLite itself on older PHP. The installer now provisions PHP 8.5/8.4 (ondrej/php or packages.sury.org, distro fallback, PHP 8.1+ hard check), the in-panel update warns while below 8.4, and queue health timestamps are UTC

Older releases (v1.5.9 and earlier): see the [full changelog](CHANGELOG.md).
...
### Current release — v1.5.11 (2026-09-17)
...
### Previous release — v1.5.10 (2026-09-17)
