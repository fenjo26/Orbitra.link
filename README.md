...
- **Шаблоны партнерских сетей**: платформенные (Everflow, CAKE, HitPath, Affise, TUNE/HasOffers) + сети Leadbit, M4Leads, Dr.Cash, AdCombo и другие
- **Шаблоны источников**: Facebook, Google, TikTok, Яндекс, Taboola, Outbrain, Email и другие
...
В Orbitra **нет** учётной записи «по умолчанию» (`admin`/`admin`) — данные администратора вы задаёте сами при первом запуске.

При первом открытии админки (`/admin.php`) система определяет, что пользователей ещё нет, и запускает **мастер первичной настройки**. В нём вы создаёте собственного администратора:

- **Логин** — минимум 3 символа
- **Пароль** — минимум 6 символов (с подтверждением)
- **Часовой пояс** и **язык интерфейса** (RU/EN)

После создания администратора мастер больше не появляется, а вход выполняется по заданным вами логину и паролю.
...
## 📝 Что нового в v0.9.3.7

### Добавлено
- ✉️ Шаблон источника трафика **Email** — для email-маркетологов. Преднастроены sub-параметры `subscriber_id`, `campaign_id`, `list_id`, `broadcast_id`, `esp`, под которые подставляются merge-теги вашего ESP.
- 🌐 **Платформенные шаблоны партнёрских сетей**: Everflow, CAKE, HitPath, Affise, TUNE/HasOffers. Теперь любую небольшую сеть, работающую на этих платформах, можно подключить выбором шаблона платформы — без отдельной записи под каждую компанию. Параметр передачи click-id в оффер заполняется по стандарту платформы (Everflow `sub1`, CAKE `s1`, HitPath `c1`, Affise `sub1`, TUNE `aff_sub`).

## 📝 Что нового в v0.9.3.6

### Исправлено
- 🐛 Ошибка HTTP 500 на потоке только с лендингом (без оффера). Лог клика требовал оффер (`offer_id NOT NULL` + внешний ключ), поэтому клик без оффера падал с ошибкой БД ещё до загрузки лендинга. Теперь колонка `clicks.offer_id` допускает NULL (автоматическая миграция БД), клики без оффера логируются с NULL, а сбой логирования больше не может уронить страницу. Статистика по лендингам работает, оффер просто не атрибутируется — как и ожидается.

## 📝 Что нового в v0.9.3.5

### Исправлено
- 🐛 Надёжнее авто-обновление: если локально изменённые файлы кода блокировали `git pull` (ошибка «Your local changes would be overwritten»), апдейтер теперь сам сбрасывает эти изменения и повторяет обновление. Данные не страдают — база, загруженные лендинги и гео-базы вне git, а `config.php` сохраняется.

## 📝 Что нового в v0.9.3.4

### Добавлено
- ✨ Переход с локального лендинга на оффер по ссылке `/?_lp=1` (совместимо с Keitaro). На странице лендинга оформите кнопку оффера как `<a href="/?_lp=1">Оффер</a>` — при клике трекер найдёт привязанный к клику оффер и сделает редирект с подстановкой макросов (`{clickid}`, `{sub_id_1}` и т.д.). Поддерживается выбор конкретного оффера: `/?_lp=1&offer_id=10`.

### Исправлено
- 🐛 Схема потока «Лендинг + Оффер» теперь работает и с одним лендингом без оффера. Раньше при удалении оффера лендинг мог не открываться — теперь выбранный лендинг всегда используется как назначение, оффер опционален.

## 📝 Что нового в v0.9.3.3

### Исправлено
- 🐛 Фильтры потоков теперь действительно применяются. Раньше обрабатывались только фильтры `Country`, `Device`, `Bot` и `Language`, а `Browser`, `OS`, `IP`, `Referer`, `Keyword`, `Weekday` и `Time` молча пропускали весь трафик (например, фильтр «Browser = TikTok» в режиме include всё равно открывался во всех браузерах). Все эти фильтры теперь проверяются.
- 🐛 Детект браузера распознаёт in-app браузеры (TikTok, Facebook, Instagram и др.) по сигнатурам user-agent — фильтр по TikTok работает корректно.
- 🐛 Фильтр `IP` поддерживает маски (`10.0.0.*`), сопоставление `Country`/`Device`/`OS` стало регистронезависимым.
- 🐛 Если IP не определяется бесплатной гео-базой (страна `Unknown`), фильтр по стране такого посетителя пропускает, а не блокирует — чтобы не терять реальный трафик.

### Добавлено
- ✨ Фильтр `ISP` (по провайдеру/сети) через бесплатную базу **MaxMind GeoLite2-ASN**. Загрузите `GeoLite2-ASN.mmdb` в папку `/geo/` (тем же ключом MaxMind, что и City) — и фильтр заработает, сопоставляя организацию сети и AS-номер. Инструкция и ссылка добавлены на страницу «ГеоБазы».

> ℹ️ Без базы GeoLite2-ASN фильтр `ISP` просто пропускает трафик (ничего не ломается). Фильтр `Connection` (wifi/mobile/cable) по-прежнему не поддерживается — для него нет бесплатного источника данных.

## 📝 Что нового в v0.9.3.2

### Исправлено
- 🐛 Локальные лендинги (ZIP) теперь корректно отдаются по клику. Раньше обработчик кликов искал файлы в `/api/landings/{id}`, тогда как загрузка сохраняла их в `/landings/{id}` — из-за этого появлялась ошибка «Local landing files not found» и не происходил переход на лендинг. Пути приведены к единому виду.
