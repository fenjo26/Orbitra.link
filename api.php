...
try {
    switch ($action) {
        ...
        case 'chart':
            ...
            $chartQuery = "
                SELECT
                    strftime($timeFormat, clicks.created_at, '$dbTzOffset') as period,
                    ...
                    COALESCE(SUM(revenue_records.revenue), 0) as real_revenue
                FROM clicks
                LEFT JOIN conversions ON conversions.click_id = clicks.id
                LEFT JOIN revenue_records ON revenue_records.click_id = clicks.id
                $whereCl
                GROUP BY period
                ORDER BY period ASC
                LIMIT 100
        ...
        case 'offers':
            ...
            // Полный список офферов со статистикой - OPTIMIZED
            $stmt = $pdo->prepare("
                SELECT
                    o.id, o.name, o.group_id, o.affiliate_network_id, o.url, o.redirect_type,
                    ...
                    COUNT(DISTINCT cl.ip) as unique_clicks,
                    COALESCE(SUM(cl.is_conversion), 0) as conversions,
                    COALESCE(SUM(conv.payout), 0) as revenue
                FROM offers o
                ...
                LEFT JOIN conversions conv ON conv.click_id = cl.id
        ...
        case 'logs':
            ...
            if ($type === 'traffic') {
                $stmt = $pdo->prepare("
                    SELECT cl.id, cl.id as click_id, cl.created_at, c.name as campaign_name, cl.ip, cl.country as country_code, cl.device_type, cl.user_agent, o.url as redirect_url, '' as subid
                    FROM clicks cl
        ...
        case 'campaign_report':
            ...
            $sql = "
                SELECT
                    ...
                    SUM(clicks.is_conversion) as conversions,
                    COALESCE(SUM(conversions.payout), 0) as revenue,
                    COALESCE(SUM(revenue_records.amount), 0) as real_revenue
                FROM clicks
                LEFT JOIN conversions ON conversions.click_id = clicks.id
                LEFT JOIN revenue_records ON revenue_records.click_id = clicks.id
                WHERE $where
        ...
        case 'system_status':
            try {
                ...
                $ip2locFile = __DIR__ . '/geo/IP2LOCATION-LITE-DB3.BIN';
                ...
                $geoDbs[] = [
                    'name' => 'IP2Location DB3',
                    'status' => file_exists($ip2locFile) ? 'ok' : 'missing',
                    'size' => file_exists($ip2locFile) ? filesize($ip2locFile) : 0,
                    'updated' => file_exists($ip2locFile) ? date('Y-m-d H:i', filemtime($ip2locFile)) : null
                ];
        ...
        case 'geo_dbs':
            ...
            $ip2loc = [
                ...
                'type' => 'Country-Region-City',
                'status' => file_exists($ip2locDb) ? 'OK' : 'Нет базы',
                'updated_at' => file_exists($ip2locDb) ? date('Y-m-d H:i:s', filemtime($ip2locDb)) : null,
                'size' => file_exists($ip2locDb) ? filesize($ip2locDb) : 0
            ];
            $dbs[] = $ip2loc;
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                try {
                    ...
                    if ($ext === 'zip') {
                            ...
                            foreach ($iter as $file) {
                                if ($file->isFile()) {
                                    ...
                                    if ($fExt === 'dat') {
                                        ...
                                        copy($file->getPathname(), __DIR__ . '/geo/IP2LOCATION-LITE-DB3.BIN');
                        ...
                        if (!is_dir(__DIR__ . '/geo')) mkdir(__DIR__ . '/geo', 0755, true);
                        if (move_uploaded_file($fileTmp, __DIR__ . '/geo/IP2LOCATION-LITE-DB3.BIN')) {
        ...
        case 'geo_db_update':
            ...
            if ($dbId === 'ip2location_lite_db3') {
                ...
                $url = "https://www.ip2location.com/download/?token={$token}&file=DB3LITEBIN";
                ...
                if ($httpCode !== 200) {
                    echo json_encode(['status' => 'error', 'message' => "Failed to download database. HTTP Code: $httpCode. Check your Token."]);
                    break;
                }

                // Extract .bin
                $destPath = __DIR__ . '/geo/IP2LOCATION-LITE-DB3.BIN';
                ...
                try {
                    ...
                    if ($zip->open($tmpArchive) === TRUE) {
                        ...
                        if ($extracted) {
                            logSystem($pdo, 'INFO', 'IP2Location Geo DB Updated successfully');
                            echo json_encode(['status' => 'success', 'message' => 'База IP2Location успешно обновлена']);
                        } else {
        ...
        case 'trends':
            ...
            $sql = "
                ...
                FROM clicks cl
                LEFT JOIN conversions ON conversions.click_id = cl.id
                LEFT JOIN revenue_records ON revenue_records.click_id = cl.id
                WHERE $whereSQL
                GROUP BY period
                ORDER BY period ASC
            ";
