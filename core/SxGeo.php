<?php
/*
 * Orbitra note: this reader ships with the repository. It is Sypex Geo API
 * 2.2.3 (SxGeo.php, BSD license — see header below) with the minimal patch
 * needed for PHP 8.0+: the PHP 5 curly-brace string-offset syntax was
 * removed in PHP 8, so those offsets use square brackets here. The vendor archive
 * (sypexgeo.net/files/SxGeo22_API.zip) still ships the PHP 5 syntax and
 * does not parse on PHP 8+, so the fixed copy is kept here.
 */
/***************************************************************************\
...
class SxGeo {
	...
	protected $m_idx_arr;
	protected $m_idx_len;
	// Orbitra note: declared here for PHP 8.2+ — the vendor 2.2.3 file only
	// assigned these in the constructor, which now raises dynamic-property
	// deprecations on every lookup.
	protected $b_idx_len;
	protected $block_len;
	protected $id_len;
	protected $max_city;
