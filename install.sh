...
echo ""
echo " Once a domain points here and has a certificate (Domains section),"
echo " use https://your-domain/admin.php instead. A few integrations need it:"
echo " Telegram, for one, only calls a bot webhook back over HTTPS on a real"
echo " domain, never to an IP. Over the IP the bot falls back to polling."
echo ""
if [ -n "$PB_KEY" ]; then
