export default {
    "serverSetup": {
        "title": "Потрібен разовий крок на сервері",
        "text": "Цій версії потрібна разова зміна на сервері, яку кнопка «Оновити» зробити не може: вона працює від веб-користувача й не має прав root.",
        "textLegacy": "На сервері лишилися старі правила sudo, через які веб-користувач міг отримати root (certbot з будь-якими хуками, копіювання будь-якого конфігу nginx). Кнопка «Оновити» прибрати їх не може — вона працює без root. Виконайте команду нижче один раз, і їх буде замінено безпечними помічниками.",
        "step1": "Підключіться до сервера по SSH від root або користувача з sudo.",
        "step2": "Виконайте команду:",
        "step3": "Вона завантажує скрипт налаштування з тегу цього релізу на GitHub (не з папки трекера), замінює правила sudo, ставить помічників і завершується. Повторний запуск безпечний. Трекінг не переривається.",
        "copy": "Копіювати",
        "copied": "Скопійовано",
        "recheck": "Я виконав — перевірити",
        "stillNeeded": "Сервер ще не повідомив про нове налаштування. Перегляньте вивід команди — чи немає там помилок.",
        "moreOnUpdatePage": "Відкрити сторінку оновлення",
        "ok": "Налаштування сервера актуальне (версія {version})."
    },
    "security": {
        "totpTitle": "Двофакторна автентифікація",
        "totpDesc": "При кожному вході запитувати одноразовий код із застосунку-автентифікатора.",
        "totpEnable": "Увімкнути",
        "totpDisable": "Вимкнути",
        "totpEnabledBanner": "Двофакторну автентифікацію увімкнено.",
        "totpDisabledBanner": "Двофакторну автентифікацію вимкнено.",
        "totpSecretLabel": "Секретний ключ",
        "totpOtpauthLabel": "otpauth URI",
        "totpSecretHint": "QR-коду немає: додайте акаунт у застосунок-автентифікатор вручну (опція «ввести ключ налаштування») і вставте секрет. otpauth URI нижче містить ті самі дані.",
        "totpCodeLabel": "Код із застосунку",
        "totpCodePlaceholder": "6-значний код",
        "totpCodeRequired": "Введіть код із застосунку",
        "totpConfirm": "Підтвердити",
        "totpBadCode": "Невірний код",
        "totpBadPassword": "Невірний пароль",
        "totpDisablePasswordLabel": "Ваш пароль",
        "totpDisablePasswordRequired": "Введіть пароль",
        "totpDisableHint": "Щоб вимкнути двофакторну автентифікацію, підтвердьте дію паролем акаунта.",
        "totpSetupError": "Не вдалося почати налаштування",
        "totpEnableError": "Не вдалося увімкнути двофакторну автентифікацію",
        "totpDisableError": "Не вдалося вимкнути двофакторну автентифікацію",
        "currentPassword": "Поточний пароль",
        "currentPasswordPlaceholder": "Введіть поточний пароль",
        "currentPasswordRequired": "Введіть поточний пароль, щоб змінити його",
        "currentPasswordInvalid": "Поточний пароль вказано невірно",
        "crmIngestTitle": "Безпека прийому лідів у CRM",
        "crmIngestLabel": "Секрет підпису CRM-прийому",
        "crmIngestHint": "Якщо задано, /crm-ingest приймає лише запити із заголовком X-Orbitra-Signature (HMAC-SHA256 від тіла, hex). Порожньо — ендпоінт відкритий (з обмеженням частоти).",
        "crmIngestSaved": "Збережено",
        "crmIngestError": "Не вдалося зберегти",
        "vpnAsnLabel": "Вважати VPN-ASN сигналом бота",
        "vpnAsnHint": "VPN/проксі-мережі постачальника вважаються сигналом бота. Вимкніть, якщо ваша авдиторія масово сидить за комерційним VPN."
    },
    "streams": {
        "desktop": "Десктоп",
        "mobile": "Мобільні",
        "tablet": "Планшети"
    },
    "parameters": {
        "adId": "ID оголошення",
        "adsetId": "ID групи оголошень",
        "campaignId": "ID кампанії",
        "adName": "Назва оголошення",
        "adsetName": "Назва групи",
        "campaignName": "Назва кампанії",
        "site": "Майданчик",
        "keyword": "Ключове слово",
        "cost": "Витрати",
        "currency": "Валюта",
        "externalId": "Зовнішній ID",
        "creativeId": "ID креативу",
        "adCampaignId": "ID рекламної кампанії",
        "source": "Джерело",
        "enterParamKey": "Введіть ключ параметра (наприклад, utm_placement, placement, custom_id):",
        "addParam": "Додати параметр",
        "additionalSubIds": "Додаткові Sub ID (sub_id_1 .. sub_id_30)",
        "sourceDrivenHint": "Список параметрів береться з джерела трафіку кампанії — доки воно не обране, список універсальний.",
        "placement": "Плейсмент (utm_placement)",
        "facebookParams": "Параметри Facebook",
        "facebookParamsHint": "Скопіювати URL-параметри Facebook Ads (без початкового «?») для вставки в Meta Ads Manager",
    },
    "costModels": {
        "cpc": "CPC — оплата за клік",
        "cpv": "CPV — вартість за візит",
        "cpuc": "CPuC — оплата за унікальний клік",
        "cpm": "CPM — оплата за 1000 показів",
        "cpa": "CPA — оплата за дію",
        "cps": "CPS — оплата за продаж",
        "revShare": "RevShare — відсоток від доходу"
    },
    "mcpPage": {
        "title": "ІІ-асистент (MCP)",
        "intro": "Підключіть Claude Desktop та інші ІІ-асистенти до трекера й керуйте ним звичайною мовою — аналізуйте кампанії, створюйте їх масово, підключайте домени, редагуйте офери тощо.",
        "step1Title": "Створіть API-ключ",
        "step1Desc": "Відкрийте «Користувачі → API-ключі» та згенеруйте ключ. Ключ Read — лише аналітика; ключ Write — ще й керування кампаніями, оферами й доменами.",
        "step1Hint": "Якщо попросити асистента щось створити або змінити з Read-ключем, повернеться «403 API key is read-only» — для цього потрібен Write-ключ.",
        "remoteTitle": "Варіант A — вставити URL (працює і в браузері, і в застосунку)",
        "remoteDesc": "Створіть ключ у Користувачі → API-ключі, натисніть поруч кнопку з посиланням — у буфер потрапить готовий URL. Вставте його в Claude → Settings → Connectors → Add custom connector. Встановлювати нічого не треба. Ключ їде просто в адресі, тож ставтеся до URL як до пароля, а щоб закрити доступ — видаліть ключ.",
        "orLocal": "Варіант B — запустити сервер поруч з асистентом. Так уміє лише Claude Desktop, і через діалог Connectors це не підключається — саме тому там немає поля для API-ключа.",
        "nodePathHint": "Шлях до Node дізнайтеся командою",
        "nodePathHint2": "(macOS/Linux) або",
        "nodePathHint3": "(Windows) і підставте результат у «command». Просто «node» спрацює, лише якщо він є в системному PATH.",
        "step2Title": "Встановіть сервер",
        "step2Desc": "На машині з ІІ-асистентом один раз встановіть залежності в папці трекера:",
        "step3Title": "Додайте в Claude Desktop",
        "step3Desc": "У Claude Desktop відкрийте Settings → Developer → Edit Config і вставте це, потім замініть шлях на абсолютний шлях до mcp/src/index.js, а ORBITRA_API_KEY — на ключ із кроку 1. Перезапустіть застосунок повністю: закрити вікно недостатньо.",
        "copyConfig": "Скопіювати конфіг",
        "copied": "Скопійовано",
        "toolsNote": "Доступний 31 інструмент: аналітика (метрики, кампанії, конверсії, звіти) та керування (створення / масове створення / редагування / видалення кампаній, оферів, доменів, джерел, лендингів).",
        "fullGuide": "Повна інструкція:"
    },
    "tpl": {
        "src_email": "Електронна пошта",
        "src_custom": "Спеціальне джерело",
        "net_generic": "загальний",
        "net_custom": "Спеціальна мережа",
        "net_everflow": "Everflow (платформа)",
        "net_cake": "ТОРТ (платформа)",
        "net_hitpath": "HitPath (платформа)",
        "net_affise": "Affise (платформа)",
        "net_tune": "TUNE / HasOffers (платформа)",
        "net_onewin": "1win.run",
        "net_twentytwobet": "22BET.com",
        "net_fourrabet": "4rabetpartner.com",
        "net_advertise": "Advertise.net",
        "net_alfaleads": "Alfaleads.net",
        "net_appsflyer": "AppsFlyer.com",
        "net_boomerang": "Boomerang-partners.com",
        "net_biamo": "Biamopartners.com",
        "net_cataffs": "Cataffs.team",
        "net_cpabro": "Cpabro.vip",
        "net_enot": "Enot.partners",
        "net_gamblingpro": "Gambling.pro",
        "net_ggbetaff": "GGBetAff",
        "net_hellpartners": "Hellpartners.com",
        "net_jimpartners": "Jimpartners.com",
        "net_m1shop": "M1-shop.ru",
        "net_mbpartners": "MB.partners",
        "net_mostbetcpa": "Mostbet.partners (CPA)",
        "net_mostbetrs": "Mostbet.partners (RevShare)",
        "net_nutramedia": "Nutra.Media",
        "net_cparip": "Partners.cpa.rip",
        "net_pinuppartners": "Pin-up.partners",
        "net_profitov": "Profitov.Partners",
        "net_q3network": "Q3.network",
        "net_riddick": "Riddick.guru",
        "net_royalpartners": "Royal.partners",
        "net_vulkanbet": "Vulkan.bet",
        "net_vulkanpartner": "Vulkanpartner.com",
        "net_welcomepartners": "WelcomePartners",
        "net_xpartners": "X-partners.com",
        "net_notes_onewin": "1. У кабінеті 1win перейдіть у PR Інструменти - Посилання. Колонка Дії - натисніть на іконку посилання.\n\n2. У Postback Url status=[REPLACE] замінити на:\n\nstatus=lead - для Реєстрації\n\nstatus=sale - для Першого депозиту\n\nstatus=sale&tid={event_id} - для Депозиту\n\ncurrency=[ISO] - замінити валюту у форматі iso (приклад: currency=usd)",
        "net_notes_ggbetaff": "На бічній панелі ліворуч перейдіть у розділ Постбеки. Виберіть подію, бренд та оффер і заповніть Postback URL\n\nЗамініть status=REPLACE на:\n\nstatus=lead - для Registration\n\nstatus=sale - для Lead Hold\n\nstatus=rejected - для Lead Reject",
        "net_notes_hellpartners": "Для події reg замініть REPLACE на lead\n\nДля події ftd замініть REPLACE на sale\n\nПередайте обидва посилання менеджеру партнерської мережі\n\nУ полі Параметри для оффера замініть CHANGE_FOR_SUMM на виплата по CPA.",
        "net_notes_mbpartners": "Скопіюйте посилання постбеку.\n\nДля події Лід замініть REPLACE на lead\n\nДля події Продаж замініть REPLACE на sale\n\nДо посилання постбеку для статусу sale допишіть: &tid={payment.id}",
        "net_notes_cparip": "1. У кабінеті Partners.cpa.rip перейдіть в Інструменты - Глобальний Постбек - Додати глобальний постбек.\n\n2. Виконується для - Все. Тип цілі - CPA.\n\n3. Вставте Postback URL у поле Постбек.",
        "net_notes_royalpartners": "Скопіюйте посилання постбеку.\n\nДля події Лід замініть REPLACE на lead\n\nДля події Продаж замініть REPLACE на sale",
        "net_notes_welcomepartners": "Посилання Postback Url потрібно віддати менеджеру партнерської мережі."
    },
    "bulkImport": {
        "enterUrl": "Введіть принаймні одну URL-адресу",
        "title": "Джерела масового імпорту",
        "formatTitle": "Формат введення:",
        "urlOnly": "лише URL",
        "nameAndUrl": "Назва та URL",
        "mySource": "Моє джерело",
        "onePerLine": "Одне джерело на рядок",
        "listLabel": "Список джерел",
        "linesSuffix": "лінії",
        "resultTitle": "Результат імпорту:",
        "added": "Додано",
        "duplicates": "Дублікати",
        "errors": "Помилки",
        "andMore": "і",
        "importing": "Імпорт...",
        "importBtn": "Імпорт"
    },
    "common": {
        "reset": "Скинути",
        "done": "Готово",
        "hide": "Сховати",
        "showDetails": "Показати деталі",
        "copied": "Скопійовано",
        "colResizeHint": "Потягніть, щоб змінити ширину · подвійний клік — скинути",
        "selectAll": "Вибрати все",
        "orManual": "Або налаштувати вручну",
        "connectModeManual": "Пряме підключення (токен)",
        "connectModeOauth": "1-Click OAuth",
        "edit": "Змінити",
        "status": "Статус",
        "size": "Розмір",
        "update": "Оновити",
        "deleteError": "Не вдалося видалити",
        "save": "Зберегти зміни",
        "saving": "Збереження...",
        "cancel": "Скасувати",
        "confirm": "Підтвердити",
        "back": "Назад",
        "finish": "Закінчити",
        "loading": "Завантаження...",
        "success": "Успіх",
        "error": "Помилка",
        "networkError": "Сталася помилка мережі",
        "close": "Закрити",
        "apply": "Застосувати",
        "add": "додати",
        "create": "Створити",
        "actions": "Дії",
        "clearStats": "Очистити статистику",
        "clear": "ясно",
        "delete": "Видалити",
        "clearError": "Помилка під час очищення",
        "deleteConfirm": "Видалити? Цю дію не можна скасувати.",
        "deleteSelected": "Видалити вибране",
        "deleteSelectedConfirm": "Видалити вибране ({count})? Цю дію не можна скасувати.",
        "entityInUse": "Не можна видалити: використовується в активних кампаніях: {campaigns}. Приберіть його з потоку або заархівуйте кампанію.",
        "sort": "Сортувати",
        "settings": "Налаштування",
        "exportCsv": "Експорт CSV",
        "resetSort": "Скинути сортування",
        "clearSelection": "Очистити вибір",
        "search": "Пошук",
        "searchPlaceholder": "Ім'я або псевдонім",
        "refresh": "Оновити",
        "all": "все",
        "copy": "Копія",
        "notSet": "не встановлено",
        "disabled": "Вимкнено",
        "checking": "Перевірка...",
        "more": "Ще"
    },
    "nav": {
        "mediaGallery": "Галерея",
        "navigation": "Навігація",
        "adminUsers": "Користувачі, ролі та API-ключі",
        "adminBranding": "Логотип, кольори та тема",
        "adminGeoProfiles": "Готові набори країн для фільтрів",
        "adminSettings": "Валюта, часовий пояс і системні параметри",
        "adminIntegrations": "Партнерки, джерела трафіку, постбеки",
        "adminMcp": "Підключення ШІ-асистента до трекера",
        "adminStatus": "Диск, база та витрата ресурсів",
        "adminLogs": "Запити, постбеки та системні події",
        "adminArchive": "Видалене: відновити або стерти",
        "adminImport": "Завантаження конверсій із файлу або за click ID",
        "adminGeoDbs": "Бази геолокації за IP",
        "adminMigrations": "Версії схеми та імпорт із Keitaro",
        "adminCleanup": "Очищення кліків і конверсій",
        "adminUpdate": "Перевірка та встановлення нової версії",
        "adminAggregator": "Імпорт доходу й витрат із зовнішніх кабінетів",
        "dashboard": "Приладова панель",
        "campaigns": "Кампанії",
        "offers": "Пропозиції",
        "landings": "Посадки",
        "sources": "Джерела",
        "networks": "Партнерки",
        "settings": "Налаштування",
        "profile": "Профіль",
        "system": "система",
        "logout": "Вийти",
        "trends": "Тенденції",
        "analytics": "Аналітика",
        "domains": "Домени",
        "backorder": "Замовлення"
    },
    "analytics": {
        "trends": "Тренд",
        "cohort": "Когорта",
        "campaigns": "Кампанії",
        "allCampaigns": "Всі кампанії",
        "offers": "Офери",
        "allOffers": "Всі офери",
        "selectGroup": "Вибрати групу",
        "selectAll": "Вибрати всі",
        "clearAll": "Скинути",
        "search": "Пошук...",
        "groups": "Групи",
        "landings": "Лендінги",
        "allLandings": "Усі лендінги",
        "noResults": "Нічого не знайдено"
    },
    "cohort": {
        "bannerTitle": "Когорти кампаній",
        "banner": "Кампанії групуються за місяцем або кварталом створення, а кожен стовпець показує, як поводиться когорта з віком. Стабільний загальний прибуток, який тримають лише старі когорти — сигнал, що свіжі запуски вже не заводяться.",
        "what": "Що це таке",
        "whatText": "Когортний аналіз групує кампанії за часом запуску та відстежує кожну групу за місяцями життя (M0 — місяць запуску, M1 — наступний тощо). Замість одного загального підсумку видно, як поводиться кожен «вік» запуску з часом.",
        "why": "Навіщо це потрібно",
        "whyText": "Загальний дохід може здаватися стабільним, хоча його непомітно тягнуть старі кампанії, а свіжі запуски майже нічого не приносять. Коли старі відійдуть, дохід просяде — і реагувати вже запізно. Когорти показують це заздалегідь: видно, чи справді кожен новий запуск заводиться, чи лише витрачає бюджет.",
        "how": "Як читати",
        "howText": "Кожен рядок — одна когорта (місяць запуску); M0, M1, M2… — її місяці життя. Тепло клітинки показує інтенсивність обраної метрики. Перемкніть «% від M0», щоб порівнювати когорти різного розміру за формою затухання: лінія близько 100% утримує аудиторію, та, що швидко падає — втрачає.",
        "granularity": "Гранулярність:",
        "month": "Місяць",
        "quarter": "Квартал",
        "metric": "Метрика:",
        "viewMode": "Вигляд:",
        "absolute": "Абсолют",
        "retention": "% від M0",
        "revenue": "Дохід",
        "realRevenue": "Реальний дохід",
        "clicks": "Кліки",
        "conversions": "Конверсії",
        "cost": "Витрати",
        "profit": "Прибуток",
        "roi": "ROI",
        "realRoi": "Реальний ROI",
        "cr": "CR",
        "campaignsActive": "Активні кампанії",
        "launched": "Запущено",
        "cohortLabel": "Когорта",
        "period": "Період",
        "loading": "Завантаження...",
        "noData": "Немає даних для відображення",
        "noDataTitle": "Немає даних",
        "noDataText": "Немає когортних даних за обраний період. Спробуйте розширити діапазон дат.",
        "matrixTitle": "Матриця когорт",
        "curveTitle": "Криві утримання когорт",
        "summaryTitle": "Зведення за когортами",
        "totalClicks": "Усього кліків",
        "totalRevenue": "Усього дохід",
        "totalProfit": "Усього прибуток",
        "avgRoi": "Середній ROI",
        "campaigns": "Кампанії",
        "firstSeen": "Перший період",
        "lastSeen": "Останній період",
        "exportCsv": "Експорт CSV",
        "helpMetric": "Оберіть, яка метрика заповнює клітинки теплокарти.",
        "helpGranularity": "Групувати когорти за місяцями чи за кварталами."
    },
    "adminMenu": {
        "mcp": "ІІ / MCP",
        "users": "Користувачі",
        "branding": "Персоналізація",
        "geoProfiles": "Географічні профілі",
        "postback": "URL-адреса повернення",
        "settings": "Налаштування",
        "integrations": "Інтеграції",
        "conversions": "Конверсії",
        "status": "Статус",
        "logs": "Журнали",
        "archive": "Архів",
        "import": "Імпорт конверсій",
        "simulation": "Симуляція дорожнього руху",
        "cleanup": "Очистити статистику",
        "geoDbs": "Географічні бази даних",
        "migrations": "Міграції",
        "update": "Оновлення",
        "aggregator": "Агрегатор доходів",
        "feedback": "Відгук і підтримка"
    },
    "feedback": {
        "formTitle": "Надіслати відгук",
        "formDesc": "Маєте запитання, пропозицію чи знайшли помилку? Дайте нам знати!",
        "topic": "Тема",
        "topicBug": "Повідомити про помилку",
        "topicSuggestion": "Пропозиція функції",
        "topicQuestion": "Загальне питання",
        "email": "Ваша електронна пошта (для відповіді)",
        "message": "повідомлення",
        "messagePlaceholder": "Детально опишіть свою ідею чи проблему...",
        "sendButton": "Надіслати відгук",
        "appreciationTitle": "Дякуємо за вибір Orbitra.link!",
        "appreciationText": "Я розробляю цей трекер як легку, незалежну альтернативу для спільноти. Ваша підтримка допомагає мені підтримувати проект і додавати нові функції.",
        "cryptoTitle": "Підтримайте проект",
        "copied": "Скопійовано!",
        "copy": "Копія",
        "success": "Ваше повідомлення успішно надіслано! дякую",
        "error": "Не вдалося надіслати повідомлення. Спробуйте ще раз.",
        "errorNetwork": "Помилка мережі. Не вдалося підключитися до сервера.",
        "partnersTitle": "Партнери",
        "partnersDesc": "Сервіси, які ми рекомендуємо для роботи з рекламою та не лише.",
        "partnerPay2houseDesc": "Віртуальні картки для оплати реклами у Facebook, Google, TikTok Ads та онлайн-сервісів.",
        "partnerGroupbuyseoDesc": "Спільний доступ до SEO та маркетингових інструментів — Ahrefs, Semrush, ChatGPT Plus та інших."
    },
    "profile": {
        "title": "Налаштування профілю",
        "language": "Мова інтерфейсу",
        "timezone": "Часовий пояс",
        "firstDayOfWeek": "Перший день тижня",
        "monday": "понеділок",
        "sunday": "неділя",
        "changePassword": "Змінити пароль",
        "newPassword": "Новий пароль",
        "newPasswordPlaceholder": "Залиште порожнім, якщо не змінено",
        "confirmPassword": "Підтвердьте пароль",
        "confirmPasswordPlaceholder": "Повторіть новий пароль",
        "passwordsNotMatch": "Паролі не збігаються",
        "saveSuccess": "Налаштування успішно збережено",
        "stayInEditorAfterSave": "Залишатися в редакторі після збереження",
        "stayInEditorHint": "Не закривати кампанії, лендінги та офери після натискання кнопки «Зберегти».",
        "saveAndClose": "Зберегти та закрити"
    },
    "setup": {
        "title": "Початкове налаштування",
        "step1Title": "Створити адміністратора",
        "step1Desc": "Створіть основний обліковий запис користувача",
        "step2Title": "Налаштування",
        "step2Desc": "Вкажіть свої переваги",
        "step3Title": "Готово!",
        "step3Desc": "Користувача створено успішно",
        "username": "Ім'я користувача",
        "password": "Пароль",
        "passwordMin": "Мінімум 6 символів",
        "confirmPassword": "Підтвердьте пароль",
        "confirmPasswordPlaceholder": "Повторіть пароль",
        "continue": "Продовжити",
        "login": "авторизуватися",
        "timezone": "Часовий пояс",
        "language": "Мова інтерфейсу",
        "passwordRecovery": "Відновлення пароля",
        "recoveryHint": "Якщо ви забули пароль, запустіть на сервері:"
    },
    "login": {
        "emptyFields": "Введіть ім'я користувача та пароль",
        "invalidStatus": "Недійсне ім'я користувача або пароль",
        "subtitle": "Система управління дорожнім рухом",
        "title": "Вхід на панель",
        "usernameLabel": "Ім'я користувача",
        "flowTitle": "Кожен клік — маршрут у реальному часі",
        "legendLive": "Спрямовано на живий офер",
        "legendSafe": "Відведено на безпечну сторінку",
        "trackerOnline": "Трекер онлайн",
        "trackerOffline": "Трекер недоступний",
        "trackerChecking": "Перевірка трекера…",
        "usernamePlaceholder": "Введіть ім'я користувача",
        "passwordLabel": "Пароль",
        "passwordPlaceholder": "Введіть пароль",
        "rememberMe": "пам'ятай мене",
        "forgotPassword": "Забули пароль?",
        "loggingIn": "Вхід...",
        "loginButton": "авторизуватися",
        "allRightsReserved": "Всі права захищені.",
        "recoveryTitle": "Відновлення пароля",
        "recoveryInstruction": "Щоб відновити пароль, виконайте таку команду на сервері через SSH:",
        "terminal": "Термінал",
        "alternativeSqLite": "Альтернатива через SQLite:",
        "recoveryFooter": "Після зміни пароля ви можете увійти з новим паролем.",
        "cliPhp": "php cli/reset_password.php YOUR_LOGIN YOUR_NEW_PASSWORD",
        "cliSqlite": "php -r \"require 'config.php'; \\$pdo->prepare('UPDATE users SET password=?, is_active=1 WHERE username=?')->execute([password_hash('NEW_PASSWORD', PASSWORD_DEFAULT), 'YOUR_LOGIN']);\""
    },
    "dashboard": {
        "today": "Сьогодні",
        "yesterday": "вчора",
        "thisWeek": "Цей тиждень",
        "last7Days": "Останні 7 днів",
        "thisMonth": "Цей місяць",
        "last30Days": "Останні 30 днів",
        "customRange": "Спеціальний діапазон...",
        "allTime": "Весь час",
        "allCampaigns": "Усі кампанії",
        "dashboardSettings": "Налаштування інформаційної панелі",
        "selectRangeTitle": "Виберіть діапазон дат",
        "from": "Від",
        "to": "до",
        "colDate": "Дата і час",
        "colCampaign": "Кампанія",
        "colGeo": "GEO",
        "colOs": "ОС",
        "colIp": "IP-адреса",
        "colUa": "Агент користувача",
        "colUrl": "Цільова URL-адреса",
        "metricsCards": "Метричні карти",
        "dataBlocks": "Блоки даних",
        "recentClicksColumns": "Стовпці останніх кліків",
        "saveSettings": "Зберегти налаштування",
        "settingsSubtitle": "Налаштуйте картки метрик, блоки даних і стовпці останніх кліків",
        "selectedMetrics": "Вибрано: {count}",
        "groupTraffic": "Трафік та обсяги",
        "groupFinancial": "Фінанси та прибуток",
        "groupUnitEconomics": "Юніт-економіка",
        "groupRates": "Конверсії та відсотки",
        "quickPresets": "Швидкі пресети",
        "presetGeneral": "Стандартний",
        "presetCod": "COD / Нутра",
        "presetMediaBuying": "Медіабаїнг",
        "trafficDynamics": "Динаміка руху",
        "noStatsData": "Немає статистичних даних",
        "noData": "Немає даних",
        "scale": "Масштаб (%)",
        "topCampaigns": "Кампанії (топ-10)",
        "topOffers": "Пропозиції (10 найкращих)",
        "topLandings": "Посадки (10 найкращих)",
        "topSources": "Джерела (10 найкращих)",
        "tableName": "Ім'я",
        "tableUnique": "унікальний",
        "tableConv": "Конв.",
        "recentClicksLog": "Останні кліки (журнали)",
        "colDirection": "Напрямок",
        "noLogs": "Без журналів",
        "showMoreLogs": "Показати більше (+10)",
        "showLess": "Згорнути",
        "last10Logs": "Останні 10 кліків (натисніть, щоб розгорнути)",
        "viewFullLogs": "Переглянути повні журнали"
    },
    "metrics": {
        "clicks": "Кліки",
        "uniqueClicks": "Унікальні",
        "conversions": "Конверсії",
        "leads": "Ліди",
        "sales": "Продажі",
        "bots": "Боти",
        "cost": "Вартість",
        "revenue": "Дохід",
        "profit": "Прибуток",
        "roi": "ROI",
        "revenueConfirmed": "Підтверджений дохід",
        "profitConfirmed": "Підтверджений прибуток",
        "roiConfirmed": "Підтверджений ROI",
        "cpc": "CPC",
        "cpv": "CPV — вартість за візит",
        "cpa": "CPA",
        "cpl": "CPL",
        "cps": "CPS",
        "cr": "CR %",
        "approveRate": "Апрув %",
        "lpCtr": "LP CTR %",
        "botRate": "Боти %",
        "ctr": "CTR",
        "visits": "Візити",
        "uniqueVisits": "Унік. візити",
        "trash": "Сміття",
        "approve": "Апрув %",
        "epc": "EPC",
        "epv": "EPV",
        "clicksHint": "Кліки по оферу: відвідувачі, відправлені на офер — з каталогу або за «Прямим URL» потоку — одразу, плюс покази лендингу, де відвідувач справді перейшов за посиланням оферу. Потоки-дії та safe page кліками не вважаються. Усі хіти — це Visitors",
        "visitorsHint": "Усі хіти URL кампанії — покази лендінгу, прямі переходи та інше; знаменник CPV/EPV",
        "lpViewsHint": "Усього переглядів лендінгу",
        "lpClicksHint": "Усього кліків на CTA-кнопки лендінгу",
        "lpCtrHint": "Пробив лендінгу (Кліки / Перегляди) — на прямих офферах прочерк: CTA немає",
        "realLpClicksHint": "Перегляди лендінгу, після яких відвідувач реально пішов за посиланням оффера (перехід зафіксовано) — а не передобраний стрімом офер",
        "realOfferClicksHint": "Кліки, реально дійшлі до оффера: прямі плюс завершені переходи з лендінгу",
        "realLpCtrHint": "(Реальні переходи LP / Перегляди LP) × 100% — чесна конверсія CTA лендінгу",
        "timeOnLpHint": "Середній час на лендінгу за ВСІМА візитами, включно з тими, хто не натиснув оффер — вимірює таймер самого лендінга",
        "lpBounceRateHint": "Частка виміряних візитів коротших за 5 секунд. Знаменник — виміряні візити, а не кліки",
        "lpScrollDepthHint": "Середня максимальна глибина скролу лендінга, 0-100%",
        "lpMeasuredHint": "Візити, які справді відзвітували про час — вибірка для «Час на ЛП», відмов і скролу",
        "pwaIntentsHint": "Натискання кнопки «Встановити» на PWA-лендінгу (до системного промпта)",
        "pwaInstallsHint": "Події встановлення PWA-лендінгу, включно з ботами",
        "realPwaInstallsHint": "Встановлення PWA з неботних кліків — appinstalled (Android) або перше standalone-відкриття (iOS)",
        "pwaOpensHint": "Повторні відкриття встановленого PWA з головного екрана (не частіше одного разу на 10 хвилин на клік)",
        "pwaInstallRateHint": "(Реальні встановлення PWA / кліки) × 100% — у рядку PWA-лендінгу це і є перегляд→встановлення; прочерк за нуля встановлень",
        "pushSubscribedHint": "Кліки, де відвідувач дозволив повідомлення та підписку збережено в базу",
        "pwaScreenViewsHint": "Сумарні покази екранів PWA-воронки: кожен показ екрана відвідувачу — один візит може дати кілька",
        "cpvHint": "Витрати на візит — універсальна: Витрати / усі вхідні візити (прямі оффери і лендінги)",
        "cpcHint": "Витрати на клік по CTA лендінгу (Витрати / LP-кліки) — на прямих офферах дорівнює CPV",
        "epvHint": "Дохід на візит — універсальна: Дохід / усі вхідні візити (прямі оффери і лендінги)",
        "epcHint": "Дохід на клік по CTA лендінгу (Дохід / LP-кліки) — на прямих офферах дорівнює EPV",
        "realRevenue": "Реальний дохід",
        "realRoi": "Реальний ROI",
        "marginHint": "Прибуток ÷ дохід × 100%. Не плутати з ROI: там прибуток ÷ витрати",
        "uniqueClicksHint": "Відвідувачі, пораховані один раз на кампанію за IP (або IP + user agent) у межах вікна унікальності кампанії",
        "conversionsHint": "Усі конверсії, що прийшли постбеком, у будь-якому статусі (лід, продаж, відмова, треш)",
        "uniqueClicksStreamHint": "Відвідувачі, пораховані один раз на потік: хто потрапив у два потоки, рахується в кожному",
        "uniqueClicksGlobalHint": "Відвідувачі, пораховані один раз по всіх кампаніях трекера",
        "botsHint": "Хіти, у яких user agent схожий на краулер чи інструмент (ті самі сигнатури, що в клоаки). Вони входять і у Visitors",
        "botRateHint": "Боти ÷ Visitors × 100%",
        "proxiesHint": "Хіти з проксі / VPN / Tor. Потрібна база IP2Proxy (Налаштування → Гео бази)",
        "emptyReferrersHint": "Хіти без реферера — прямий ввід, in-app браузери або джерело, що його зрізає",
        "crHint": "Конверсії ÷ Кліки × 100%"
    },
    "campaigns": {
        "allGroups": "Усі групи",
        "groups": "Групи",
        "sources": "Джерела",
        "allSources": "Всі джерела трафіку",
        "campaign": "Кампанія",
        "campaigns": "Кампанії",
        "group": "Група",
        "unique": "Унікальний",
        "noCampaignsCreated": "Кампанії не створено",
        "pauseRemoteTitle": "Зупинити кампанію та рекламні кабінети?",
        "pauseRemoteWarning": "Це призупинить кампанію в трекері та миттєво ЗУПИНИТЬ усі пов'язані кампанії в рекламних кабінетах.",
        "pauseRemoteLinked": "Будуть зупинені:",
        "confirmPauseAll": "Зупинити всю рекламу",
        "createFirstCampaign": "Ви можете створити першу кампанію",
        "updateCosts": "Витрати на оновлення",
        "clearStatsWarning": "Усі кліки та переходи для цієї кампанії буде видалено. Цю дію не можна скасувати.",
        "costAmount": "Вартість ($)",
        "startDate": "Дата початку",
        "endDate": "Дата закінчення",
        "distributeUniqueOnly": "Розподіл тільки між унікальними кліками",
        "deleteConfirm": "Видалити кампанію? Цю дію не можна скасувати.",
        "updatedClicks": "Оновлені кліки: {count}",
        "copySelected": "Вибрана копія",
        "searchPlaceholder": "Пошук кампаній...",
        "bulkCopyConfirm": "Копіювати вибрані кампанії? Буде створено копії з новими псевдонімами та маркерами.",
        "copied": "Скопійовано",
        "copyErrors": "Помилки"
    },
    "rotationAuto": {
        "auto": "Авто",
        "toggleOnHint": "Автооптимізація увімкнена: cron перераховує ваги за обраною метрикою. Вимкніть, щоб заморозити й редагувати вручну.",
        "toggleOffHint": "Увімкніть, щоб оптимізатор автоматично перебудовував ваги ротації",
        "conditions": "Умови",
        "conditionsTitle": "Умови оптимізації",
        "metricLabel": "Метрика",
        "metricSales": "Підтверджені продажі",
        "metricCr": "CR (підтверджений)",
        "metricEpv": "EPV (підтверджений)",
        "metricEpc": "EPC (підтверджений)",
        "metricRoi": "ROI (підтверджений)",
        "needsCost": "потрібні витрати",
        "needsCostHint": "ROI потребує витрат: підключіть синк витрат або задайте ручну вартість кампанії.",
        "minSampleLabel": "Підтверджені продажі до участі в розподілі",
        "minSampleHint": "Поки стільки продажів не набрано, елемент тримає рівну частку — не виграє і не програє трафік.",
        "lookbackPrefix": "Дивимось на останні",
        "lookbackUnitForms": "дн.",
        "lookbackHint": "Рахуються лише конверсії всередині цього вікна. Старіші не враховуються.",
        "floorPrefix": "Ніколи не надсилати менше",
        "floorHint": "Той, хто програє, зберігає цю частку — буде шанс відігратися.",
        "capPrefix": "Ніколи не надсилати більше",
        "capHint": "Навіть лідер не отримує більше — решта залишаються вимірюваними.",
        "intervalPrefix": "Перераховувати кожні",
        "intervalSuffix": "хв",
        "intervalHint": "Як часто перераховуються ваги.",
        "pctSuffix": "%",
        "exampleShare": "Зараз {count} {items}, кожному по {share}%.",
        "exampleNeeds": "{Item} починає забирати трафік, коли набере {min} {sales} за останні {days} {daysWord}.",
        "exampleBounds": "Нижче {floor}% і вище {cap}% не буває, а ваги зсуваються щонайбільше на {move} {points} за годину.",
        "exampleFew": "Увімкніть принаймні два елементи — інакше ділити нічого.",
        "itemOfferForms": "офер|офера|оферів",
        "itemLandingForms": "лендінг|лендінги|лендінгів",
        "itemOfferOne": "офер",
        "itemLandingOne": "лендінг",
        "salesForms": "підтверджену продаж|підтверджені продажі|підтверджених продажів",
        "daysForms": "день|дні|днів",
        "pointsForms": "пункт|пункти|пунктів",
        "lastUpdated": "Оновлено",
        "waitingFirstRun": "Очікування першого проходу оптимізатора…",
        "skippedNoCost": "Пропущено: у кампанії немає витрат",
        "window": "вікно",
        "sampleLabel": "вибірка",
        "managedWeight": "Керується автооптимізацією — вагу перераховує cron",
        "equalizeBlocked": "«Поділити порівну» недоступне, поки списком керує Авто"
    },

    "editor": {
        "collectClicks": "Збирати кліки",
        "collectClicksHint": "Записувати кліки в статистику та звіти для цього потоку. Вимкнено: відвідувач одержує напрямок, але рядок кліку не пишеться — конверсії з цього потоку нічого прив’язати.",
        "collectClicksDisabledNote": "Кліки для цього потоку не записуються в статистику",
        "showText": "Показати текст / порожньо",
        "toCampaign": "Надіслати в кампанію",
        "selectCampaign": "— оберіть кампанію —",
        "textPayloadHint": "текст; порожньо = порожня біла сторінка",
        "tracking": "Tracking",
        "integrationCode": "Код інтеграції",
        "intCode_link": "Посилання",
        "intCode_iframe": "Iframe",
        "intCode_script": "Скрипт",
        "offerSelection": "Вибір офера",
        "offerSelectionBefore": "Перед кліком",
        "offerSelectionAfter": "Після кліку",
        "offerSelectionBeforeHint": "Зараховує кожен показ лендінгу як перехід на офер — переходи LP та LP CTR майже завжди ~100% (стара поведінка)",
        "offerSelectionAfterHint": "Перехід зараховується лише за реального кліку на офер — чесні переходи LP, LP CTR та статистика часу на лендінгу",
        "offerSelectionHint": "Коли визначається офер. «Після кліку» обирає його в момент переходу з лендінга — тоді слот, що закінчився, поки людина читала, не виявиться вже витраченим на неї.",
        "quickCreateLanding": "Створити лендінг",
        "quickCreateOffer": "Створити офер",
        "quickCreateName": "Назва",
        "quickCreateType": "Тип",
        "quickCreateZip": "ZIP-архів",
        "quickCreateZipHint": "Необовʼязково — можна завантажити пізніше з редактора лендінга.",
        "quickCreateEditFilesHint": "Керуйте файлами цього лендінга на сторінці «Лендінги».",
        "quickCreateError": "Не вдалося створити",
        "editLanding": "Редагувати лендінг",
        "newCampaign": "Нова кампанія",
        "createCampaign": "Створити кампанію",
        "campaign": "Кампанія",
        "unsavedChanges": "Є незбережені зміни. Вийти без збереження?",
        "unsavedLeave": "Вийти без збереження",
        "unsavedStay": "Продовжити редагування",
        "fillNameAndAlias": "Будь ласка, введіть ім'я та псевдонім",
        "saved": "Збережено!",
        "save": "зберегти",
        "copyUrl": "Копіювати URL",
        "clickLog": "Натисніть Журнал",
        "reports": "Звіти",
        "fullReportCsv": "Повний звіт (CSV)",
        "conversionsLog": "Журнал переходів",
        "trafficSimulation": "Симуляція дорожнього руху",
        "runSimulation": "Запустити симуляцію",
        "ipAddress": "IP-адреса",
        "userAgent": "Агент користувача",
        "country": "Країна",
        "deviceType": "Тип пристрою",
        "language": "Мова",
        "trafficAsn": "ASN (необов'язково)",
        "trafficIsp": "ISP / Організація (необов'язково)",
        "jsExecuted": "JavaScript виконано",
        "general": "Загальний",
        "finance": "Фінанси",
        "params": "Параметри",
        "integrations": "Інтеграції",
        "notes": "Примітки",
        "name": "Ім'я *",
        "alias": "псевдонім *",
        "generateRandom": "Генерувати випадкові",
        "autoFormat": "Автоформат",
        "autoFormatHint": "Зібрати назву з оферу першого потоку, джерела трафіку та GEO оферу: Назва - Джерело - [GEO]",
        "nameFormatPrefix": "Шаблон:",
        "nameSourceOrganic": "Органіка",
        "noGroup": "-- Немає групи --",
        "domain": "Домен",
        "indexDomain": "-- Індексний домен --",
        "trafficSource": "Джерело трафіку",
        "organicTraffic": "-- Органічний трафік --",
        "uniqueness": "Унікальність",
        "uniquenessHours": "Період унікальності (годин)",
        "uniquenessIp": "IP",
        "uniquenessIpUa": "IP та User-Agent",
        "uniquenessCookies": "Печиво",
        "rotationType": "Тип обертання",
        "clickApiToken": "Натисніть Маркер API",
        "clickApiTokenHint": "Унікальний маркер для Click API. Використовується в сценаріях інтеграції замість кампанії. Оригінальний маркер зберігається під час міграції Keitaro.",
        "regenerateToken": "Регенерувати маркер",
        "regenerateTokenConfirm": "Згенерувати новий маркер? Старий маркер перестане працювати, і всі сценарії, які його використовують, більше не відстежуватимуть кліки.",
        "streamWeight": "Вага потоку",
        "streamWeightHelp": "Використовується лише тоді, коли для обертання потоку встановлено зважене значення.",
        "totalWeight": "Сумарна вага потоків",
        "equalizeSplit": "Розділити порівну",
        "campaignUrl": "URL-адреса кампанії",
        "rewardModel": "Модель витрат",
        "costValue": "Вартість",
        "setupParams": "Налаштувати відображення параметрів",
        "selectIntegration": "Виберіть інтеграцію",
        "addPostback": "+ Додати повернення",
        "pb": {
            "insertMacro": "Вставити макрос",
            "sourceHint": "Джерело «{name}» уміє приймати S2S-постбек, якого в кампанії немає — додайте, щоб конверсії йшли в джерело",
            "backfillBtn": "Дослати пропущені ({hours} год)",
            "backfillHint": "якщо S2S замовк після оновлення — поставить у чергу конверсії, що не потрапили у відправку",
            "backfillDone": "У чергу поставлено {queued} відправок (переглянуто конверсій: {scanned}, уже надіслано: {existing}, не пройшли фільтр: {filtered}) — воркер надішле їх протягом хвилини",
            "addSource": "Додати S2S джерела",
            "customStatus": "свій статус…"
        },
        "pbTest": {
            "title": "Перевірка постбеку",
            "run": "Запустити тест",
            "needSaved": "Збережіть кампанію, щоб перевірити її постбек",
            "recorded": "«{from}» записано як «{to}» — конверсія дійшла",
            "noMapping": "«{from}» записано як «custom» — створіть тип конверсії зі значенням «{from}» (Налаштування → Типи конверсій)",
            "notRecorded": "Конверсію НЕ записано (HTTP {code}) — дивіться системний лог нижче",
            "queued": "поставлено в чергу, надішле воркер",
            "filterMiss": "фільтр статусів «{filter}» не пропускає «{status}» — додайте статус у список вище",
            "probeOk": "доставлено: HTTP {code} · {ms} мс",
            "probeFail": "не доставлено: {err}",
            "workerAlive": "Воркер черги живий (пінг {sec} с тому)",
            "workerDead": "Воркер черги не відповідає — S2S не підуть. Крон і діагноз: Налаштування → Автоматизація",
            "workerNever": "Воркер черги жодного разу не відзначався — встановіть крон у Налаштування → Автоматизація",
            "macrosLeft": "в URL не підставилися: {list} — переконайтеся, що трафік приходить із цими параметрами (макроси пішли в джерело як текст)",
            "placeholdersLeft": "в URL лишилися заглушки шаблону: {list} — замініть їх на реальні значення з кабінету джерела",
            "filterViaOriginal": "постбек пройшов фільтр за вихідним словом «{word}» — надійніше увімкнути чіп «{status}» у статусах цього постбеку",
            "filterViaAlias": "постбек пройшов через чіп custom — до 1.5.11 слово «{word}» записувалося як custom; увімкніть чіп «{status}», щоб налаштування не залежало від сумісності",
        },
        "statuses": "Статуси",
        "yourNotes": "Ваші нотатки...",
        "streams": "Потоки",
        "createStream": "Створити потік",
        "streamStatsPeriod": "Період статистики потоків",
        "streamStatsToday": "Сьогодні",
        "streamStatsYesterday": "Вчора",
        "streamStats7d": "7 днів",
        "streamStats30d": "30 днів",
        "streamStatsRefresh": "Оновити статистику",
        "streamStatsHint": "Скільки візитів потрапило в потік за період, включно з ботами та safe page",
        "streamStatsVisitors": "Візити",
        "streamStatsUnique": "Унік.",
        "streamStatsBots": "Боти",
        "streamStatsUnsaved": "Збережіть кампанію, щоб бачити статистику",
        "streamStatsOther": "Ще {visitors} візитів ({share}%) записано на видалені потоки або старі ID потоків — до v1.5.14 кожне збереження кампанії перестворювало потоки.",
        "streamDefaultName": "Потік {n}",
        "streamIntercepting": "Перехоплення",
        "streamInterceptingShort": "ПЕРЕХВАТ",
        "streamRegular": "Регулярний",
        "streamRegularShort": "РЕГУЛЯРНИЙ",
        "streamFallback": "Запасний варіант",
        "streamFallbackShort": "ЗАПАС",
        "noStreamsTitle": "Немає доступних потоків",
        "noStreamsDesc": "У цій кампанії ще не створено жодного потоку. Додайте перший потік для отримання трафіку.",
        "streamName": "Назва потоку",
        "on": "Увімкнено",
        "moveUp": "Рухатися вгору",
        "moveDown": "Рухатися вниз",
        "duplicate": "дублікат",
        "schema": "Схема",
        "directLinking": "Пряме посилання на пропозицію",
        "landingOffer": "Посадка + Пропозиція",
        "action": "Дія",
        "selectAction": "-- Дія --",
        "doNothing": "Нічого не робити",
        "show404": "Показати 404",
        "showHtml": "Показати HTML",
        "add": "Додати",
        "addOffers": "Додати оферів",
        "addLandings": "Додати посадки",
        "createOfferDropdown": "Створити офер",
        "createLandingDropdown": "Створити посадку",
        "selectAll": "Вибрати все",
        "noLandingsAdded": "Посадки не додано. Трафік піде одразу на офери.",
        "noOffersAdded": "Офери не додано. Додайте їх через «Додати оферів».",
        "selectLandingPlaceholder": "Натисніть, щоб вибрати посадку…",
        "selectOfferPlaceholder": "Натисніть, щоб вибрати офер…",
        "weight": "Вага (частка трафіку)",
        "filters": "Фільтри",
        "addFilter": "+ Додати фільтр",
        "filtersLogicHint": "Комбінація фільтрів: AND — має виконатися всі, OR — достатньо одного",
        "editFilter": "Редагувати фільтр",
        "noFilters": "Без фільтрів — весь трафік пропускається",
        "filterType": "Тип",
        "filterMode": "Режим",
        "allow": "Дозволити",
        "deny": "Заперечувати",
        "values": "Цінності",
        "newStream": "Новий потік",
        "copy": "Копія",
        "landings": "Посадки",
        "offers": "Пропозиції",
        "landingInfo": "-- Посадка --",
        "offerInfo": "-- Пропозиція --",
        "addOffersHelp": "Натисніть «+ Додати», щоб вибрати пропозиції.",
        "weightWarning": "Сума ваг перевищує 100% (на даний момент",
        "weightWarningEnd": "%). Відкоригуйте значення.",
        "timePeriod": "Інтервал часу",
        "costAmount": "Вартість",
        "onlyUniqueClicks": "Тільки за унікальні кліки",
        "noLogs": "Без журналів",
        "geoPlaceholder": "Введіть або виберіть країни..."
    },
    "rotationType": {
        "position": "За посадою",
        "weight": "За вагою"
    },
    "filters": {
        "country": "Країна",
        "device": "пристрій",
        "os": "Операційна система",
        "browser": "Браузер",
        "bot": "Бот",
        "botYes": "Так — підозрілий трафік",
        "language": "Мова браузера",
        "isp": "Інтернет-провайдер",
        "ispPlaceholder": "Comcast, Verizon, Vodafone...",
        "connection": "Тип підключення",
        "ip": "IP-адреса",
        "keyword": "Ключове слово",
        "referer": "реферер",
        "weekday": "День тижня",
        "time": "час"
    },
    "landings": {
        "title": "Посадки",
        "allStates": "Усі статуси",
        "searchPlaceholder": "Знайти лендінг...",
        "noGroup": "Без групи",
        "noLandings": "Посадки не завантажені",
        "noLandingsDesc": "Ви можете створити або завантажити першу цільову сторінку"
    },
    "offers": {
        "title": "Пропозиції",
        "titleSingular": "Пропозиція",
        "createOffer": "Створити пропозицію",
        "noOffers": "Пропозицій не створено",
        "noOffersDesc": "Ви можете створити свою першу пропозицію",
        "noOffersFiltered": "Немає пропозицій, що відповідають цим фільтрам",
        "changeFilters": "Зміна параметрів фільтра",
        "network": "Партнерська мережа",
        "allGroups": "Всі групи",
        "allNetworks": "Всі партнерські мережі",
        "allStates": "Всі статуси",
        "inactiveStates": "В архіві / Вимкнені",
        "localOffer": "Місцева пропозиція",
        "redirect": "Перенаправлення",
        "iframe": "IFrame",
        "local": "Місцевий",
        "total": "Усього ({count} пропозицій)",
        "copySelected": "Вибрана копія",
        "searchPlaceholder": "Пошук офферів...",
        "bulkCopyConfirm": "Копіювати вибрані пропозиції?",
        "copied": "Скопійовано",
        "copyErrors": "Помилки"
    },
    "sources": {
        "title": "Джерела трафіку",
        "noSources": "Джерела не створені",
        "noSourcesDesc": "Ви можете створити своє перше джерело трафіку",
        "search": "Пошук джерел...",
        "pausedPlural": "Призупинено",
        "activePlural": "Активний",
        "template": "Шаблон",
        "notFound": "Нічого не знайдено",
        "noSourcesAdd": "Немає джерел трафіку. Натисніть «Створити», щоб додати.",
        "customTemplate": "Custom",
        "infoDesc": "Джерела трафіку – це шаблони для отримання UTM-тегів рекламної мережі та надсилання S2S-повернень (конверсій).",
        "infoList1": "• Виберіть шаблон зі списку популярних рекламних мереж",
        "infoList2": "• Налаштувати параметри для передачі тегів у кампанію",
        "infoList3": "• Укажіть URL-адресу зворотного зв’язку S2S для надсилання конверсій назад до джерела",
        "infoList4": "• Пов’яжіть джерело з кампанією, щоб автоматично додавати параметри",
        "deleteConfirm": "Видалити це джерело? Кампанії буде від’єднано.",
        "checkAllUrls": "Перевірте всі URL-адреси",
        "importBtn": "Імпорт",
        "urlStatus": "URL / Статус",
        "check": "Перевірте"
    },
    "networks": {
        "title": "Партнерські мережі",
        "noNetworks": "Мереж не створено",
        "noNetworksDesc": "Ви можете створити свою першу партнерську мережу",
        "deleteConfirm": "Видалити цю афілійовану мережу? Пропозиції буде від’єднано.",
        "headerDesc": "Партнерські мережі отримують зворотні повідомлення від трекера після конвертації. Налаштуйте URL-адресу зворотного зв’язку на інформаційній панелі вашої мережі.",
        "noNetworksAdd": "Немає партнерських мереж. Натисніть «Створити», щоб додати.",
        "postbackUrl": "URL-адреса повернення",
        "offerParams": "Параметри пропозиції",
        "offersCount": "Пропозиції",
        "setupPostback": "Як налаштувати Postback",
        "step1": "Створіть партнерську мережу або виберіть існуючу",
        "step2": "Скопіюйте URL-адресу повернення даних зі списку",
        "step3": "Вставте URL-адресу на інформаційну панель партнерської мережі (Global Postback)",
        "step4": "Переконайтеся, що subid передається в пропозицію через «Параметри пропозиції»",
        "step5": "Перевірте вхідні конверсії на вкладці «Конверсії».",
        "examplePostback": "Приклад зворотної URL-адреси"
    },
    "affiliateNetworks": {
        "postbackUrl": "Postback URL",
        "offerParams": "Параметри оффера",
        "useNetworkTemplate": "Шаблон партнерки",
        "useGenericTemplate": "Стандартний Postback",
        "resetToTemplate": "Скинути до шаблону мережі"
    },
    "components": {
        "aliasName": "Псевдонім / Ім'я",
        "group": "Група",
        "type": "Тип",
        "status": "Статус",
        "clicks": "Кліки",
        "uniques": "Унікальні",
        "lpClicks": "Переходи LP",
        "lpCtr": "LP CTR",
        "active": "Активний",
        "archive": "Архів",
        "edit": "Редагувати",
        "paused": "Призупинено"
    },
    "domains": {
        "group": "Група",
        "noGroup": "Без групи",
        "createGroup": "Створити",
        "adminDashboard": "Адмін-панель",
        "allowAccess": "Дозволити",
        "denyAccess": "Заборонити",
        "allowRobotsShort": "Дозволити",
        "disallowShort": "Заборонити",
        "on": "Увімк",
        "off": "Вимк",
        "httpsOnlyShort": "Лише HTTPS",
        "cloudflareProxy": "Проксі Cloudflare",
        "registrar": "Реєстратор",
        "optional": "Опціонально",
        "dnsProvider": "DNS-провайдер",
        "manageVia": "Керувати DNS через",
        "manageViaNone": "— акаунт за замовчуванням —",
        "manageViaHint": "Акаунт, через який паркується та синхронізується A-запис цього домену.",
        "statusOk": "OK",
        "statusActive": "Активний",
        "statusDisabled": "Вимкнено",
        "addMore": "Додати ще",
        "savedAddMore": "Збережено — можна вводити наступний домен",
        "parkingSsl": "Паркуємо — випускаємо сертифікати… (до ~30 с, решта за кілька хвилин)",
        "domainBulkHelper": "Щоб додати кілька доменів, введіть список через кому. HTTP(S) очищається автоматично.",
        "adminAccessHint": "Якщо доступ заборонено, адмін-панель і API на цьому домені повертають 404. Панель залишається доступною через інші домени та по IP сервера.",
        "cfProxyHint": "Edge Cloudflare завершує TLS для відвідувача. Оріджин все одно має відповідати HTTPS на порту 443 — Orbitra налаштовує це автоматично з самопідписаним сертифікатом. Використовуйте режим SSL Cloudflare ‘Full’. ‘Flexible’ не рекомендується: з’єднання між Cloudflare та ориджином йде у відкритому вигляді.",
        "customSslCert": "Власний SSL-сертифікат (Full Strict)",
        "customSslHint": "Для режиму Cloudflare Full Strict вкажіть шляхи до файлів сертифікату та ключа Cloudflare Origin CA на сервері. Залиште пустим для автоматичного управління (Let’s Encrypt або самопідписаний).",
        "customCertPath": "Шлях до сертифіката",
        "customKeyPath": "Шлях до ключа",
        "customSslWarning": "Файли сертифіката повинні існувати на сервері. Дивіться документацію для інструкцій налаштування.",
        "sslMode": "Режим SSL",
        "sslLetsEncrypt": "Let's Encrypt",
        "sslCloudflare": "Cloudflare",
        "sslCustom": "Власний",
        "sslSelfSigned": "Самопідписаний",
        "sslModeAutoHint": "Режим не обрано — Orbitra вирішить сама: Cloudflare для проксованих доменів, інакше Let\u2019s Encrypt.",
        "sslLetsEncryptHint": "Сертифікат видається автоматично. Спрямуйте A-запис на IP сервера.",
        "sslCloudflareHint": "Cloudflare обслуговує SSL-сертифікат. Самопідписаний сертифікат на сервері для режиму Full SSL.",
        "sslCustomHint": "Вкажіть шляхи до власних файлів сертифіката та ключа.",
        "title": "Парковані домени",
        "serverIp": "IP сервера:",
        "searchPlaceholder": "Пошук доменів...",
        "addDomain": "Додати домен",
        "domain": "Домен",
        "indexPage": "Індексна сторінка",
        "https": "HTTPS",
        "dateAdded": "Дата додавання",
        "actions": "Дії",
        "status": "Статус",
        "ok": "добре",
        "awaitingDns": "Очікування DNS",
        "activeCloudflare": "Активний (Cloudflare)",
        "activeLocalhost": "Активний (Localhost)",
        "wrongIp": "Невірна IP",
        "dnsReasonNoResolve": "Домен не резолвиться в IP-адресу",
        "dnsReasonWrongIp": "Домен резолвиться в {{ip}} замість IP сервера",
        "ignoreDnsLabel": "Ігнорувати DNS (UI)",
        "ignoreDnsHint": "Тільки для інтерфейсу користувача: показувати всі домени як OK, навіть якщо DNS ще не налаштовано. Корисно для міграції/тестування.",
        "notSelected": "Не вибрано",
        "loading": "Завантаження...",
        "noDomains": "Немає доданих доменів",
        "editDomain": "Редагувати домен",
        "addDomainTitle": "Додати домен",
        "domainName": "Доменне ім'я",
        "domainNameHint": "(наприклад: example.com)",
        "indexPageLabel": "Індексна сторінка (кампанія за умовчанням)",
        "indexPageHint": "Відкривається при прямому доступі до домену без псевдоніма.",
        "catch404": "Лови 404",
        "searchRobots": "Пошукові роботи",
        "disallowRobots": "Заборонити (noindex, Disallow: /)",
        "allowRobots": "Дозволити індексацію",
        "robotsHint": "Рекомендується заборонити індексацію посилань кампанії, щоб уникнути трафіку ботів.",
        "httpsOnly": "Лише HTTPS (примусове перенаправлення 301)",
        "deleteConfirm": "Видалити домен? Це також розірве посилання кампаній, які його використовують.",
        "dnsTitle": "Очікування DNS",
        "dnsInstruction": "Перейдіть до налаштувань DNS у свого реєстратора та встановіть A-запис:",
        "dnsNote1": "Реєстратор оновлює записи DNS протягом 24 годин.",
        "dnsNote2": "Next check on page refresh.",
        "forceCheckConfirm": "Check DNS for all domains? This may take a moment.",
        "forceChecking": "Checking DNS...",
        "forceCheckTitle": "Force-check DNS for all domains",
        "checkDns": "Check DNS",
        "checkingShort": "Checking...",
        "bulkHint": "multiple, comma-separated",
        "bulkPlaceholder": "domain1.com, domain2.com, domain3.com",
        "bulkExample": "Example: tracker1.com, tracker2.com, tracker3.com",
        "sslStatus": "SSL Status",
        "sslInstalled": "SSL installed",
        "sslCloudflareStatus": "SSL від Cloudflare (проксований домен)",
        "sslInstalling": "SSL installing...",
        "sslEnvAcmeNotWritable": "Каталог для перевірки Let's Encrypt закритий для запису — валідація домену не пройде.",
        "sslCertbotNoOutput": "Certbot нічого не відповів — перевірте, що він встановлений і доступний вебсерверу через sudo.",
        "sslIncompleteChain": "У файлі сертифіката немає проміжного — сайт відкривається у Firefox, але не в Chrome. Перевипустіть сертифікат.",
        "sslChainUnreadable": "Сертифікато видано і сайт працює, але панель не може прочитати файл сертифіката, щоб перевірити ланцюжок — перевірте доступ веб-користувача до /etc/letsencrypt. Це попередження, а не помилка.",
        "sslChainUnverified": "Сертифікат видано, але панель ще не підтвердила ланцюжок: файл сертифіката недоступний панелі, і домен поки не відповідає за HTTPS. Перевірка повториться автоматично — це попередження, а не помилка.",
        "sslChainCheckPending": "Ланцюжок не підтверджено",
        "sslDeleteFailed": "Не вдалося видалити старий сертифікат перед повторним випуском. Перевірте, що веб-користувач може запускати sudo certbot.",
        "sslDnsMismatch": "Домен не вказує на цей сервер. Сертифікат не може бути випущений, поки DNS не пошириться.",
        "reissueSsl": "Перевипустити SSL-сертифікат",
        "reissueConfirm": "Ви впевнені, що хочете перевипустити SSL-сертифікат для",
        "sslIssued": "SSL-сертифікат успішно випущено",
        "sslError": "Не вдалося випустити SSL-сертифікат",
        "sslDnsSeen": "A-запис домену",
        "sslNoARecord": "A-запису немає",
        "sslPending": "Awaiting installation",
        "sslQueued": "SSL-сертифікат встановлюється у фоновому режимі (1–2 хвилини)",
        "sslEnvTitle": "Автовипуск SSL на цьому сервері недоступний",
        "sslEnvNoShell": "PHP тут не може запускати зовнішні команди — ні certbot, ні планувальник звідси не запустити. Домени так і чекатимуть на сертифікат.",
        "sslEnvNoCertbot": "На сервері не встановлено certbot — випускати сертифікати нічим. Встановіть: apt install certbot",
        "sslEnvNoSudoCertbot": "Certbot встановлено, але веб-сервер не може запустити помічник випуску сертифікатів через sudo — тому панель не може їх випускати. Виконайте один раз від root: sudo bash /var/www/orbitra/cli/server_setup.sh",
        "sslEnvNoNginx": "Конфіг nginx не знайдено: схоже, сервер працює не на nginx, і вписати випущений сертифікат автоматично не вийде.",
        "sslEnvHint": "Сертифікати доведеться випускати та підключати засобами вашого хостингу. Автовипуск розрахований на окремий VPS з root-доступом — саме так ставить install.sh.",
        "issueSsl": "Випустити SSL",
        "issueSslTitle": "Запросити сертифікати для доменів, які їх ще не мають",
        "sslRunIssued": "Випущено сертифікатів",
        "sslRunWaitingDns": "Очікують DNS",
        "sslRunFailed": "Не вдалося",
        "sslRunError": "Не вдалося запустити випуск",
        "sslWaitingDns": "Очікування DNS — домен поки не вказує на цей сервер. Сертифікат випишеться автоматично, щойно A-запис розійдеться.",
        "sslAwaitingDnsSwitch": "DNS усе ще вказує на Cloudflare, тому сертифікат для origin поки не можна випустити. Прапорець проксі відновлено відповідно до DNS; повідомлення зникне саме, коли запис розійдеться.",
        "sslRetrying": "Остання спроба випуску не вдалася. Трекер продовжить намагатися автоматично.",
        "sslNotWired": "Сертифікат випущено, але вебсервер його ще не віддає — конфіг перезбирається.",
        "sslFailed": "SSL error",
        "allGroups": "Всі групи",
        "groups": "Групи",
        "groupsTitle": "Керувати групами доменів",
        "bulkChangeGroup": "Змінити групу",
        "bulkChangeGroupTitle": "Змінити групу для вибраних доменів",
        "bulkChangeGroupText": "Змінити групу для {count} вибраних доменів",
        "selectGroup": "Виберіть групу",
        "applyGroup": "Застосувати",
        "bulkDeleteConfirm": "Видалити {count} доменів?",
        "bulkDeleteTitle": "Видалити вибрані домени"
    },
    "backorder": {
        "bannerTitle": "Backorder Domain Monitor",
        "bannerText": "Add a list of domains and check registration availability (Available/Registered). Auto-check is enabled by default (while the page is open) and runs gradually (1 domain per step). For 24/7 checks without an open page, go to Settings → Automation and enable cron (UI can attempt to install into crontab).",
        "openAutomation": "Set up 24/7 (Automation)",
        "import": "Import",
        "importTitle": "Import domains",
        "importPlaceholder": "example.com\nanotherdomain.net\n...",
        "importResult": "Inserted: {inserted}, duplicates: {duplicates}, invalid: {invalid}",
        "deleteSelected": "Delete selected",
        "deleteSelectedConfirm": "Delete selected ({count})? This action cannot be undone.",
        "searchPlaceholder": "Search domains...",
        "domain": "Domain",
        "status": "Status",
        "lastChecked": "Last check",
        "metrics": "Metrics",
        "notes": "Notes",
        "noRows": "No domains added",
        "checkNow": "Check now",
        "editTitle": "Notes",
        "batchRun": "Run checks",
        "batchRunning": "Checking...",
        "batchStarting": "Starting batch checks...",
        "batchProgress": "Checked this session: {session_checked}. Remaining to check: {due_remaining} of {due_total}.",
        "batchDone": "Done: domains checked in this pass.",
        "batchNothingToDo": "Nothing to check (no domains are due right now).",
        "batchStopped": "Stopped",
        "autoRunLabel": "Auto-check (while page is open)",
        "autoStarting": "Розпочато автоматичну перевірку...",
        "autoIdle": "Автоматична перевірка: зараз нічого не потрібно робити (чекає інтервал).",
        "exportTxt": "TXT",
        "exportTxtHint": "Завантажити список доменів (1 домен на рядок). Експортує вибрані домени, якщо такі є, інакше експортує поточний видимий список.",
        "exportCsv": "CSV",
        "exportCsvHint": "Завантажте таблицю з доменами, статусами та помилками. Експортує вибрані домени, якщо такі є, інакше експортує поточний видимий список.",
        "statusAvailable": "в наявності",
        "statusDnsAvailable": "Немає NS (імовірно доступний)",
        "statusRegistered": "Зареєстрований",
        "statusUnknown": "В очікуванні",
        "statusRateLimited": "Оцінка обмежена (429)",
        "statusUnsupported": "Не можу перевірити",
        "statusError": "Помилка"
    },
    "admin": {
        "of": "з",
        "mcp": "ІІ-асистент (MCP)",
        "users": "Користувачі",
        "noGeoDb": "Геобаза даних не встановлена. Географічні фільтри не працюватимуть.",
        "diskCritical": "Критично мало місця на диску! Звільніть місце.",
        "diskWarning": "Мало місця на диску. Рекомендовано очистити старі логи.",
        "cpuCritical": "Високе навантаження на CPU. Розгляньте апгрейд сервера.",
        "cpuWarning": "Підвищене навантаження на CPU.",
        "ramCritical": "Критично мало оперативної пам'яті!",
        "ramWarning": "Мало вільної оперативної пам'яті.",
        "dbOver500": "База даних перевищує 500MB. Розгляньте перехід на MySQL для кращої продуктивності.",
        "dbGrowing": "База даних росте. Після досягнення 500MB рекомендовано перейти на MySQL.",
        "branding": "Персоналізація",
        "geoProfiles": "Географічні профілі",
        "postback": "URL-адреса повернення",
        "settings": "Налаштування",
        "integrations": "Інтеграції",
        "status": "Статус системи",
        "logs": "Журнали",
        "archive": "Архів",
        "import": "Імпорт конверсій",
        "simulation": "Симуляція дорожнього руху",
        "cleanup": "Очистити статистику",
        "geoDbs": "Географічні бази даних",
        "migrations": "Міграції",
        "update": "Оновлення",
        "aggregator": "Агрегатор доходів",
        "systemStatus": "Статус системи",
        "systemNormal": "Система нормальна",
        "attention": "Увага",
        "critical": "Критичний стан",
        "disk": "диск",
        "ram": "ОЗУ",
        "cpuLoad": "ЦП (навантаження)",
        "free": "безкоштовно",
        "cores": "ядер",
        "clicks": "Кліки",
        "conversions": "Конверсії",
        "dbSize": "Розмір БД",
        "version": "Версія",
        "systemComponents": "Компоненти системи",
        "component": "компонент",
        "versionDetails": "Версія / Подробиці",
        "environment": "Навколишнє середовище",
        "phpVersion": "Версія PHP",
        "phpSapi": "PHP SAPI",
        "memoryLimit": "Обмеження пам'яті",
        "phpMemory": "Пам'ять PHP",
        "webServer": "Веб-сервер",
        "serverSoftware": "Серверне програмне забезпечення",
        "cpuCores": "Ядра ЦП",
        "extensions": "Розширення",
        "recommendations": "Рекомендації",
        "sectionInDevelopment": "Розділ у розробці",
        "comingSoonText": "Ця функція стане доступною незабаром.",
        "importTitle": "Імпорт переходів",
        "importDesc": "Використовуйте імпорт конверсій, щоб додати відсутні конверсії для оновлення стану.",
        "formatList": "Формат списку:",
        "subid": "Субідент кліку або конверсії.",
        "payoutOrRevenue": "Сума виплати. Наприклад, 0,12.",
        "tidOptional": "Ідентифікатор транзакції (необов’язково).",
        "statusField": "Статус конверсії (потенційний клієнт, продаж, відхилено, кошик).",
        "exampleWithoutTid": "Приклад (без тайму):",
        "exampleWithTid": "Приклад (з tid):",
        "warningTitle": "Попередження!",
        "warningText": "Неможливо видалити конверсію, натомість перемістіть її до відхилених:",
        "csvData": "Дані CSV",
        "importButton": "Імпорт",
        "importing": "Імпорт...",
        "cleanupTitle": "Очистити статистику",
        "cleanupWarning": "Очищення статистики незворотне. Рекомендується зробити резервну копію.",
        "deleteData": "Видалити дані",
        "olderThan30": "Старше 30 днів",
        "olderThan60": "Старше 60 днів",
        "olderThan90": "Старше 90 днів",
        "allStatistics": "Вся статистика",
        "clearClicks": "Чіткі кліки",
        "clearConversions": "Очистити конверсії",
        "clearAll": "Очистити все",
        "detectingSystem": "Виявлення системи...",
        "errorGettingStatus": "Помилка отримання статусу системи",
        "enterImportData": "Введіть дані для імпорту",
        "errorsInRows": "Помилки в рядках:",
        "networkError": "Помилка мережі",
        "networkErrorSending": "Помилка мережі під час надсилання:"
    },
    "aggregator": {
        "title": "Агрегатор доходів",
        "connections": "Зв'язки",
        "revenue": "Дохід",
        "syncLogs": "Журнали синхронізації",
        "connectionsDesc": "Підключіть кабінети партнерської програми, щоб автоматично збирати дані про доходи.",
        "addConnection": "Додати підключення",
        "addFirst": "Додайте перше підключення",
        "noConnections": "Підключення ще не налаштовано",
        "connectionName": "Назва підключення",
        "engine": "Двигун",
        "network": "Партнерська мережа",
        "dealType": "Тип угоди",
        "lastSync": "Остання синхронізація",
        "status": "Статус",
        "actions": "Дії",
        "never": "Ніколи",
        "syncNow": "Синхронізувати зараз",
        "edit": "Редагувати",
        "delete": "Видалити",
        "editConnection": "Редагувати підключення",
        "selectNetwork": "-- Виберіть мережу --",
        "syncInterval": "Інтервал синхронізації",
        "clickIdParam": "Натисніть Параметр ідентифікатора",
        "clickIdParamHint": "Ім'я параметра в партнерській програмі, що містить Click_id Orbitra (зазвичай sub_id або subid)",
        "credentials": "Облікові дані API",
        "testConnection": "Тестове підключення",
        "cancel": "Скасувати",
        "save": "зберегти",
        "create": "Створити",
        "confirmDelete": "Видалити це підключення? Усі імпортовані записи доходів для нього також буде видалено.",
        "syncSuccess": "Синхронізацію завершено",
        "syncError": "Помилка синхронізації",
        "fetched": "отримано",
        "matched": "збігаються",
        "new": "новий",
        "pending": "В очікуванні",
        "totalRecords": "Загальні записи",
        "totalRevenue": "Загальний дохід",
        "matchedClicks": "Відповідні кліки",
        "matchRate": "Коефіцієнт відповідності",
        "date": "Дата",
        "connection": "Підключення",
        "eventType": "Тип події",
        "amount": "Сума",
        "country": "Країна",
        "noRecords": "Немає записів про доходи за цей період",
        "of": "з",
        "duration": "Тривалість",
        "error": "Помилка",
        "noLogs": "Журналів синхронізації ще немає",
        "manualTitle": "Як користуватися агрегатором доходів",
        "manualStep1": "1. Додайте підключення",
        "manualStep1Desc": "Перейдіть на вкладку «Підключення» та натисніть «Додати підключення». Виберіть свою партнерську мережу та відповідну систему (наприклад, ReferOn, Affilka). Заповніть свої облікові дані API.",
        "manualStep2": "2. Налаштуйте синхронізацію",
        "manualStep2Desc": "Установіть тип угоди (CPA, RevShare), базову лінію та інтервал синхронізації. Orbitra буде автоматично отримувати дані на основі цього розкладу.",
        "manualStep3": "3. Перевірте дохід",
        "manualStep3Desc": "Натисніть «Синхронізувати зараз», щоб отримати дані негайно. Записи з’являться на вкладці «Дохід». Система порівнює реальний дохід із кліками за допомогою ідентифікатора кліку.",
        "manualStep4": "4. Перегляд аналітики",
        "manualStep4Desc": "Перейдіть до звітів про кампанію або тенденцій. Тепер ви можете ввімкнути такі показники, як «Реальний дохід» і «Реальний ROI», щоб порівняти дохід від трекера з фактичними показниками агрегатора.",
        "refresh": "Оновити"
    },
    "trends": {
        "valuePlaceholder": "Введіть значення...",
        "period": "Крапка",
        "grouping": "Групувати за:",
        "months": "Місяці",
        "daysOfWeek": "Дні тижня",
        "days": "днів",
        "hours": "години",
        "addFilter": "Додати фільтр",
        "exportCsv": "Експорт CSV",
        "metrics": "Метрики:",
        "clicks": "Кліки",
        "uniqueClicks": "Унікальні кліки",
        "conversions": "Конверсії",
        "revenue": "Дохід",
        "realRevenue": "Реальний дохід",
        "cost": "Вартість",
        "profit": "Прибуток",
        "realRoi": "Реальний ROI",
        "ctr": "CTR",
        "cr": "CR",
        "chartTitle": "Графік тенденцій",
        "loading": "Завантаження...",
        "noData": "Немає даних для відображення",
        "reportTitle": "звіт",
        "records": "записи",
        "noDataTitle": "Немає даних",
        "noDataPeriod": "Немає даних за вибраний період",
        "addFilterTitle": "Додати фільтр",
        "field": "Поле",
        "condition": "Стан",
        "value": "Значення",
        "cancel": "Скасувати",
        "add": "додати",
        "country": "Країна",
        "device": "пристрій",
        "campaignId": "ID кампанії",
        "offerId": "ID пропозиції",
        "ipAddress": "IP-адреса",
        "browser": "Браузер",
        "os": "ОС",
        "contains": "Містить",
        "notContains": "Не містить",
        "equals": "Дорівнює",
        "notEquals": "Не дорівнює",
        "startsWith": "Починається з",
        "endsWith": "Закінчується на",
        "regexp": "Регулярний вираз",
        "regexpExclude": "Регулярний вираз виключає"
    },
    "conversions": {
        "title": "Журнал переходів",
        "search": "Пошук",
        "searchPlaceholder": "Пошук за subid, tid, ip...",
        "status": "Статус",
        "allStatuses": "Всі статуси",
        "lead": "Свинець",
        "sale": "Розпродаж",
        "rejected": "Відхилено",
        "registration": "Реєстрація",
        "deposit": "депозит",
        "trash": "сміття",
        "dateFrom": "Дата з",
        "dateTo": "Дата до",
        "payout": "Виплата",
        "campaign": "Кампанія",
        "offer": "Пропозиція",
        "date": "Дата",
        "noConversions": "Жодних конверсій",
        "noConversionsText": "Конверсії з’являться після отримання зворотного зв’язку",
        "shown": "Показано",
        "of": "з",
        "logInfo": "Конверсії фіксуються за датою отримання постбеку в трекері. Для запису повторних конверсій використовуйте параметр tid (ідентифікатор транзакції)."
    },
    "postback": {
        "title": "URL-адреса повернення",
        "subtitle": "Додайте цю URL-адресу до налаштувань зворотного зв’язку вашої партнерської мережі",
        "examplesTitle": "Приклади використання",
        "basicPostback": "Базовий постбек:",
        "withPayout": "З виплатою:",
        "withTid": "З TID для повторних перетворень:",
        "statusMapping": "Відображення стану:",
        "settingsTitle": "Налаштування",
        "postbackKey": "Ключ повернення",
        "postbackKeyHint": "Унікальний ключ для захисту від спуфінгу",
        "postbackDefaultKeyWarning": "Ключ постбеків дорівнює публічному дефолту з репозиторію — на трекер можна надіслати підроблені конверсії. Згенеруйте новий ключ (CLI: php cli/generate_postback_key.php).",
        "defaultCurrency": "Валюта за умовчанням",
        "aliasesTitle": "Псевдоніми параметрів",
        "aliasesDesc": "Налаштуйте альтернативні назви параметрів для отримання зворотних повідомлень від різних партнерських мереж",
        "testTitle": "Тест зворотного зв'язку",
        "clickSubid": "Натисніть Subid",
        "clickIdPlaceholder": "Натисніть ID",
        "paramsTitle": "Параметри зворотного зв'язку",
        "param": "Параметр",
        "required": "Обов'язковий",
        "description": "опис",
        "subidDesc": "Ідентифікатор кліку трекера",
        "statusDesc": "Статус конверсії (потенційний клієнт, продаж, відхилено, ...)",
        "payoutDesc": "Сума виплати",
        "tidDesc": "Ідентифікатор транзакції для повторних конверсій",
        "currencyDesc": "Валюта (дол. США, євро, руб.)",
        "subIdDesc": "Додаткові параметри",
        "saleStatusDesc": "Мапінг: статуси на продаж",
        "leadStatusDesc": "Відображення: статуси для потенційних клієнтів",
        "copied": "Скопійовано в буфер обміну",
        "copy": "Копія",
        "test": "Тест",
        "saveSettings": "Зберегти налаштування"
    },
    "simulation": {
        "title": "Симуляція дорожнього руху",
        "campaign": "Кампанія",
        "ipAddress": "IP адреса",
        "userAgent": "Користувач-Агент",
        "geoCode": "Geo (код країни)",
        "deviceType": "Тип пристрою",
        "language": "Мови браузера (Accept-Language)",
        "webdriverFlag": "Ознака автоматизації (navigator.webdriver)",
        "desktop": "Робочий стіл",
        "mobile": "мобільний",
        "tablet": "планшет",
        "runTest": "Запустити тест",
        "simulating": "Імітація...",
        "resultTitle": "Результат (Trace потоку)",
        "runToSee": "Запустіть тест, щоб переглянути результати маршрутизації."
    },
    "clickDetails": {
        "title": "Натисніть Деталі",
        "loading": "Завантаження інформації про кліки...",
        "error": "Помилка",
        "notFound": "Не вдалося знайти дані",
        "close": "Закрити",
        "noData": "Немає даних",
        "yes": "так",
        "no": "немає",
        "sections": {
            "data": "дані",
            "id": "ID",
            "connection": "Підключення",
            "finance": "Фінанси",
            "parameters": "Параметри",
            "geoDevice": "GEO та пристрій",
            "calendar": "Календар",
            "cloak": "Клоакінг"
        },
        "fields": {
            "campaign": "Кампанія",
            "campaignAlias": "Псевдонім кампанії",
            "landing": "Посадка",
            "offer": "Пропозиція",
            "affiliateNetwork": "Партнерська мережа",
            "source": "Джерело",
            "streamType": "Тип потоку",
            "referer": "Реферер",
            "clickId": "Натисніть ID",
            "campaignId": "ID кампанії",
            "offerId": "ID пропозиції",
            "landingId": "ID посадки",
            "streamId": "ID потоку",
            "sourceId": "Ідентифікатор джерела",
            "ip": "IP",
            "isp": "ISP",
            "asn": "ASN",
            "proxyType": "Тип проксі",
            "botScanner": "Бот / Сканер",
            "cost": "Вартість",
            "revenue": "Дохід",
            "profit": "Прибуток",
            "keyword": "Ключове слово",
            "externalId": "Зовнішній ID",
            "creativeId": "Creative ID",
            "adCampaignId": "Ідентифікатор рекламної кампанії",
            "sourceParam": "Параметр джерела",
            "country": "Країна (код)",
            "region": "Регіон",
            "city": "місто",
            "zipcode": "Поштовий індекс",
            "timezone": "Часовий пояс",
            "latitude": "Широта",
            "longitude": "Довгота",
            "language": "Мова веб-переглядача (основна)",
            "acceptLanguageRaw": "Accept-Language (Raw)",
            "deviceType": "Тип пристрою",
            "os": "ОС",
            "browser": "Браузер",
            "userAgent": "Агент користувача",
            "dateTime": "Дата і час",
            "landingAt": "Лендінг показано",
            "offerAt": "Перехід на офер",
            "timeToOffer": "Час до оффера",
            "timeOnLp": "Час на лендінгу",
            "lpScroll": "Глибина скролу",
            "conversion": "Перетворення",
            "route": "Маршрут",
            "cloakVerdict": "Вердикт клоакінгу",
            "cloakReasons": "Причини клоакінгу"
        }
    },
    "archive": {
        "title": "Архів ресурсу",
        "loading": "Завантаження архіву...",
        "deleteAll": "Видалити весь архів",
        "confirmAction": "Ви впевнені? Цю дію не можна скасувати.",
        "networkError": "Помилка мережі",
        "actionError": "Помилка виконання дії",
        "sectionManagement": "Керівництво розділом:",
        "restoreAll": "Відновити все",
        "clearSection": "Очистити розділ",
        "id": "ID",
        "name": "Ім'я",
        "deletedAt": "Дата видалення",
        "actions": "Дії",
        "restore": "Відновити",
        "deleteForever": "Видалити назавжди",
        "loadingData": "Завантаження даних...",
        "emptySection": "Цей розділ архіву порожній",
        "tabs": {
            "campaigns": "Кампанії",
            "offers": "Пропозиції",
            "landings": "Посадки",
            "sources": "Джерела",
            "networks": "мережі"
        }
    },
    "systemSettings": {
        "title": "Налаштування системи",
        "loading": "Завантаження...",
        "saveSuccess": "Налаштування успішно збережено",
        "saveError": "Помилка",
        "networkError": "Помилка мережі",
        "statsCollection": "Збір статистики",
        "statsCollectionDesc": "Вмикає та вимикає запис кліків і переходів у базу даних. Ведення журналу залишається активним.",
        "ignorePrefetch": "Ігнорувати запити на попередню вибірку",
        "ignorePrefetchDesc": "Запобігає помилковим клацанням, створеним попереднім завантаженням браузера (Moz/Preview).",
        "updateNotify": "Сповіщати про нові версії",
        "updateNotifyDesc": "Під час завантаження панелі перевіряє GitHub на наявність новішої версії. Вимкнено — запит не надсилається взагалі: перевірка йде мережею до GitHub і на хостингах без доступу до нього коштує очікування.",
        "logRetention": "Термін зберігання журналу в днях",
        "logRetentionHint": "Дані, старші за вказаний період, будуть очищені.",
        "archiveRetention": "Термін зберігання архівного ресурсу (в днях)",
        "archiveRetentionHint": "Ресурси, видалені більше ніж указану кількість днів тому, будуть остаточно видалені.",
        "adminAccess": "Доступ до панелі адміністратора (IP/підмережі)",
        "adminAccessPlaceholder": "Залиште порожнім для доступу з будь-якого місця",
        "adminAccessHint": "Один IP або підмережа на рядок. Попередження: ви можете заблокувати себе!",
        "adminPath": "Шлях до адмін-панелі",
        "adminPathPlaceholder": "admin.php (за замовчуванням)",
        "adminPathHint": "Переносить панель з /admin.php на /ваш-шлях, щоб форма входу не була за адресою, яку перебирає кожен сканер. 3-64 символи: малі латинські літери, цифри, \"-\" та \"_\". Порожнє поле залишає /admin.php. Це приховує панель, але не замінює пароль.",
        "adminPathWarning": "Збережіть нову адресу в закладки, перш ніж піти зі сторінки. Якщо забули — виконайте на сервері:",
        "adminPathMoved": "Збережено. Панель переїхала на",
        "adminPathInvalid": "3-64 символи: малі латинські літери, цифри, \"-\" та \"_\", перший символ — літера або цифра.",
        "adminPathReserved": "Це ім'я зайняте самим трекером. Оберіть інше.",
        "adminPathAliasTaken": "Такий алiас уже має кампанія — шлях перекрив би її."
    },
    "conversionTypes": {
        "title": "Спеціальні типи перетворення",
        "loading": "Завантаження...",
        "addType": "Додайте тип",
        "name": "Ім'я",
        "statusMacros": "Статуси (макроси)",
        "conversion": "Перетворення",
        "conversionTitle": "Зарахувати як конверсію",
        "profit": "Прибуток",
        "profitTitle": "Розраховувати як прибуток",
        "s2s": "S2S",
        "s2sTitle": "Надішліть S2S далі",
        "cap": "Кепка",
        "capTitle": "Впливає на пропозицію Cap",
        "noTypes": "Типи перетворення не додано",
        "editType": "Тип редагування",
        "newType": "Новий тип перетворення",
        "cancel": "Скасувати",
        "metricName": "Назва метрики в інтерфейсі",
        "metricNamePlaceholder": "Наприклад: Upsell",
        "expectedMacros": "Очікувані макроси стану (розділені комами)",
        "expectedMacrosPlaceholder": "Наприклад: upsell, upsell2",
        "expectedMacrosHint": "Оригінальні статуси з партнерської мережі.",
        "labelColor": "Колір мітки",
        "customColor": "Свій колір (піпетка)",
        "colorNotSet": "Колір не задано — використовується стандартний",
        "affectConversions": "Впливає на показник \"Конверсії\".",
        "affectConversionsDesc": "Збільшує лічильник конверсій",
        "affectProfit": "Впливає на показник \"Прибуток\".",
        "affectProfitDesc": "Додає до доходу",
        "sendS2s": "Надіслати зворотне повідомлення S2S джерелу трафіку",
        "sendS2sDesc": "За умови, що URL-адресу налаштовано для цього статусу для даного джерела",
        "affectCap": "Враховується в обмеженнях пропозиції (щоденне обмеження)",
        "saveType": "Тип збереження",
        "nameAndMacrosRequired": "Ім’я та макроси є обов’язковими",
        "deleteConfirm": "Видалити тип конверсії?",
        "deleteError": "Видалити помилку",
        "unmappedStatuses": "Непридатані статуси",
        "noUnmappedStatuses": "Непридатних статусів не знайдено",
        "originalStatus": "Початковий статус",
        "count": "Кількість",
        "firstSeen": "Перше появлення",
        "lastSeen": "Останнє появлення",
        "mapTo": "Мапити в",
        "mapToTitle": "Мапити в {type}",
        "mapToBtn": "{type}",
        "mappedSuccessfully": "Успішно замаплено та перекласифіковано {count} конверсій",
        "alreadyMapped": "Цей статус вже замаплений в цей тип"
    },
    "users": {
        "usersAndProfile": "Користувачі та профіль",
        "apiKeys": "API-ключі",
        "roleHint": "Ролей всього дві. Адмін має повний доступ; акаунт, створений під час встановлення (id 1), видалити не можна — як і останнього адміна. Юзер бачить лише розділи, видані в «Правах» (іконка щита) — шаблон вище просто підставляє типовий набір.",
        "roleTemplate": "Шаблон ролі",
        "templateAdmin": "Адмін (повний доступ)",
        "templateMediaBuyer": "Медіабаєр",
        "templateVideoEditor": "Відеомонтажер / Дизайнер",
        "templateDeveloper": "Розробник",
        "templateCustom": "Свої права",
        "financeData": "Фінансові дані",
        "financeCosts": "Показувати витрати (Cost, CPC, CPA)",
        "financeRevenue": "Показувати дохід (Revenue, Profit, ROI)",
        "financePayout": "Показувати виплати (пейменти офферів)",
        "resourceLogs": "Логи",
        "campaignItems": "Призначені кампанії (видні з доступом Own + Selected / Selected)",
        "mcpTitle": "Підключити ІІ-асистента (MCP)",
        "mcpDesc": "Згенеруйте ключ вище, потім вставте це в конфіг Claude Desktop. Замініть шлях на абсолютний шлях до mcp/src/index.js, а ORBITRA_API_KEY — на ваш ключ. Повна інструкція: mcp/README.md.",
        "mcpCopyConfig": "Скопіювати конфіг",
        "apiKeyHint": "Ключі «read» дають лише аналітику. Ключі «write» також дозволяють керувати кампаніями, оферами й доменами — їх використовує MCP-сервер Orbitra для нейромереж (див. mcp/README.md).",
        "copyMcpUrl": "Скопіювати URL конектора (для Claude → Add custom connector)",
        "urlCopied": "URL коннектора MCP скопійовано в буфер обміну!",
        "keyCopied": "API-ключ скопійовано в буфер обміну!",
        "newReadKey": "Ключ читання",
        "newWriteKey": "Ключ запису",
        "title": "Користувачі",
        "loading": "Завантаження...",
        "addUser": "Додати користувача",
        "username": "Ім'я користувача",
        "role": "Роль",
        "createdAt": "Створено",
        "actions": "Дії",
        "noUsers": "Немає користувачів",
        "edit": "Редагувати",
        "delete": "Видалити",
        "permissions": "Дозволи",
        "copy": "Копія",
        "usernamePlaceholder": "Введіть ім'я користувача",
        "passwordPlaceholder": "Залиште пустим, щоб залишити без змін",
        "passwordPlaceholderNew": "Введіть пароль",
        "deleteConfirm": "Видалити користувача?",
        "language": "Мова"
    },
    "landingEditor": {
        "fileNew": "Новий файл",
        "fileUpload": "Завантажити файл",
        "fileRename": "Перейменувати або перенести",
        "fileNewPrompt": "Назва файлу, наприклад page2.html або css/style.css",
        "fileRenamePrompt": "Нова назва або шлях",
        "fileDeleteConfirm": "Видалити цей файл?",
        "fileUploadDirPrompt": "Тека для завантаження (порожньо — у корінь)",
        "fileOpError": "Не вдалося виконати",
        "adapterTitle": "JS-адаптер (за бажанням)",
        "adapterCopy": "Копіювати",
        "adapterCopied": "Скопійовано",
        "adapterHintLocal": "Переносить клік на внутрішні сторінки та форми лендінга, щоб багатосторінковий лендінг далі знав відвідувача. Вставте в <head> на кожній сторінці.",
        "adapterHintRedirect": "Обовʼязковий для лендінга на чужому хостингу: без нього сторінка не знає, якому кліку належить, і посилання на офер не спрацює. Вставте в <head> на кожній сторінці.",
        "adapterPostbackHint": "Він же дозволяє сторінці самій надіслати конверсію, без партнерки посередині:",
        "actionTypeLabel": "Дія",
        "actionToCampaign": "Надіслати в кампанію",
        "actionNotFound": "Показати 404 помилку",
        "actionShowText": "Показати як текст",
        "actionShowHtml": "Показати як HTML",
        "actionDoNothing": "Нічого не робити",
        "actionToCampaignHint": "Передає відвідувача в іншу кампанію. Клік враховується в обох; на відміну від Keitaro це коштує одного зайвого редиректу.",
        "actionNotFoundHint": "Віддає порожню сторінку зі статусом 404.",
        "actionShowTextHint": "Віддає текст нижче як звичайний текст.",
        "actionShowHtmlHint": "Віддає HTML нижче як сторінку.",
        "actionDoNothingHint": "Відповідає без вмісту — відвідувач залишається там, де він є.",
        "actionTargetCampaign": "Цільова кампанія",
        "actionPickCampaign": "Оберіть кампанію…",
        "actionTextLabel": "Текст",
        "actionHtmlLabel": "HTML",
        "offerLinkTitle": "Посилання на офер",
        "offerLinkHint": "Кнопку переходу оформіть макросом {offer} — трекер підставить замість нього URL офера, прив’язаного до цього потоку, разом із click id. Решта розмітки не змінюється.",
        "offerLinkExtra": "Якщо оферів на сторінці кілька, вибір має йти через трекер — {offer} завжди розгортається в той офер, який обрав потік:",
        "offerLinkExampleSingle": "<a href=\"{offer}\">Купити</a>",
        "offerLinkExampleMulti": "<a href=\"/?_lp=1&offer_id=10\">Офер 10</a>\n<a href=\"/?_lp=1&offer_id=22\">Офер 22</a>",
        "postbackExample": "<a href=\"{offer}\" onclick=\"orbitraPostback(this, event, 'lead', 10)\">Купити</a>",
        "loadError": "Помилка завантаження даних посадки",
        "savedFiles": "Посадка збережена. Тепер ви можете завантажувати файли.",
        "savedSuccess": "Дані успішно збережено",
        "saveError": "Помилка збереження",
        "networkError": "Помилка мережі",
        "serverError": "Сервер повернув помилку. Подробиці — у var/logs/php_errors.log",
        "slugInvalid": "Недопустима папка: лише латиниця, цифри, дефіс і підкреслення, до 64 символів",
        "slugReserved": "Цю папку зарезервовано системою, оберіть іншу",
        "slugTaken": "Таку папку вже зайнято іншим лендингом",
        "slugCheckFailed": "Не вдалося перевірити папку — спробуйте ще раз",
        "uploadExceedsPostMax": "Архів більший, ніж дозволяє PHP: збільште post_max_size та upload_max_filesize у php.ini",
        "uploadErrIniSize": "Файл більший за upload_max_filesize у php.ini",
        "uploadErrFormSize": "Файл перевищує ліміт, заданий формою",
        "uploadErrPartial": "Файл передано не повністю — спробуйте ще раз",
        "uploadErrNoFile": "Файл не було надіслано",
        "uploadErrNoTmpDir": "На сервері немає тимчасового каталогу для завантажень (upload_tmp_dir)",
        "uploadErrCantWrite": "Сервер не зміг записати файл на диск — перевірте права",
        "uploadErrExtension": "Завантаження зупинило розширення PHP",
        "uploadErrUnknown": "Завантаження не вдалося",
        "missingExtFileinfo": "На сервері немає розширення PHP fileinfo. Встановіть: apt install php-fileinfo",
        "missingExtZip": "На сервері немає розширення PHP zip. Встановіть: apt install php-zip",
        "notAZip": "Це не ZIP-архів — сервер визначив інший тип файлу",
        "dirNotCreated": "Не вдалося створити каталог лендингу: вебсервер не може писати в landings/",
        "dirNotWritable": "Каталог лендингу закритий для запису вебсерверу",
        "zipUnsupportedCompression": "Архів стиснуто методом, який PHP не розпаковує. Перезберіть ZIP звичайним Deflate",
        "zipExtractFailed": "Архів відкрився, але розпакувати не вдалося — перевірте права та місце на диску",
        "zipOpenFailed": "Не вдалося відкрити ZIP-архів: файл пошкоджено або це не ZIP",
        "uploadFailed": "Не вдалося завантажити архів",
        "phpScanFailed": "PHP в архіві викликає заборонені функції (запуск команд, eval, симлінки) — приберіть їх або завантажте архів без них",
        "stageDirNotCreated": "Не вдалося створити тимчасову папку для розпакування — перевірте права",
        "destDirNotCleared": "Не вдалося очистити попередні файли перед встановленням нового архіву",
        "destDirSwapFailed": "Не вдалося перенести розпаковані файли на місце — перевірте права",
        "archiveSanitized": "Ці PHP-виклики видалено автоматично — вони можуть зняти обмеження часу виконання трекера:",
        "archiveUploaded": "Архів успішно завантажено та розпаковано.",
        "archiveError": "Помилка завантаження ZIP",
        "fileUploadError": "Не вдалося завантажити файл.",
        "fileReadError": "Помилка читання файлу",
        "fileSaveError": "Не вдалося зберегти файл",
        "fileSaveError2": "Помилка під час збереження",
        "title": "Landing Editor",
        "name": "Ім'я",
        "namePlaceholder": "Назва посадки",
        "group": "Група",
        "groupOrProduct": "Група / Продукт",
        "noGroup": "-- Немає групи --",
        "status": "Статус",
        "active": "Активний",
        "paused": "Призупинено",
        "archived": "Архівовано",
        "landingType": "Тип посадки",
        "typeLocal": "Локальний",
        "typeRedirect": "Переспрямування",
        "typePreload": "Передзавантаження",
        "typeAction": "Дія",
        "slugLabel": "Тека",
        "slugHint": "Тека, куди розпакуються файли посадки. Залиште порожнім для авто-генерації з назви.",
        "slugPlaceholder": "моя-посадка",
        "redirectMethodLabel": "Метод переспрямування",
        "redirectHttp": "HTTP 302 переспрямування",
        "redirectJs": "JavaScript переспрямування",
        "redirectMeta": "Meta refresh",
        "copyCode": "Копіювати",
        "codeCopied": "Скопійовано",
        "offerLinkWord": "Купити",
        "localZip": "Місцевий (архів ZIP)",
        "redirect": "Переспрямування (URL)",
        "preload": "Попереднє завантаження (попереднє завантаження URL-адреси)",
        "action": "Дія (корисне навантаження дії)",
        "urlLabel": "URL-адреса *",
        "urlPlaceholder": "https://...",
        "preloadHint": "Для успішного попереднього завантаження цільовий сайт має дозволяти iframe і перехресні запити.",
        "actionPayloadLabel": "Корисне навантаження дії (код JS / HTML) *",
        "actionPayloadPlaceholder": "window.location.replace('{пропозиція}');",
        "saveFirst": "Спочатку збережіть налаштування посадки для завантаження архівних файлів.",
        "uploadZip": "Завантажити ZIP",
        "uploadingZip": "Завантаження архіву…",
        "zipDropHint": "Перетягніть .zip сюди або натисніть для вибору файлу",
        "zipReplaceHint": "Натисніть або перетягніть інший файл для заміни",
        "viewCode": "Код",
        "viewSplit": "Спліт",
        "viewPreview": "Перегляд",
        "livePreview": "Живий попередній перегляд",
        "unsavedPreview": "Незбережений HTML",
        "savedPreview": "Збережений лендінг",
        "filesTitle": "Файли",
        "replaceImage": "Замінити зображення",
        "replaceImageTypeError": "Виберіть зображення того самого типу, щоб наявні посилання лендінгу продовжили працювати.",
        "imageLoadError": "Не вдалося завантажити зображення",
        "formatCode": "Форматувати",
        "quickInsertHint": "Сніпети вставляються в позицію курсора; адаптер і Back Trap — перед закривальними тегами.",
        "findReplace": "Пошук і заміна",
        "findPlaceholder": "Знайти текст або regex…",
        "replacePlaceholder": "Замінити на (напр. {offer})…",
        "matchCase": "Враховувати регістр",
        "wholeWord": "Слово цілком",
        "useRegex": "Регулярні вирази",
        "noMatches": "Немає збігів",
        "invalidRegex": "Некоректний regex",
        "matchCount": "{current} із {total}",
        "replaceBtn": "Замінити",
        "replaceAllBtn": "Замінити все",
        "replacedCount": "Замінено: {count}",
        "findNext": "Наступний збіг (Enter)",
        "findPrev": "Попередній збіг (Shift+Enter)",
        "closeFind": "Закрити (Esc)",
        "toggleReplace": "Показати/приховати заміну",
        "fullscreen": "На весь екран",
        "exitFullscreen": "Вийти з повноекранного режиму",
        "openInTab": "Відкрити в новій вкладці",
        "previewNeedsSlug": "Перегляд стане доступним, коли в лендингу з'явиться папка — спочатку збережіть налаштування.",
        "zipOnCreateHint": "Необов'язково. Архів завантажиться одразу після створення лендингу — вікно залишиться відкритим, файли з'являться праворуч.",
        "selectFile": "Виберіть файл для редагування",
        "cancel": "Скасувати",
        "save": "зберегти",
        "createLanding": "Створити посадку",
        "landing": "Посадка",
        "saveChanges": "Зберегти зміни"
    },
    "picker": {
        "offersTitle": "Офери",
        "landingsTitle": "Посадки",
        "safeLandingsTitle": "Вибір білого лендінга",
        "safeOffersTitle": "Вибір локального оффера",
        "search": "Пошук",
        "allGroups": "Усі групи",
        "allNetworks": "Усі мережі",
        "allCountries": "Усі країни",
        "alreadyAdded": "Вже додано",
        "noResults": "Нічого не знайдено",
        "group": "Група",
        "countries": "Країни",
        "network": "Партнерська мережа",
    },
    "landingColumns": {
        "title": "Вибір і порядок колонок",
        "required": "обов'язкова",
        "conversions": "Конверсії",
        "cr": "CR",
        "lastEvent": "Остання подія",
        "never": "Ніколи",
    },
    "columnsOrder": {
        "title": "Вибір і порядок колонок",
        "required": "обов'язкова",
    },
    "offerColumns": {
        "geo": "GEO",
        "payout": "Виплата",
        "allGeo": "Усі GEO",
        "payoutAuto": "Авто (за постбеком)",
        "leads": "Ліди",
        "sales": "Сейли",
        "rejected": "Відхилені",
        "revenueConfirmed": "Дохід (підтв.)",
        "cost": "Витрати",
        "cr": "CR",
        "epcConfirmed": "EPC (підтв.)",
        "cpc": "CPC",
        "profitConfirmed": "P/L (підтв.)",
        "roiConfirmed": "ROI (підтв.)",
    },
    "offerEditor": {
        "title": "Редактор оферу",
        "savedOffer": "Збережений офер",
        "desktop": "Десктоп",
        "mobile": "Мобільний (375px)",
        "urlTemplate": "URL офера",
        "localArchive": "Файли локального оффера",
        "uploadZip": "Завантажити ZIP",
        "zipError": "Помилка завантаження ZIP",
        "zipOnCreateHint": "Архів завантажиться одразу після створення оффера (завантаженню потрібен id оффера). index.html стане сторінкою оффера.",
        "filesLabel": "файлів",
        "noFiles": "Файлів поки немає",
        "fillName": "Введіть назву пропозиції",
        "fillUrl": "Укажіть URL-адресу пропозиції або виберіть тип «Місцева пропозиція».",
        "savedSuccess": "Пропозицію успішно збережено.",
        "saveError": "Помилка:",
        "networkError": "Помилка мережі",
        "maxValues": "Максимум 10 значень",
        "namePlaceholder": "Назва пропозиції",
        "paramNamePlaceholder": "Назва (наприклад, ціна)",
        "paramValuePlaceholder": "Вартість (наприклад, $99,00)",
        "notesPlaceholder": "Ваші примітки щодо пропозиції...",
        "nameLabel": "Ім'я",
        "group": "Група",
        "groupOrProduct": "Група / Продукт",
        "noGroup": "-- Немає групи --",
        "affiliateNetwork": "Партнерська мережа",
        "networkTooltip": "Вибрана мережа додає теги до URL-адреси та вмикає показники у звітах.",
        "noNetwork": "-- Немає мережі --",
        "redirectType": "Тип перенаправлення",
        "httpRedirect": "Перенаправлення HTTP (302)",
        "preloadCurl": "Попереднє навантаження (завиток)",
        "countries": "Країни",
        "countriesPlaceholder": "Введіть або виберіть країни...",
        "payouts": "Виплати",
        "payoutByParam": "За параметром виплати",
        "rebills": "Перерахунки",
        "conversionCap": "Обмеження конверсії",
        "dailyLimit": "Денний ліміт",
        "timezone": "Часовий пояс",
        "altOffer": "Альтернативна пропозиція",
        "notSelected": "-- Не вибрано --",
        "valuesTitle": "Цінності",
        "valuesDesc": "Створення макросів для переходу на цільову сторінку (до 10 значень).",
        "add": "додати",
        "noValues": "Жодних цінностей не створено",
        "usageExamples": "Приклади використання на цільовій сторінці:",
        "empty": "(порожній)"
    },
    "sourceEditor": {
        "namePlaceholder": "Наприклад: Facebook Ads - Main",
        "noParams": "Без параметрів. Натисніть «Додати» або виберіть шаблон.",
        "notesPlaceholder": "Додаткова інформація...",
        "templateLabel": "Вихідний шаблон",
        "selectTemplate": "-- Виберіть шаблон --",
        "templateHint": "Виберіть шаблон для автоматичного заповнення параметрів",
        "noTemplateOption": "Спеціальне джерело (без шаблону)",
        "noTemplateHint": "Створіть власне джерело — просто введіть назву та URL нижче. Шаблон необов’язковий.",
        "checkUrlLabel": "URL (для перевірки наявності)",
        "available": "в наявності",
        "checkedAt": "Перевірено",
        "nameLabel": "Ім'я *",
        "status": "Статус",
        "active": "Активний",
        "paused": "Призупинено",
        "postbackDesc": "URL-адреса для надсилання конверсій назад до джерела трафіку",
        "sendStatuses": "Надіслати статуси:",
        "availableMacros": "Доступні макроси:",
        "parameters": "Параметри",
        "addParam": "додати",
        "paramsDesc": "Налаштуйте параметри, які автоматично додаватимуться до URL-адреси кампанії, коли вибрано це джерело",
        "alias": "Псевдонім",
        "param": "Параметр",
        "sourceMacro": "Вихідний макрос",
        "notes": "Примітки",
        "urlLabel": "URL (для перевірки)",
        "urlPlaceholder": "https://example.com"
    },
    "networkEditor": {
        "namePlaceholder": "Наприклад: M1 Shop",
        "notesPlaceholder": "Будь-які примітки щодо налаштування...",
        "basicSettings": "Основні налаштування",
        "nameLabel": "Ім'я *",
        "template": "Шаблон",
        "status": "Статус",
        "active": "Активний",
        "disabled": "Вимкнено",
        "postbackHint": "Додайте цю URL-адресу на інформаційну панель партнерської мережі (Global Postback)",
        "postbackMacros": "Макроси постбеку Orbitra:",
        "offerParams": "Параметри пропозиції",
        "offerParamsDesc": "Параметри, які автоматично додаються, щоб запропонувати URL-адресам цієї мережі передавати subid та інші дані.",
        "offerParamsLabel": "Параметри (додано до URL-адреси пропозиції)",
        "example": "приклад:",
        "availableMacros": "Доступні макроси",
        "macroSubid": "Ідентифікатор кліку (підряд)",
        "macroIp": "IP-адреса",
        "macroCountry": "Країна",
        "macroDevice": "Тип пристрою",
        "macroReferer": "Реферер",
        "macroKeyword": "Ключове слово",
        "macroCost": "Натисніть Вартість",
        "macroExternalId": "Зовнішній ID",
        "macroCreative": "Creative ID",
        "macroAdCampaign": "Ідентифікатор рекламної кампанії",
        "notes": "Примітки"
    },
    "botSettings": {
        "ispTitle": "Глобальний чорний список Bot ISP",
        "ispHint": "Один провайдер на рядок, порівнюється цілісно з ISP та ASN відвідувача (списки через коми надалі працюють). Потоки Cloak із увімкненим фільтром Bot ISP спрямовують таких відвідувачів на білу сторінку.",
        "ispPlaceholder": "Один провайдер на рядок, напр. Amazon.com, Inc.",
        "ispIgnoredWarning": "Проігноровані записи (занадто короткі або загальний корпоративний суфікс — вони збігалися б майже з кожним ISP)",
        "save": "Зберегти",
        "saved": "Збережено!",
        "saving": "Збереження...",
        "saveError": "Помилка збереження",
        "noItems": "Ще немає записів",
        "noSearchResults": "Немає записів, що відповідають вашому пошуку",
        "searchPlaceholder": "Пошук у списку…",
        "showing": "показано",
        "of": "з",
        "ofTotal": "із усіх",
        "loadMore": "Показати ще",
        "title": "Захист від ботів",
        "ipTitle": "IP-адреси ботів",
        "signaturesTitle": "Підписи (Агент користувача)",
        "ipPlaceholder": "Введіть IP-адреси або CIDR (по одній на рядок)",
        "signaturePlaceholder": "Введіть слова для підпису (наприклад, Googlebot, павук) по одному на рядок",
        "fillName": "Введіть ім'я",
        "clearAll": "Очистити все",
        "feedTitle": "Діапазони IP дата-центрів і краулерів",
        "feedMissing": "Ще не завантажені — доки їх немає, шар діапазонів у клоаці не працює",
        "feedFresh": "Актуальні",
        "feedStale": "Старші за добу",
        "feedUpdate": "Оновити зараз",
        "feedUpdating": "Оновлення…",
        "feedUpdateError": "Не вдалося оновити діапазони",
        "clearSourceHint": "Видалити лише записи з цього джерела",
        "addIp": "Додайте IP",
        "addSignature": "Додайте підписи",
        "addIsp": "Додати Bot ISP",
        "noRecords": "Записів немає",
        "loading": "Завантаження...",
        "loadError": "Помилка завантаження баз даних",
        "addedCount": "Записи додано:",
        "skippedDuplicates": "дублікатів пропущено:",
        "networkError": "Помилка мережі",
        "deleteError": "Помилка видалення",
        "clearError": "Очистити помилку",
        "cleared": "Базу даних очищено",
        "confirmClear": "Ви впевнені, що бажаєте повністю очистити цю базу даних?"
    },
    "geoSelector": {
        "placeholder": "Введіть коди країн, розділені пробілами або комами (наприклад, RU US GB)",
        "switchToTextarea": "Перейти до текстового поля",
        "switchToSelect": "Перемикач для вибору",
        "insertFromProfile": "Вставити з геопрофілю",
        "addUnknown": "Додати \"Невідомий\"",
        "nothingFound": "Нічого не знайдено",
        "geoProfiles": "Географічні профілі",
        "noProfiles": "Немає доступних профілів",
        "unknown": "Невідомий"
    },
    "customMetrics": {
        "title": "Спеціальні показники",
        "createMetric": "Створити метрику",
        "name": "Ім'я",
        "formula": "Формула",
        "format": "Формат",
        "noMetrics": "Немає доданих показників",
        "editMetric": "Редагувати показник",
        "newMetric": "Нова метрика",
        "formulaVarsTitle": "Доступні змінні для формули:",
        "totalClicks": "Загальна кількість кліків",
        "uniqueClicks": "Унікальні кліки",
        "conversionsVar": "Перетворення",
        "revenueVar": "Дохід",
        "costVar": "Вартість",
        "operatorsHint": "Оператори: + - * / ( ). Приклад ROI:",
        "metricNameInReports": "Назва метрики у звітах",
        "calculationFormula": "Формула розрахунку",
        "outputFormat": "Вихідний формат",
        "number": "Номер",
        "currency": "Валюта",
        "percent": "Відсоток",
        "decimalPlaces": "Десяткові знаки",
        "nameAndFormulaRequired": "Необхідно вказати назву та формулу",
        "saved": "Збережено",
        "saveError": "Помилка збереження",
        "networkError": "Помилка мережі",
        "deleteConfirm": "Видалити показник?",
        "deleteError": "Видалити помилку",
        "saveMetric": "Зберегти метрику",
        "placeholder": "Наприклад: EPC"
    },
    "groups": {
        "placeholder": "Нова назва групи...",
        "delete": "Видалити"
    },
    "groupsModal": {
        "domainGroups": "Групи доменів",
        "offerGroups": "Групи пропозицій",
        "landingGroups": "Посадкові групи",
        "campaignGroups": "Групи кампаній",
        "groups": "Групи",
        "add": "додати",
        "noGroups": "Групи не створено",
        "createFirst": "Створіть першу групу вище",
        "createError": "Помилка створення",
        "networkError": "Помилка мережі",
        "deleteConfirm": "Видалити групу? Пропозиції в цій групі буде розгруповано.",
        "deleteError": "Помилка видалення",
        "rename": "Перейменувати",
        "renameError": "Помилка перейменування",
        "close": "Закрити",
        "loading": "Завантаження..."
    },
    "geoDb": {
        "noDatabase": "Немає бази",
        "title": "Географічні бази даних",
        "infoText": "Геобази даних використовуються трекером для швидкої ідентифікації країни, регіону, міста та провайдера за IP-адресою відвідувача.",
        "supportedTitle": "📚 Які бази даних Geo підтримуються?",
        "recommendedFree": "✅ Рекомендовано (безкоштовно):",
        "sypexDesc": "країна, регіон, місто. Оновлюється щомісяця.",
        "ip2locLiteDesc": "країна, регіон, місто + широта/довжина, ZIP, часовий пояс (DB11, потрібна реєстрація).",
        "maxmindLiteDesc": "безкоштовні бази City та ASN (потрібні обліковий запис MaxMind і ліцензійний ключ).",
        "paidDbs": "💎 Платні бази даних:",
        "ip2locDb4Desc": "країна, регіон, місто, провайдер",
        "ip2locPx2Desc": "виявлення проксі/VPN",
        "sypexFullDesc": "розширена версія з більш високою точністю",
        "maxmindFullDesc": "висока точність (потрібна ліцензія)",
        "asnTitle": "Визначення мережі, проксі та VPN",
        "asnDesc": "ASN-бази визначають мережу й провайдера. IP2Proxy PX12 додатково розпізнає проксі, VPN, Tor, дата-центри та загрози.",
        "asnDownload": "Завантажити",
        "asnSetup": "Для GeoLite2 City/ASN використовуйте дані MaxMind, а для DB11, ASN LITE і PX12 — токен IP2Location. Збережіть ключі та оновіть потрібні рядки.",
        "asnNote": "Без ASN недоступна перевірка ISP/мережі. Без PX12 клоака продовжить працювати за ASN, User-Agent і списками блокування.",
        "downloadFrom": "Де завантажити:",
        "sypexFree": "Sypex Geo (безкоштовно)",
        "maxmindAccountIdPlaceholder": "Введіть ідентифікатор облікового запису MaxMind",
        "maxmindAccountIdHint": "Ідентифікатор облікового запису з вашого облікового запису MaxMind (необхідний для GeoLite2).",
        "maxmindPlaceholder": "Введіть ліцензійний ключ MaxMind",
        "maxmindHint": "Необхідний для завантаження оновлень бази даних MaxMind GeoLite2.",
        "ip2locationPlaceholder": "Введіть маркер завантаження IP2Location",
        "ip2locationHint": "Потрібен для автооновлення IP2Location DB11, ASN LITE та IP2Proxy PX12.",
        "saveKeys": "Зберегти ключі",
        "savingKeys": "Збереження...",
        "keysSaved": "Ключі успішно збережено",
        "keySaveError": "Помилка збереження ключа",
        "uploadFile": "Завантажити файл бази (.dat, .zip, .bin, .mmdb)",
        "uploading": "Завантаження...",
        "downloadAndUpload": "Завантажити",
        "andUploadHere": "і завантажити сюди",
        "colName": "Ім'я",
        "colType": "Тип",
        "colStatus": "Статус",
        "colUpdated": "Оновлено",
        "colSize": "Розмір",
        "colActions": "Дії",
        "never": "Ніколи",
        "update": "оновлення",
        "downloading": "Завантаження...",
        "updateSuccess": "Базу даних успішно оновлено",
        "updateError": "Не вдалося оновити БД",
        "noDbs": "Бази даних відсутні",
        "loadingDbs": "Завантаження інформації про базу даних..."
    },
    "campaignEditor": {
        "clickLogTitle": "Журнал кліків",
        "clickLog": "Натисніть Журнал",
        "reports": "Звіти",
        "clickLogFilterAll": "ВСІ",
        "clickLogFilterSafe": "SAFE",
        "clickLogFilterMoney": "MONEY",
        "clickLogVerdict": "Вердикт",
        "clickLogReasons": "Причини",
        "clickLogIsp": "ISP",
        "clickLogAsn": "ASN",
        "clickLogProxyType": "Проксі",
        "clickLogUserAgent": "User-Agent",
        "clickLogNoClicks": "Немає кліків у цій категорії",
        "clickLogLast24h": "За останні 24 години",
        "clickLogOpenDetails": "Відкрити деталі кліку"
    },
    "geoProfiles": {
        "title": "Географічні профілі",
        "searchPlaceholder": "Пошук профілів...",
        "create": "Створити",
        "noProfiles": "Немає геопрофілів. Натисніть «Створити», щоб додати.",
        "notFound": "Профілі не знайдено",
        "edit": "Редагувати",
        "delete": "Видалити",
        "namePlaceholder": "Наприклад: США та Канада",
        "countrySearch": "Пошук країн...",
        "template": "Шаблон",
        "more": "більше",
        "editProfile": "Редагувати геопрофіль",
        "createProfile": "Створити геопрофіль",
        "profileName": "Ім'я",
        "countriesSelected": "Вибрані країни:",
        "selectAll": "Вибрати все",
        "clear": "ясно",
        "selectedCountries": "Вибрані країни:",
        "enterProfileName": "Введіть назву профілю",
        "selectAtLeastOne": "Виберіть хоча б одну країну",
        "saveError": "Помилка збереження:",
        "deleteConfirm": "Видалити цей геопрофіль?",
        "deleteError": "Помилка видалення:",
        "cancel": "Скасувати",
        "save": "зберегти",
        "loading": "Завантаження..."
    },
    "logs": {
        "title": "Системні журнали",
        "lastRecords": "Натисніть «Показати ще», щоб гортати далі, або вивантажте все в CSV (до 100 000 рядків, з урахуванням фільтрів).",
        "shownCount": "Показано записів: {count}.",
        "loadMore": "Показати ще",
        "exportCsv": "Експорт CSV",
        "exportHint": "Завантажити лог поточної вкладки з урахуванням фільтрів (до 100 000 рядків)",
        "colStream": "Потік",
        "filterByStream": "ID потоку",
        "dateFrom": "Дата з",
        "dateTo": "Дата по",
        "traffic": "Трафік",
        "incomingPostbacks": "Вхідні постбеки",
        "sentS2s": "Надіслані S2S",
        "systemLog": "Системний журнал",
        "auditLog": "Журнал аудиту",
        "loadingLogs": "Завантаження журналів...",
        "noData": "Немає даних для відображення",
        "colTime": "Час",
        "colClickId": "ID кліку",
        "colSubid": "SubID",
        "colCampaign": "Кампанія",
        "colIp": "IP",
        "colGeo": "Гео",
        "colDevice": "Пристрій",
        "colStatus": "Статус",
        "colOrigStatus": "Ориг. статус",
        "colPayout": "Виплата",
        "colLevel": "Рівень",
        "colMessage": "Повідомлення",
        "colEvent": "Подія",
        "colResource": "Ресурс",
        "colResponseCode": "Код відповіді",
        "colConversionId": "ID конверсії",
        "colUrl": "URL",
        "colSourceIp": "IP джерела",
        "colResult": "Результат",
        "colError": "Помилка",
        "colSource": "Джерело",
        "resultRecorded": "Записано",
        "resultUpdated": "Оновлено",
        "resultRejected": "Відхилено",
        "resultError": "Помилка",
        "sourcePostback": "Постбек",
        "sourcePixel": "Піксель",
        "direct": "Прямий",
        "colRoute": "Маршрут",
        "colReason": "Причина",
        "colDestination": "Призначення",
        "colLpTime": "Час на ЛП",
        "colIsp": "ISP",
        "colAsn": "ASN",
        "colProxyType": "Тип проксі",
        "routeMoney": "Гроші",
        "routeSafe": "Safe",
        "routeNone": "—",
        "filterByCampaign": "Фільтр за кампанією",
        "filterByRoute": "Фільтр за маршрутом",
        "routeFilterAll": "Усі маршрути",
        "routeFilterMoney": "Лише гроші",
        "routeFilterSafe": "Лише safe",
        "filterByReason": "Фільтр за причиною"
    },
    "campaignReports": {
        "layers": "Шари групування",
        "addLayer": "Додати шар",
        "allCampaigns": "Усі кампанії",
        "campaign": "Кампанія",
        "adsetId": "Adset ID",
        "adId": "Ad ID",
        "adCampaignId": "ID рекламної кампанії",
        "offer": "Оффер",
        "landing": "Лендінг",
        "browser": "Браузер",
        "day": "День",
        "total": "Разом",
        "report": "звіт:",
        "exportCsv": "Експорт CSV",
        "close": "Закрити",
        "grouping": "Групувати за:",
        "period": "Період:",
        "geoCountry": "GEO (країна)",
        "deviceType": "Тип пристрою",
        "language": "Мова браузера",
        "stream": "Потік",
        "source": "Джерело",
        "clicks": "Кліки",
        "unique": "Унікальний",
        "conversions": "Конверсії",
        "revenue": "Дохід",
        "cost": "Вартість",
        "profit": "Прибуток",
        "noDataFilters": "Немає даних для вибраних фільтрів",
        "loadError": "Помилка завантаження звіту:",
        "networkError": "Помилка мережі під час завантаження звіту"
    },
    "navbar": {
        "adminTitle": "Адміністрація",
        "enableDark": "Увімкнути темну тему",
        "enableLight": "Увімкнути світлу тему"
    },
    "app": {
        "dashboard": "Приладова панель",
        "domains": "Управління доменом",
        "backorder": "Монітор замовлень",
        "campaigns": "Кампанії",
        "offers": "Пропозиції",
        "sources": "Джерела трафіку",
        "networks": "Партнерські мережі",
        "conversions": "Журнал перетворення",
        "postback": "Налаштування повернення даних",
        "landings": "Посадки",
        "simulation": "Симуляція дорожнього руху",
        "updated": "Оновлено:",
        "updateAvailable": "Оновлення доступне!",
        "updateDesc": "Версія {version} готова до встановлення. Натисніть, щоб перейти до оновлення.",
        "geoDbTitle": "Не встановлено жодної гео-бази",
        "geoDbText": "Країна та місто не визначаються: гео-фільтри не працюють, а клоака за країнами бачить кожного відвідувача як Unknown — це виглядає як «клоака зламана», хоча бракує лише бази. Встановіть безкоштовну Sypex Geo однією кнопкою або завантажте свою базу в налаштуваннях.",
        "geoDbInstall": "Встановити Sypex Geo (безкоштовно)",
        "geoDbInstalling": "Завантажуємо базу…",
        "geoDbSettings": "Налаштування гео-баз",
        "geoDbFailed": "Не вдалося завантажити — сервер Sypex недоступний з цього сервера. Завантажте базу вручну в налаштуваннях гео-баз."
    },
    "workerHealth": {
        "title": "Фонові задачі потребують уваги",
        "queueWorkerMissing": "Обробник черги постбеків / CAPI не запущено: у черзі {count} подій, найстаріша чекає {minutes} хв. Конверсії не доходять до Facebook, TikTok і партнерок. Встановіть крон: * * * * * php /var/www/orbitra/postback_queue_cron.php",
        "queueStalled": "Черга постбеків / CAPI не розвантажується: {count} подій, найстаріша чекає {minutes} хв.",
        "aggregatorNotScheduled": "Агрегатор витрат не запускається за розкладом — витрати оновлюються лише вручну. Встановіть крон: */15 * * * * php /var/www/orbitra/aggregator_cron.php",
        "shadowedStatuses": "Користувацькі типи конверсій {statuses} перекривають вбудований статус без події Meta — такі конверсії не надсилаються до Conversions API."
    },
    "mainChart": {
        "chartError": "Помилка відтворення діаграми",
        "invalidData": "Недійсний формат даних"
    },
    "dataTables": {
        "records": "записи"
    },
    "telegram": {
        "description": "Підключіть бота Telegram, щоб отримувати статистику кампаній і сповіщення про конверсії безпосередньо в Telegram.",
        "connectBot": "Підключити бота",
        "connectInstructions": "Створіть бота через @BotFather у Telegram, скопіюйте токен і вставте його нижче. Вебхук буде встановлено автоматично.",
        "tokenPlaceholder": "123456:ABC-DEF1234ghIkl-zyx57W2v...",
        "connect": "Підключитися",
        "disconnect": "Відключити",
        "connected": "Бот підключений!",
        "disconnected": "Бот відключено.",
        "connectionError": "Помилка підключення. Перевірте маркер.",
        "statusConnected": "Підключено",
        "statusError": "Помилка підключення",
        "modeWebhook": "вебхук",
        "modePolling": "опитування",
        "connectedPolling": "Telegram не може дістатися до панелі через HTTPS, тому бот сам забирає повідомлення. Більше нічого робити не треба.",
        "pollingReason": "Панель відкрита не на HTTPS-домені, тому Telegram не може її викликати. Натомість бот раз на хвилину сам запитує повідомлення.",
        "pollActive": "Опитування працює. Підключіть чат за кодом нижче.",
        "pollStale": "Опитувач не виходив на зв'язок понад 5 хвилин. Перевірте, чи є завдання в cron, або запустіть вручну:",
        "webhookDeliveryError": "Telegram не зміг доставити оновлення в панель:",
        "webhookUrl": "Вебхук зареєстровано на:",
        "noChats": "До бота ще не підключено жодного чату.",
        "noChatsHintWebhook": "Отримайте код нижче й надішліть його боту. Якщо нічого не відбувається — Telegram не дістався до панелі, перегляньте статус вебхука вище.",
        "noChatsHintPolling": "Отримайте код нижче, надішліть його боту й оновіть за хвилину — опитування йде раз на хвилину.",
        "domainHintTitle": "Відкрийте панель за доменом, а не за IP",
        "domainHintBody": "Telegram викликає вебхук бота лише через HTTPS, на доменному імені з дійсним сертифікатом — на IP-адресу та на звичайний http він не піде. Зараз панель відкрита за адресою",
        "domainHintFallback": "Спрямуйте домен на цей сервер, випустіть для нього сертифікат у розділі «Домени», потім відкрийте панель за https://ваш-домен/admin.php і підключіть бота звідти. Без домену бот теж працює — він сам забирає повідомлення раз на хвилину.",
        "pollingSwitchToDomain": "Щоб перейти на вебхук: відкрийте панель за https://ваш-домен/admin.php і підключіть бота заново. Зараз вона відкрита за адресою",
        "connectedChats": "Підключені чати",
        "linkTitle": "Підключити чат",
        "linkDesc": "Користуватися ботом можуть лише чати, підключені за одноразовим кодом. Отримайте код і надішліть його боту з потрібного чату (особистого чи групи).",
        "linkGenerate": "Отримати код",
        "linkSend": "Надішліть боту:",
        "linkOpenBot": "Відкрити бота",
        "linkExpires": "Код одноразовий і діє 15 хвилин.",
        "linkError": "Не вдалося створити код.",
        "removeChat": "Відключити цей чат",
        "removeChatConfirm": "Відключити {name} від бота? Сповіщення перестануть надходити, доки чат не підключать знову за новим кодом.",
        "chatRemoved": "Чат відключено.",
        "sendTest": "Тестове повідомлення",
        "refresh": "Оновити",
        "testSent": "Тестове повідомлення надіслано!",
        "testFailed": "Не вдалося надіслати тестове повідомлення.",
        "settingsSaved": "Налаштування збережено.",
        "notificationSettings": "Налаштування сповіщень",
        "notifyConversions": "Сповіщення про перетворення",
        "notifyConversionsDesc": "Надсилайте повідомлення для кожної нової конверсії",
        "dailySummary": "Щоденний підсумок",
        "dailySummaryDesc": "Надсилати щоденний статистичний звіт у час:",
        "commandsTitle": "Доступні команди бота",
        "cmdStart": "Вітальне повідомлення",
        "cmdStats": "Сьогоднішня статистика",
        "cmdStats7d": "Статистика за останні 7 днів",
        "cmdCampaigns": "Список активних кампаній",
        "cmdCampaignId": "Конкретні деталі кампанії",
        "cmdTop": "Топ-5 кампаній за доходами",
        "cmdConversions": "Останні 10 конверсій",
        "cmdNotify": "Перемкнути сповіщення про перетворення",
        "cmdDaily": "Перемкнути щоденний підсумок",
        "cmdLang": "Змінити мову бота"
    },
    "settings": {
        "general": "Загальний",
        "profile": "Профіль",
        "bots": "Боти",
        "conversionTypes": "Типи перетворення",
        "customMetrics": "Спеціальні показники",
        "automation": "автоматизація",
        "system": "система",
        "privacy": "Конфіденційність"
    },
    "table": {
        "copyLink": "Копіювати посилання",
        "campaignUrl": "Посилання кампанії",
        "clickLog": "Лог кліків",
        "conversionLog": "Лог конверсій",
        "openInNewTab": "Відкрити в новій вкладці",
        "duplicate": "Дублювати",
        "perPage": "На сторінці",
        "showingRows": "Показано {start}-{end} з {total} рядків",
        "pageSize": "Рядків на стор.",
        "first": "Перша",
        "last": "Остання",
        "prev": "Назад",
        "next": "Далі",
        "all": "Усі",
        "total": "РАЗОМ",
        "name": "Назва",
        "group": "Група",
        "actions": "Дії",
        "selectAll": "Обрати всі рядки",
    },
    "extension": {
        "title": "Оверлей Orbitra для Ads Manager",
        "last3Days": "Останні 3 дні",
        "last4Days": "Останні 4 дні",
        "today": "Сьогодні",
        "yesterday": "Вчора",
        "overview": "Огляд",
        "dailyHistory": "Історія по днях",
        "landingsAndOffers": "Лендінги та Офери",
        "pixelAccuracy": "Точність Pixel та CAPI",
        "winner": "ТОП / WINNER",
        "spend": "Витрати",
        "revenue": "Дохід",
        "profit": "Прибуток",
        "roi": "ROI",
        "cpa": "CPA",
        "cpl": "CPL",
        "cps": "CPS",
    },
    "automation": {
        "status": "Статус",
        "active": "Активно",
        "paused": "Призупинено",
        "clickToPause": "Натисніть, щоб зупинити",
        "clickToResume": "Натисніть, щоб запустити",
        "statusUpdated": "Статус успішно оновлено",
        "statusUpdateError": "Не вдалося оновити статус у рекламній мережі",
        "noConnection": "API-підключення Facebook не знайдено — спочатку підключіть рекламний кабінет в імпорті витрат",
        "unsupportedNetwork": "Ця рекламна мережа ще не підтримується",
        "invalidId": "Невірний id сутності",
        "statusDisapproved": "Відхилено",
        "statusInReview": "На перевірці",
        "title": "Cron і автоматизація",
        "backorderCronTitle": "Замовлення: заплановані перевірки домену",
        "backorderCronDesc": "Перевірка доступності домену виконується окремим сценарієм cron і обробляє 1 домен за один запуск. Тут ви можете ввімкнути/вимкнути виконання та скопіювати готову до використання команду cron.",
        "enableBackorderCron": "Увімкнути перевірки через cron",
        "enableBackorderCronDesc": "Якщо вимкнено, cron може залишатися в системі, але скрипт не перевірятиме домени.",
        "cronCommand": "Команда Cron",
        "cronHint": "Додайте цей рядок до свого сервера crontab. Рекомендовано: кожні 3 хвилини (або частіше, якщо у вас багато доменів).",
        "noCronExample": "Не вдалося створити команду",
        "copy": "Копія",
        "copied": "Скопійовано",
        "copyError": "Помилка копіювання",
        "yes": "так",
        "no": "немає",
        "refresh": "Оновити",
        "saveSuccess": "Налаштування збережено",
        "saveError": "Помилка збереження",
        "loadError": "Помилка завантаження",
        "networkError": "Помилка мережі",
        "installSuccess": "Cron встановлений",
        "installError": "Не вдалося встановити cron",
        "removeSuccess": "Cron видалено",
        "removeError": "Не вдалося видалити cron",
        "installUserSuccess": "Cron (crontab) встановлений",
        "installUserError": "Не вдалося встановити cron (crontab)",
        "removeUserSuccess": "Cron (crontab) видалено",
        "removeUserError": "Не вдалося видалити cron (crontab)",
        "lastPing": "Останній cron ping",
        "lastChecked": "Час останньої перевірки",
        "lastDomain": "Останній домен",
        "lastResult": "Останній результат",
        "domainsTotal": "Всього доменів",
        "domainsNeverChecked": "Ніколи не перевіряла",
        "cronFileTitle": "Cron з інтерфейсу користувача (спроба встановлення)",
        "cronFileDesc": "Зазвичай веб-сервер (PHP) не може писати в /etc/cron.d. Якщо немає дозволів, скористайтеся наведеними нижче командами (потрібен root).",
        "cronFile": "Файл /etc/cron.d",
        "cronInstalled": "встановлено",
        "cronWritable": "Можливість запису",
        "rootRequired": "Потрібні привілеї root",
        "installCron": "Встановити cron",
        "removeCron": "Видаліть cron",
        "cronUserHint": "Користувач Cron: {user}",
        "rootCommandsHint": "Якщо кнопки не працюють (немає дозволів), запустіть на сервері:",
        "installCommand": "Встановити (створити файл):",
        "removeCommand": "видалити:",
        "userCronTitle": "Cron через crontab (без root)",
        "userCronDesc": "Спроба встановити завдання в crontab поточного користувача PHP. Це часто працює на VPS без root. Якщо shell_exec вимкнено, цей метод недоступний.",
        "shellExec": "доступний shell_exec",
        "crontab": "crontab",
        "userCronInstalled": "Присутній запис Orbitra",
        "installUserCron": "Встановити в crontab",
        "removeUserCron": "Видалити з crontab",
        "intervalTitle": "Інтервал повторної перевірки",
        "intervalDesc": "Домен перевіряється знову, коли минуло більше ніж: {interval}. Це впливає на автоматичну перевірку інтерфейсу користувача та cron.",
        "intervalLabel": "Повторно перевіряйте кожен",
        "intervalExample": "Наприклад: 1, 5, 15, 60",
        "intervalPreset1m": "1 хв",
        "intervalPreset5m": "5 хв",
        "intervalPreset15m": "15 хв",
        "intervalPreset60m": "60 хв",
        "rdapTitle": "Завантажувальна програма RDAP (IANA)",
        "rdapBootstrapOk": "Кеш оновлено: {mtime} (вік: {age})",
        "rdapBootstrapMissing": "Кеш початкового завантаження не знайдено. Якщо ви часто бачите повідомлення «Не вдається перевірити», перевірте доступ до сервера за адресою https://data.iana.org/rdap/dns.json.",
        "rdapHint": "«Неможливо перевірити» відображається, якщо для домену верхнього рівня не знайдено жодної служби RDAP і WHOIS не може визначити статус (або не вдається отримати завантажувальний файл і для .com/.net/.name використовується мінімальний запасний варіант).",
        "days": "d",
        "hours": "ч",
        "minutes": "хв"
    },
    "generalSettings": {
        "allowPhpLandings": "Дозволити PHP-лендінги",
        "allowPhpLandingsHint": "Типово вимкнено. PHP-лендінг виконує завантажений код усередині трекера, у вебруті. Завантаження перевіряється на виклики шелла та eval, але перевірка вихідників — не пісочниця: вмикайте лише для лендінгів, чиєму коду довіряєте.",
        "phpLandingTimeout": "Таймаут виконання, секунд",
        "phpLandingTimeoutHint": "Від 1 до 9. Лендінг, що працює довше, обривається — щоб одна повільна сторінка не займала воркери, спільні для всього сайту.",
        "serverIpOverride": "Перевизначення IP сервера",
        "serverIpOverrideHint": "Ручне перевизначення для визначення IP сервера. Залиште порожнім для автовизначення. Ця IP-адреса буде використовуватися для перевірки DNS доменів.",
        "title": "Загальні налаштування",
        "postbackKey": "Ключ повернення",
        "postbackKeyHint": "Використовується для генерації унікальних URL-адрес для повернення S2S.",
        "defaultCurrency": "Валюта за замовчуванням (S2S)",
        "saveSuccess": "Налаштування успішно збережено",
        "saveError": "Помилка",
        "networkError": "Помилка мережі"
    },
    "privacy": {
        "title": "Налаштування конфіденційності",
        "scanProtection": "Захист від сканування",
        "scanProtectionDesc": "Приховує активні посилання від прямого доступу без необхідних параметрів.",
        "directAccessAction": "Дія прямого доступу",
        "redirect302": "Переспрямувати (302)",
        "show404": "Показати помилку 404",
        "blankPage": "Порожня сторінка",
        "redirectUrl": "URL-адреса перенаправлення",
        "redirectUrlHint": "Куди відправити користувача без параметрів кампанії.",
        "errRedirectUrl": "Для дії «Редирект» потрібен валідний http(s)-URL — вкажіть повну адресу, куди надсилатимуться такі запити.",
        "saveSuccess": "Налаштування успішно збережено",
        "saveError": "Помилка",
        "networkError": "Помилка мережі"
    },
    "help": {
        "campaignBannerTitle": "Що таке кампанія?",
        "campaignBanner": "Кампанія є основною сутністю в трекері. Він отримує трафік через унікальну URL-адресу, направляє його через потоки (правила) на пропозиції та цільові сторінки. Кожна кампанія може мати кілька потоків із різними правилами націлювання, геофільтрами та A/B-тестами.",
        "offerBannerTitle": "Що таке пропозиція?",
        "offerBanner": "Пропозиція — це кінцевий пункт призначення трафіку — зазвичай це сторінка рекламодавця. Це може бути URL-адреса переспрямування, вбудований iframe або локальна сторінка. Кожна пропозиція пов’язана з партнерською мережею та відстежує конверсії та дохід.",
        "landingBannerTitle": "Що таке цільова сторінка?",
        "landingBanner": "Цільова сторінка (прелендер) – це проміжна сторінка, яка відображається перед пропозицією. Це розігріває відвідувача і збільшує конверсію. Ви можете тестувати A/B кілька посадок в одному потоці. Посадки можуть бути зовнішніми (URL) або завантаженими у вигляді ZIP-архіву.",
        "trafficSourceBannerTitle": "Що таке джерело трафіку?",
        "trafficSourceBanner": "Джерело трафіку відображає, звідки надходить ваш трафік — рекламні мережі, соціальні медіа тощо. Налаштування джерела дає змогу передавати параметри (підідентифікатори, вартість, ключові слова) і відстежувати рентабельність інвестицій для кожного джерела. Використовуйте шаблони для популярних мереж.",
        "affiliateNetworkBannerTitle": "Що таке партнерська мережа?",
        "affiliateNetworkBanner": "Партнерська мережа є партнером, який надає пропозиції. Додавання мережі дає змогу налаштувати URL-адреси зворотного зв’язку для автоматичного відстеження переходів. Мережа надсилає зворотне повідомлення, коли користувач виконує дію (продаж, залучення тощо).",
        "domainBannerTitle": "Відстеження доменів",
        "domainBanner": "Домен відстеження приховує вашу URL-адресу відстеження, а посилання виглядають природно. Направте A-запис вашого домену на IP-адресу сервера, додайте його сюди та призначте для кампаній. Увімкніть HTTPS і noindex для кращої безпеки та захисту від SEO.",
        "postbackBannerTitle": "Як працює Постбек?",
        "postbackBanner": "Повернення (S2S Postback) — це HTTP-запит від афілійованої мережі до вашого трекера, коли відбувається конверсія. URL-адреса містить ідентифікатор кліку (підідентифікатор), статус і виплату. Трекер зіставляє його з оригінальним кліком і записує конверсію.",
        "postbackMacros": "Доступні макроси: {subid} — ідентифікатор кліку, {status} — статус конверсії, {payout} — сума, {tid} — ідентифікатор транзакції, {currency} — код валюти.",
        "integrationsBannerTitle": "Як інтегрувати?",
        "integrationsBanner": "Виберіть метод інтеграції на основі ваших технічних налаштувань. Tracking Client (PHP) надає всі можливості на стороні сервера (маскування, фільтрація ботів). Tracking Client (JS) простіше встановити. Піксель відстеження працює для базового відстеження показів і конверсій.",
        "conversionsBannerTitle": "Статуси перетворення",
        "conversionsBanner": "Кожна конверсія має статус: лід — новий лід, продаж — підтверджена покупка, відхилено — відхилено рекламодавцем, депозит — перший депозит (азартні ігри), реєстрація — реєстрація користувача. Ви можете налаштувати власні статуси в налаштуваннях.",
        "trendsBannerTitle": "Як використовувати Analytics",
        "trendsBanner": "Виберіть показники для відображення, встановіть діапазон дат і групування. Використовуйте фільтри, щоб звузити список за будь-яким полем натискання. Графік і таблиця оновлюються в режимі реального часу. Порівняйте показники, щоб знайти закономірності — наприклад, висока кількість кліків, але низький CR може вказувати на проблеми з цільовою сторінкою.",
        "aliasTooltip": "Унікальний короткий ідентифікатор URL-адреси кампанії. Створено автоматично, але його можна змінити. Використовується в посиланні відстеження.",
        "costModelTooltip": "CPC — оплата за клік. CPA — плата за конверсію. CPM — оплата за 1000 показів. RevShare — відсоток доходу. CPuC — за унікальний клік. CPS — за продаж.",
        "uniquenessTooltip": "Як визначити унікальних відвідувачів. IP — лише за IP-адресою. IP+UA — за IP та User-Agent (точніше). Файли cookie — файли cookie браузера (найбільш точні, але їх можна заблокувати).",
        "rotationTooltip": "Вага — розподіляє трафік пропорційно значенням ваги. Позиція — надсилає весь трафік до першого активного потоку, інші використовує як запасні.",
        "redirectTypeTooltip": "Редирект — стандартний 302 редирект (швидкий, універсальний). iFrame — завантажується в невидимий фрейм (добре для маскування). Локальний — обслуговує локальну сторінку (найкраще для складних послідовностей).",
        "offerPostbackTooltip": "URL-адреса, яку партнерська мережа надсилає, коли відбувається конверсія. Має містити макрос ідентифікатора кліку (зазвичай {subid} або {clickid}). Установіть цю URL-адресу на інформаційній панелі партнерської мережі.",
        "postbackKeyTooltip": "Ключ безпеки додано до URL-адреси повернення даних. Запобігає неавторизованим зворотним відправленням від третіх сторін. Змініть його, якщо ваш ключ буде зламано.",
        "postbackAliasesTooltip": "Відображення імен параметрів. Якщо мережа надсилає «clickid» замість «subid», додайте його як псевдонім. Трекер прийматиме обидві назви параметрів.",
        "indexCampaignTooltip": "Яку кампанію показувати, коли хтось відвідує кореневий домен (наприклад, yourdomain.com). Якщо не встановлено, відображається порожня сторінка.",
        "httpsTooltip": "Автоматично перенаправляти всі HTTP-запити на HTTPS. Рекомендовано для безпеки. Переконайтеся, що ваш сертифікат SSL налаштовано.",
        "streamSchemaTooltip": "Налаштуйте, які пропозиції та цільові сторінки відображатимуться. Додайте кілька елементів і встановіть ваги для тестування A/B. Загальний розподіл ваги показано вище.",
        "streamFilterTooltip": "Встановіть умови, коли цей потік має бути активований: за країною, пристроєм, ОС, браузером, мовою тощо. Трафік, який не відповідає жодному потоку, спрямовується до загального потоку.",
        "dismissAll": "зрозумів",
        "showHelp": "Показати поради"
    },
    "pixelVault": {
        "pixelVaultTitle": "Сховище пікселів та CAPI",
        "quickAddTitle": "Швидке додавання пікселя в базу",
        "saveAndSelect": "Зберегти та обрати",
        "description": "Керуйте багаторазовими пікселями та серверними профілями конверсій із групуванням за нішами.",
        "addNewPixel": "Додати піксель",
        "trafficSource": "Джерело трафіку",
        "nicheTag": "Тег ніші (напр. Nutra, Crypto)",
        "pixelName": "Назва пікселя",
        "pixelId": "ID пікселя",
        "capiToken": "Токен доступу Conversions API",
        "keepToken": "Залиште порожнім, щоб зберегти поточний токен",
        "events": "Події",
        "eventUrl": "URL події (необов’язково)",
        "testEventCode": "Код тестової події (необов’язково)",
        "active": "Активний",
        "sendTestEvent": "Надіслати тестову подію",
        "sending": "Надсилання…",
        "testEventSent": "Тестову подію успішно надіслано в Meta!",
        "editPixel": "Редагувати профіль пікселя",
        "allNiches": "Усі ніші",
        "allSources": "Усі",
        "search": "Знайти піксель…",
        "token": "Токен",
        "duplicate": "Дублювати",
        "confirmDelete": "Видалити профіль і від’єднати його від усіх кампаній?",
        "deleted": "Профіль пікселя видалено",
        "duplicated": "Профіль пікселя продубльовано",
        "saved": "Профіль пікселя збережено",
        "testUnsupported": "Тестові події підтримуються для профілів Facebook і TikTok.",
        "noProfiles": "Немає профілів пікселів, що відповідають фільтрам.",
        "attachPixel": "Підключити збережений піксель",
        "noPixel": "Піксель не підключено",
        "niche": "Ніша",
        "inactive": "неактивний",
        "pixelHint": "Підключення профілю автоматично надсилає серверні CAPI-конверсії цієї кампанії.",
        "saveCampaignFirst": "Збережіть кампанію перед підключенням профілю зі сховища.",
        "attached": "Профіль пікселя підключено",
        "detached": "Профіль пікселя відключено",
        "vaultProfile": "Сховище"
    },
    "pixels": {
        "title": "Пікселі відстеження",
        "addPixel": "Додайте піксель",
        "pixelId": "Ідентифікатор пікселя / ідентифікатор тегу",
        "apiToken": "Маркер API (необов'язково)",
        "events": "Події",
        "eventsHint": "Розділені комами: PageView, Lead, Purchase",
        "active": "Активний",
        "noPixels": "Немає підключених пікселів",
        "noPixelsDesc": "Додайте пікселі відстеження з рекламних мереж, щоб автоматично надсилати події конверсії",
        "saved": "Піксель збережено",
        "deleted": "Піксель видалено",
        "confirmDelete": "Видалити цей піксель?",
        "selectPlatform": "Виберіть платформу, щоб додати піксель",
        "capiTitle": "Conversions API (server-side)",
        "capiHint": "Якщо вказано токен Conversions API, Orbitra надсилає кожну конверсію в Meta з сервера — такі події не губляться на блокувальниках і обмеженнях iOS, які ріжуть браузерний піксель.",
        "mapping": "Статус → подія Meta",
        "doNotSend": "Не надсилати",
        "testEventCode": "Test event code",
        "proxy": "Проксі (опційно)",
        "sendTestEvent": "Надіслати тестову подію",
        "sending": "Надсилання…",
        "syntheticClick": "(свіжого кліку з fbclid немає — використано синтетичний)",
        "eventSourceUrl": "URL події / сторінки подяки",
        "eventSourceUrlHint": "Надсилається до Meta CAPI як event_source_url. Підтримує макроси {campaign_url}, {landing_url}, {offer_url}, {clickid} або JSON-мапу за назвою події Meta."
    },
    "appConfig": {
        "title": "Конфігурація програми",
        "subtitle": "Віддалена конфігурація JSON для програм і веб-сайтів",
        "name": "Назва конфігурації",
        "campaign": "Пов’язана кампанія (необов’язково)",
        "configKey": "відкритий ключ",
        "configUrl": "URL конфігурації",
        "jsonEditor": "Конфігурація JSON",
        "active": "Активний",
        "inactive": "Неактивний",
        "create": "Створити конфігурацію",
        "edit": "Редагувати конфігурацію",
        "download": "Завантажити JSON",
        "copyUrl": "Копіювати URL",
        "noConfigs": "Конфігурації не створено",
        "noConfigsDesc": "Створіть конфігурацію JSON, щоб дистанційно керувати своїми програмами та веб-сайтами",
        "saved": "Конфігурацію збережено",
        "deleted": "Конфігурацію видалено",
        "confirmDelete": "Видалити цю конфігурацію?",
        "invalidJson": "Недійсний формат JSON",
        "templateDefault": "Шаблон за замовчуванням",
        "templateWebview": "Програма WebView",
        "templateBanner": "Контроль банерів",
        "templateStub": "Перемикач заглушки/вимкнення",
        "previewTitle": "Попередній перегляд JSON",
        "webviewEnabled": "WebView увімкнено",
        "webviewUrl": "URL-адреса WebView",
        "bannerEnabled": "Банер увімкнено",
        "bannerImage": "URL-адреса зображення банера",
        "bannerClickUrl": "URL-адреса клацання банера",
        "stubEnabled": "Заглушку ввімкнено",
        "stubMessage": "Заглушка",
        "forceUpdate": "Примусове оновлення",
        "appVersion": "Версія програми"
    },
    "wordpress": {
        "title": "WordPress",
        "description": "Фрагмент PHP для теми WordPress functions.php. Перевіряє віддалену конфігурацію та умовно відображає вміст.",
        "instruction": "Додайте цей код до файлу functions.php вашої теми. Замініть CONFIG_KEY на фактичний ключ конфігурації."
    },
    "staticSite": {
        "title": "Статичний сайт",
        "description": "Код JavaScript для статичних сайтів. Отримує віддалену конфігурацію та контролює елементи сторінки.",
        "instruction": "Додайте цей сценарій перед закриваючим тегом </body>. Замініть CONFIG_KEY на фактичний ключ конфігурації."
    },
    "branding": {
        "saveError": "Не вдалося зберегти налаштування. Спробуйте ще раз.",
        "saving": "Збереження...",
        "themeTitle": "Тема інтерфейсу",
        "light": "світло",
        "lightDesc": "Чистий стиль SaaS",
        "dark": "темний (графіт)",
        "darkDesc": "Глибокий фон, приємний для очей",
        "green": "хвойні",
        "greenDesc": "Глибокий зелений і м'ятний",
        "neon": "Неонова ніч",
        "neonDesc": "Абсолютний чорний фон, яскраві кольори",
        "cobalt": "Кобальт",
        "cobaltDesc": "Сигнальний синій на білому тлі",
        "cobaltDark": "Темний кобальт",
        "cobaltDarkDesc": "Сигнальний синій на глибокому чорному",
        "canary": "Канарка",
        "canaryDesc": "Чорні акценти й канарковий жовтий",
        "canaryDark": "Темна канарка",
        "canaryDarkDesc": "Канарковий жовтий на теплому графіті",
        "parchment": "Пергамент",
        "parchmentDesc": "Теплий кремовий фон і чорні кнопки",
        "parchmentDark": "Темний пергамент",
        "parchmentDarkDesc": "Чорне тло з кремовими акцентами",
        "indigo": "Індиго",
        "indigoDesc": "Синьо-фіолетові акценти на світлому тлі",
        "indigoDark": "Темне індиго",
        "indigoDarkDesc": "Синьо-фіолетова палітра на глибокому індиго",
        "terminal": "Термінал",
        "terminalDesc": "Фосфорно-зелений ЕПТ-екран: сканлайни, вінєтка, табличні цифри",
        "aurora": "Аврора",
        "auroraDesc": "Холодне світло, що пливе темно-синім, і сяйво під шапкою",
        "custom": "Спеціальна тема",
        "customDesc": "Налаштуйте кольори відповідно до свого бренду",
        "customColorsTitle": "Спеціальна палітра кольорів",
        "customColorsDesc": "Натисніть кольорові кола, щоб вибрати кольори свого бренду.",
        "primaryAccent": "Основний акцент",
        "mainBg": "Основний фон",
        "cardBg": "фон картки",
        "textPrimary": "Основний текст",
        "resetColors": "Скинути кольори",
        "preview": "Попередній перегляд",
        "buttons": "кнопки",
        "badges": "Значки",
        "forms": "Форми",
        "links": "Посилання",
        "linkExample": "Приклад посилання",
        "reset": "Скинути",
        "saved": "Збережено!",
        "saveSettings": "Зберегти налаштування"
    },
    "integrations": {
        "hubTitle": "Центр інтеграцій",
        "hubSubtitle": "Підключайте витрати на трафік, Conversion API, домени та інструменти трекінгу в єдиному хабі.",
        "connectionsLive": "Активних підключень",
        "filterAll": "Всі",
        "filterAds": "Реклама та Витрати",
        "filterDomains": "Домени та SSL",
        "filterSites": "Трекінг та Сайти",
        "filterTools": "Інструменти та API",
        "searchPlaceholder": "Пошук інтеграцій, рекламних мереж, доменів, скриптів...",
        "noResults": "Інтеграцій не знайдено",
        "noResultsDesc": "Спробуйте інший пошуковий запит або змініть категорію.",
        "resetFilters": "Скинути фільтри",
        "backToAll": "Назад до всіх інтеграцій",
        "configure": "Налаштувати",
        "connectModeManual": "Прямий токен",
        "connectModeOauth": "1-Клік OAuth",
        "manageAccounts": "Керування акаунтами",
        "manageCapi": "Керування CAPI",
        "manageVault": "Відкрити сховище",
        "viewApiKeys": "API Ключі",
        "manageDns": "Керування DNS",
        "openRegistrar": "Відкрити реєстратор",
        "getCode": "Отримати код",
        "getScript": "Отримати скрипт",
        "getPixel": "Отримати піксель",
        "downloadPlugin": "Завантажити плагін",
        "configureBot": "Налаштувати бота",
        "manageConfigs": "Керування конфігами",
        "configureKeys": "Налаштувати ключі",
        "getWidget": "Отримати віджет",
        "groupAds": "Рекламні мережі",
        "groupDomains": "Домени та SSL",
        "groupSites": "Сайти та лендінги",
        "groupTools": "Інструменти",
        "groupOther": "Інше",
        "chromeExtension": "Розширення Chrome та Антидетект",
        "chromeExtDesc": "Відображення статистики трекера в реальному часі прямо у Facebook Ads Manager всередині AdsPower, Dolphin{anty} та Chrome.",
        "extTemplate1": "Шаблон 1: CPA / Воронка (CPA, Спенд, Дохід, ROI, Профіт)",
        "extTemplate2": "Шаблон 2: COD / Нутра (CPL, CPS, Спенд, Підтвердж. дохід, ROI conf, Профіт)",
        "downloadExtension": "Завантажити розширення (.ZIP)",
        "antidetectInstallGuide": "Інструкція з встановлення в антидетект-браузери",
        "adspowerGuide": "AdsPower: відкрийте Центр додатків → Завантажити розширення → оберіть розпаковану папку.",
        "dolphinGuide": "Dolphin{anty}: перейдіть у Налаштування → Розширення → Додати розширення або перетягніть ZIP-архів.",
        "chromeGuide": "Google Chrome: відкрийте chrome://extensions → увімкніть Режим розробника → Завантажити розпаковане.",
        "howItWorks": "Як працюють інтеграції",
        "introText": "Інтегруйте Orbitra зі своїми зовнішніми цільовими сторінками або програмами за допомогою одного з цих методів. Це дозволяє відстежувати трафік поза стандартними посиланнями відстеження, централізуючи дані у вашій базі даних Orbitra.",
        "usageExamples": "Приклади використання:",
        "usageList1": "Відстежуйте кліки та характеристики кліків із веб-сайтів (географічне положення, пристрій, агент користувача).",
        "usageList2": "Відстежуйте конверсії з веб-сайтів за допомогою інструментів відстеження без обхідних шляхів.",
        "usageList3": "Запускайте більшість доступних функцій безпосередньо на веб-сайтах: фільтрацію трафіку за критеріями, перенаправлення, спліт-тести вмісту.",
        "introFooter": "Кожен із наведених нижче сценаріїв розроблено для конкретних випадків використання.",
        "kclientPhpIntro": "Підключає трекер до багатосторінкових веб-сайтів.",
        "kclientJsIntro": "Для розробників веб-сайтів, таких як Shopify або Tilda.",
        "jsBannerIntro": "Відображає банери на вашому сайті та дозволяє проводити спліт-тестування.",
        "kclientPhpDesc": "Підключає трекер до багатосторінкових веб-сайтів і цільових сторінок PHP. Виконує перенаправлення на стороні сервера або заміну вмісту без перенаправлення.",
        "kclientJsDesc": "Скрипт відстеження на стороні клієнта. Ідеально підходить для конструкторів веб-сайтів (Tilda, Shopify, Webflow) і статичних HTML-сторінок.",
        "jsBannerTitle": "JS реклама (банер)",
        "jsBannerDesc": "Відображає банери або рекламні блоки на вашому сайті. Дозволяє розділити банери та відстежувати кліки/покази.",
        "pixelDesc": "Найпростіший спосіб відстежувати перегляди сторінок або конверсії на сторонніх сайтах, де не можна розмістити JS (наприклад, в електронних розсилках).",
        "tiktokPixelTitle": "TikTok Піксель",
        "tiktokPixelDesc": "Браузерний піксель TikTok на локальних лендах. ID пікселя приходить у параметрі кампанії {pixel} із шаблону джерела TikTok і зберігається в cookie для сторінки подяки.",
        "codeToInsert": "Код для вставки на сайт:",
        "copyCode": "Копіювати код",
        "copied": "Скопійовано!",
        "phpWarning": "Файл kclient.php потрібно завантажити з кореня трекера та розмістити поруч із файлом index.php вашої цільової сторінки.",
        "important": "Важливо:",
        "geoRedirectTitle": "Геоперенаправлення",
        "geoRedirectDesc": "Перенаправляйте користувачів на різні пропозиції залежно від їхньої країни. Ідеально підходить для кількох географічних кампаній.",
        "deviceRedirectTitle": "Перенаправлення пристрою",
        "deviceRedirectDesc": "Перенаправляйте користувачів залежно від типу пристрою – мобільного, планшета чи комп’ютера.",
        "countdownTitle": "Таймер зворотного відліку",
        "countdownDesc": "Створіть терміновість за допомогою таймера зворотного відліку, який переспрямовує на пропозицію, коли час закінчується.",
        "backButtonTitle": "Пастка кнопки «Назад».",
        "backButtonDesc": "Перехоплення кнопки «Назад» у браузері та перенаправлення на спеціальну пропозицію замість попередньої сторінки.",
        "exitPopupTitle": "Вийти зі спливаючого вікна намірів",
        "exitPopupDesc": "Показувати спливаюче вікно, коли користувач намагається залишити сторінку (переміщує мишу вгору вікна).",
        "wpPluginTitle": "Плагін WordPress",
        "wpPluginDesc": "Завантажте готовий до використання плагін WordPress із короткими кодами для геоперенаправлення, банерів і відстеження.",
        "yandex": "Яндекс Метрика"
    },
    "wpPlugin": {
        "downloadTitle": "Плагін WordPress",
        "downloadDesc": "Завантажте готовий до використання плагін, попередньо налаштований для вашого трекера",
        "downloadBtn": "Завантажити плагін",
        "trackerUrl": "Ваша URL-адреса трекера",
        "featuresTitle": "Функції плагіна",
        "feature1Title": "Геоперенаправлення",
        "feature1Desc": "Перенаправляйте відвідувачів на різні пропозиції залежно від їхньої країни",
        "feature2Title": "Піксель відстеження",
        "feature2Desc": "Піксель автоматичного відстеження на всіх сторінках",
        "feature3Title": "Шорткоди банерів",
        "feature3Desc": "Легко додавайте відстежувані банери будь-де",
        "feature4Title": "Таймери зворотного відліку",
        "feature4Desc": "Створіть терміновість за допомогою таймерів зворотного відліку",
        "feature5Title": "Умовний зміст",
        "feature5Desc": "Показати/приховати вміст за країною відвідувача",
        "shortcodesTitle": "Доступні короткі коди",
        "shortcodeLink": "Відстежуване посилання з геоперенаправленнями та багатомовним текстом",
        "shortcodeButton": "Стильна кнопка CTA з дизайном і багатомовною підтримкою",
        "shortcodeBanner": "Чуйне зображення банера, яке можна відстежувати",
        "shortcodeGeo": "Показувати вміст лише для певних країн",
        "shortcodeCountdown": "Таймер зворотного відліку з кнопкою CTA",
        "installTitle": "монтаж",
        "install1": "Завантажте ZIP-файл плагіна",
        "install2": "Перейдіть до адміністратора WordPress → Плагіни → Додати новий → Завантажити плагін",
        "install3": "Активуйте плагін",
        "install4": "Налаштувати в Параметрах → Orbitra Tracker",
        "pluginDesc": "Інтеграція Orbitra Tracker - геоперенаправлення, пікселі відстеження, банери, зворотний відлік."
    },
    "migrations": {
        "title": "Міграція бази даних",
        "infoText": "Час виконання міграції залежить від кількості статистики. Ця таблиця відстежує всі структурні зміни в базі даних трекера.",
        "colVersion": "Версія",
        "colDescription": "опис",
        "colStatus": "Статус",
        "colActions": "Дії",
        "completed": "Виконано",
        "pending": "В очікуванні",
        "running": "Біг...",
        "repeat": "Повторіть",
        "execute": "Виконати",
        "runError": "Не вдалося виконати міграцію",
        "noMigrations": "Міграцій не знайдено",
        "loadingMigrations": "Завантаження міграцій...",
        "keitaroTitle": "Імпорт із Keitaro (дамп SQL)",
        "keitaroBackupTitle": "Як створити резервну копію Keitaro для міграції",
        "backupStep1Title": "Підключіться до сервера Keitaro",
        "backupStep1Desc": "Відкрийте свій термінал і підключіться через SSH до свого сервера Keitaro:",
        "backupStep2Title": "Виконайте команду резервного копіювання",
        "backupStep2Desc": "Скопіюйте та виконайте наведену нижче команду. Він створить /root/keitaro_orbitra_full.sql.gz із необхідними таблицями (кампанії, пропозиції, домени, потоки тощо БЕЗ журналів кліків):",
        "backupStep3Title": "Завантажте файл на комп'ютер",
        "backupStep3Desc": "Замініть YOUR_KEITARO_SERVER_IP на вашу фактичну IP-адресу сервера:",
        "copyCommand": "Копіювати команду",
        "backupTipTitle": "Підказка",
        "backupTipText": "Файл буде невеликим (зазвичай 1-5 МБ), оскільки він містить лише конфігурацію, без статистики кліків.",
        "keitaroInfo": "Завантажте файл mysqldump (.sql або .sql.gz). Orbitra аналізуватиме оператори INSERT та імпортуватиме вибрані сутності (домени, пропозиції, компанії/партнерські мережі, кампанії та зворотні перекази кампаній) у свою базу даних SQLite. Примітка: щоб перенести потоки/потоки, вам також потрібні keitaro_streams і таблиці асоціацій (додайте їх до дампа окремо). Спочатку рекомендується попередній перегляд (сухий прогін).",
        "keitaroFile": "Файл Keitaro SQL",
        "keitaroFileHint": "Підтримує .sql і .sql.gz. Дамп не виконується як SQL, він лише аналізується.",
        "keitaroOptions": "Опції",
        "keitaroDryRun": "Попередній перегляд (сухий запуск, без запису в БД)",
        "dryRunNote1": "Дані на Орбітру не записувалися. Натисніть",
        "dryRunNote2": "для реального імпорту.",
        "keitaroCompanies": "Імпортувати партнерські мережі",
        "keitaroOffers": "Імпорт пропозицій",
        "keitaroDomains": "Імпорт доменів",
        "keitaroTrafficSources": "Імпорт джерел трафіку",
        "keitaroLandings": "Імпортні посадки",
        "keitaroCampaigns": "Імпортувати кампанії",
        "keitaroPreserveCampaignIds": "Зберігати ідентифікатори кампанії (ідентифікатор Orbitra = ідентифікатор Keitaro)",
        "keitaroPreserveCampaignIdsHint": "Працює лише на свіжій Orbitra: таблиця кампаній має бути порожньою (включно з архівними).",
        "keitaroStreams": "Імпорт потоків/потоків",
        "keitaroCampaignPostbacks": "Імпорт зворотних даних кампанії",
        "keitaroNoFile": "Виберіть файл .sql (або .sql.gz).",
        "keitaroRunning": "Імпорт...",
        "keitaroPreviewBtn": "Попередній перегляд",
        "keitaroImportBtn": "Імпорт в Orbitra",
        "purgeTitle": "Скинути (тестовий сервер)",
        "purgeInfo": "Небезпечна операція. Видаляє налаштування трекера (домени, пропозиції, партнерські мережі, кампанії, потоки, зворотні передачі, джерела трафіку, лендінги та групи). Не торкається користувачів або статистики (кліків/конверсій). Використовуйте на тестовому сервері перед повторним імпортом Keitaro.",
        "purgeConfirmLabel": "Підтвердження",
        "purgeConfirmHint": "Введіть DELETE (великі літери), щоб активувати кнопку.",
        "purgeWhat": "Що буде видалено",
        "purgeWhatHint": "Домени, пропозиції, афілійовані мережі, кампанії, потоки, зворотні передачі, пікселі, джерела, лендінги та групи.",
        "purgeBtn": "Видалити все",
        "purgeRunning": "Видалення...",
        "purgeDisabledHint": "Введіть DELETE, щоб активувати кнопку.",
        "descriptions": {
            "v1": "Встановлення базової схеми бази даних",
            "v2": "Додавання permissions_json для користувачів",
            "v3": "Оновлення часового поясу та мови",
            "v4": "Додавання is_archived до ресурсів",
            "v5": "Додавання archive_retention_days до налаштувань",
            "v6": "Створення таблиці user_preferences",
            "v7": "Оптимізація індексів клацання",
            "v8": "Конфігурація безпеки сеансу"
        },
        "copied": "Скопійовано!"
    },
    "update": {
        "gitUpdateComposerNote": "Третій рядок завантажує запіннований Composer (2.10.3), лише якщо на сервері ще немає composer.phar — інсталяції до v1.6.0 зберігають свій. Кнопка оновлення в панелі робить те саме автоматично і перевіряє контрольну суму.",
        "cachedResult": "Показано кеш перевірки від",
        "checkFailed": "Не вдалося перевірити оновлення на GitHub — «актуальна версія» може бути застарілою. Оновіть вручну: SSH → cd /var/www/orbitra && git pull",
        "checkingUpdates": "Перевірка оновлень...",
        "versionInfo": "Інформація про версію",
        "checkUpdates": "Перевірте наявність оновлень",
        "currentVersion": "Поточна версія",
        "latestVersion": "Остання версія",
        "updateAvailable": "Оновлення доступне!",
        "newVersionText": "Доступна нова версія",
        "recommendUpdate": "Рекомендовано оновити трекер.",
        "changelog": "Журнал змін:",
        "upToDate": "Остання версія встановлена",
        "noUpdates": "Оновлень не знайдено.",
        "updateMethods": "Методи оновлення",
        "autoUpdate": "Автоматичне оновлення",
        "autoUpdateDesc": "Завантажте та встановіть оновлення автоматично",
        "updating": "Оновлення...",
        "updatingTitle": "Оновлення системи",
        "updateBtn": "оновлення",
        "forceConfirm": "The system says you're up to date (by version). Run git pull and update files anyway?",
        "updateSuccess": "Update installed successfully",
        "updateComplete": "Update Complete!",
        "updateCompleteDesc": "You have the latest version installed",
        "downloading": "Downloading update...",
        "installing": "Installing update...",
        "complete": "Completing...",
        "stepDownload": "Download",
        "stepInstall": "Install",
        "stepComplete": "Done",
        "gitUpdate": "Update via Git",
        "gitUpdateDesc": "Ручний спосіб для Git-встановлення. Запускайте від системного користувача, якому належить каталог Orbitra.",
        "downloadArchive": "Download Archive",
        "downloadArchiveDesc": "Download the new version and replace files manually",
        "downloadLink": "(Download link will be available after publication)",
        "serverConfig": "Update Server Configuration",
        "serverConfigDesc": "To enable automatic updates, configure the update server URL in the file",
        "serverConfigBlock": "in the block",
        "updateError": "Update error",
        "checkError": "Failed to check for updates"
    },
    "challenge": {
        "sectionTitle": "Bot Challenge",
        "sectionDesc": "Show a human verification page before redirecting to the offer. Recommended for email campaigns where corporate security bots cause fake clicks.",
        "typeLabel": "Verification type",
        "typeNone": "Disabled",
        "typeNoneDesc": "No verification — direct redirect (default)",
        "typeTurnstile": "Cloudflare Turnstile",
        "typeTurnstileDesc": "Розумна невидима перевірка ботів від Cloudflare. Не вимагає кліків від 99%+ реальних людей.",
        "typeV2": "reCAPTCHA v2",
        "typeV2Desc": "Classic \"I'm not a robot\" checkbox. Requires reCAPTCHA v2 keys in Integrations.",
        "typeV3": "reCAPTCHA v3 (invisible)",
        "typeV3Desc": "Score-based invisible check. Low score triggers the challenge. Requires reCAPTCHA v3 keys in Integrations.",
        "typeCustom": "Custom code",
        "typeCustomDesc": "Paste your own HTML/JS verification widget.",
        "customCodeLabel": "Custom verification HTML/JS",
        "customCodePlaceholder": "<!-- Your verification widget code here -->\n<!-- After the user passes, submit a form with hidden field: -->\n<!-- <input type=\"hidden\" name=\"_challenge_ok\" value=\"1\"> -->",
        "customCodeHint": "The code runs on the visitor's side. After verification, your form must POST with a hidden field _challenge_ok=1 to complete the flow.",
        "noKeysWarning": "reCAPTCHA keys are not configured. Go to Integrations → reCAPTCHA to add them.",
        "infoEmailTitle": "Recommended for email traffic",
        "infoEmailDesc": "Corporate email security gateways auto-click all links in emails, creating thousands of fake clicks. The Bot Challenge filters them out before they ever reach your stats.",
        "setupGuideTitle": "Швидке налаштування (3 кроки)",
        "setupStep1": "Створіть сайт reCAPTCHA на сторінці",
        "setupStep2": "Виберіть тип {type} при реєстрації сайту.",
        "setupStep3": "Скопіюйте згенеровані ключі, перейдіть до розділу",
        "setupStep3Path": "Інтеграції → reCAPTCHA в боковому меню, вставте їх у Site Key та Secret Key та збережіть.",
        "setupStep4": "Збережіть цю кампанію — захист активується миттєво."
    },
    "recaptcha": {
        "tabTitle": "reCAPTCHA",
        "tabDesc": "Налаштуйте ключі Google reCAPTCHA для функції Bot Challenge. Ключі спільні для всіх кампаній.",
        "v2Title": "Ключі reCAPTCHA v2",
        "v2Desc": "Класичний прапорець «Я не робот». Отримайте ключі в консолі адміністратора Google reCAPTCHA.",
        "v3Title": "Ключі reCAPTCHA v3",
        "v3Desc": "Невидима перевірка на основі балів. Оцінки варіюються від 0,0 (бот) до 1,0 (людина).",
        "siteKey": "Ключ сайту",
        "secretKey": "Секретний ключ",
        "siteKeyPlaceholder": "6Lc...",
        "secretKeyPlaceholder": "6Lc...",
        "v3Threshold": "Мінімальний поріг балів",
        "v3ThresholdDesc": "Клацання з оцінкою нижче цього значення показують сторінку завдання. Рекомендовано: 0,5",
        "turnstileTitle": "Ключі Cloudflare Turnstile",
        "turnstileDesc": "Розумна невидима перевірка ботів без картинок та прапорців. Отримайте Site Key та Secret Key у панелі Cloudflare Dashboard.",
        "adminConsoleLink": "Відкрийте консоль адміністратора reCAPTCHA",
        "saved": "Налаштування reCAPTCHA збережено",
        "saveError": "Не вдалося зберегти налаштування reCAPTCHA",
        "configured": "Налаштовано",
        "notConfigured": "Не налаштовано"
    },
    "emailSource": {
        "espHintsTitle": "Посилання на теги злиття ESP",
        "espHintsDesc": "Зіставте кожен параметр із тегом злиття вашого постачальника послуг електронної пошти. Приклади значень ESP:",
        "espCol": "ESP",
        "subscriberIdCol": "subscriber_id",
        "campaignIdCol": "ідентифікатор_кампанії",
        "listIdCol": "list_id",
        "broadcastIdCol": "broadcast_id",
        "espNameCol": "esp (статичне значення)",
        "hintsNote": "Вставте тег злиття з таблиці в поле макросу відповідного параметра.",
        "showHints": "Показати посилання ESP",
        "hideHints": "Приховати посилання ESP"
    },
    "redirectTypes": {
        "jsName": "JS-редирект",
        "metaName": "Meta Refresh",
        "iframeName": "Iframe",
        "formName": "Form Submit",
        "curlProxyName": "curl-проксі",
        "redirectDesc": "Стандартний HTTP-редирект 302. Швидкий, типовий для більшості офферів.",
        "jsDesc": "Віддає HTML-сторінку з редиректом через JavaScript. Оминує блокування серверного редиректу і зберігає referrer у нових браузерах.",
        "metaDesc": "Використовує тег meta refresh. Максимальна сумісність зі старими або обмеженими браузерами.",
        "iframeDesc": "Показує оффер в iframe на весь екран. Відвідувач залишається на вашому домені, переглядаючи оффер.",
        "formDesc": "Авто-відправка POST-форми: оффер отримує дані трекінгу в тілі запиту, а не в URL.",
        "preloadDesc": "Завантажує віддалену сторінку через ваш сервер і підставляє base-тег, щоб ресурси резолвились.",
        "curlProxyDesc": "Віддає віддалену сторінку через цей сервер з base-тегом. Корисно, коли треба розмістити контент під своїм доменом."
    },
    "cloaking": {
        "targetingTitle": "Фільтри таргетингу (невідповідність → біла сторінка)",
        "geoFilter": "Фільтр країн",
        "geoPlaceholder": "Виберіть країни (наприклад, US, DE, GB)...",
        "allowIn": "Дозволити (у списку)",
        "blockIn": "Заблокувати (не у списку)",
        "deviceFilter": "Типи пристроїв",
        "allowOnly": "Лише обрані",
        "blockSelected": "Заблокувати обрані",
        "blockBotIsps": "Блокувати ботів і датацентр-провайдерів (Facebook, Google, Amazon, Hetzner та ін.)",
        "botIspPlaceholder": "Локальний список: один провайдер на рядок (порожньо — глобальний список)",
        "botIspHint": "Один провайдер на рядок, перевіряється за ISP та ASN відвідувача. Залиште порожнім, щоб використовувати глобальний список із Налаштування → Боти.",
        "botIspIgnoredWarning": "Проігноровані записи (занадто короткі або загальний корпоративний суфікс — вони збігалися б майже з кожним ISP)",
        "jsChallenge": "Перевірка JavaScript",
        "jsChallengeHint": "Відвідувач спершу бачить білу сторінку; фонова перевірка браузера вирішує, чи пускати його на money page. Усе, що не виконує JS, лишається на білій. Додає один перехід.",
        "mode": "Cloaking",
        "description": "Підозрілі відвідувачі (боти, модератори, трафік з дата-центрів/VPN) бачать безпечну сторінку; реальні люди — грошову.",
        "detectionLayers": "Шари визначення",
        "datacenter": "Дата-центри / хостинги",
        "vpnProxy": "VPN / проксі",
        "bots": "Чорні списки ботів",
        "uaHeuristics": "Евристика User-Agent",
        "sensitivity": "Чутливість",
        "sensitivityLow": "Низька",
        "sensitivityMedium": "Середня",
        "sensitivityHigh": "Висока",
        "sensitivityHint": "Низька позначає лише явні потрапляння у списки або кілька сигналів. Висока — будь-який сигнал (максимально агресивно).",
        "safePage": "Безпечна сторінка (для ботів)",
        "safePageHint": "Показується підозрілим відвідувачам. Оберіть лендинг, URL або вбудований HTML.",
        "safeLanding": "Безпечний лендинг",
        "safeLandingNone": "— немає —",
        "safeOfferNone": "— немає —",
        "noLocalOffers": "Локальних офферів ще немає — створіть його в Оферах (тип «Локальний») або зберіть у LeadForge.",
        "safeUrl": "Безпечний URL",
        "safeHtml": "Безпечний HTML (вбудований)",
        "dontRecordSafeClicks": "Не записувати кліки на Safe Page",
        "dontRecordSafeClicksHint": "Боти, модератори та парсери, що потрапили на Safe Page, не записуватимуться в базу кліків і не потраплять у звіти.",
        "moneyPage": "Грошова сторінка (для відвідувачів)",
        "moneyPageHint": "Показується реальним відвідувачам. Працює як схема лендинг+оффер.",
        "noGeoDbWarning": "Кожен відвідувач визначається як країна Unknown, тому цей фільтр відправить 100% вашого трафіку на Safe Page.",
        "diagnosticsTitle": "Останні 24год",
        "diagnosticsWindow": "Вікно: {from} — {to} ({tz})",
        "diagnosticsEmpty": "Поки немає кліків",
        "diagnosticsStats": "{hits} хітів  →  {money} гроші  ·  {safe} safe",
        "diagnosticsTopReasons": "Топ причин:",
        "diagnosticsReasonItem": "{reason} ×{count}",
        "diagnosticsViewLogs": "Дивитись у логу кліків",
        "almostAllSafeWarning": " майже весь трафик направляється на безпечну сторінку. Перевірте причини нижче, перш ніж продовжувати витрачати бюджет на цю кампанію.",
        "inactiveDatacenterWarning": "Визначення датацентрів неактивне: не встановлено базу ASN.",
        "inactiveVpnWarning": "Визначення VPN/проксі неактивне: не встановлено базу IP2Proxy PX12.",
        "inactiveBotWarning": "Чорний список ботів неактивний: не встановлено базу ASN.",
        "inactiveIspWarning": "Блокування ISP неактивне: не встановлено базу ASN.",
        "logSafeClicks": "Записувати кліки на Safe Page",
        "logSafeClicksHint": "Кліки на Safe Page записуються в лог кліків з is_safe_page=1. Зніміть прапорець, щоб не зберігати їх у базі даних.",
        "excludeSafeFromReports": "Виключити кліки Safe Page зі звітів",
        "excludeSafeFromReportsHint": "Кліки на Safe Page залишаються видні в лозі кліків, але виключаються з Візитів і всього похідного — унікальних, ботів, порожніх реферерів, витрат і CPV. У Кліки вони не потрапляли ніколи (safe-хіт не доходить до офера), тому Кліки, CPC і CR не змінюються."
    },
    "postbackQueue": {
        "title": "Черга постбеків",
        "description": "Вихідні S2S-постбеки потрапляють у чергу й доставляються фоновим воркером із повторами. Без встановленого крона черга не розбирається і постбеки не надсилаються.",
        "lastRun": "Останній запуск (доставлено / оброблено)",
        "cronHint": "Воркер запускається щохвилини. Розклад повторів: 60с → 5х → 30х → 2г → 24г, після чого рядок позначається як невдалий.",
        "status": "Статус",
        "statusPending": "У черзі",
        "statusInFlight": "У доставці",
        "statusDelivered": "Доставлено",
        "statusFailed": "Помилка",
        "attempts": "Спроби",
        "nextRetry": "Наступна спроба",
        "lastError": "Остання помилка",
        "httpCode": "HTTP",
        "installCron": "Встановити cron доставки",
        "removeCron": "Прибрати cron доставки",
        "cronInstalled": "Cron доставки постбеків встановлено (запуск щохвилини).",
        "cronRemoved": "Cron доставки постбеків видалено.",
        "queueHealthy": "Воркер доставки працює.",
        "queueStale": "Воркер доставки давно не відзначався. Встановіть cron, щоб увімкнути повтори."
    },
    "cloakReasons": {
        "geo_country": "Країна відвідувача не входить до списку дозволених для потоку.",
        "geo_unknown": "Країну не вдалося визначити — не встановлено geo-базу або IP не розпізнано.",
        "device_type": "Тип пристрою відвідувача не входить до списку дозволених для потоку.",
        "isp_blocklist": "ISP відвідувача збігся з чорним списком ботів/датацентрів.",
        "crawler_or_tool_ua": "User-agent збігся з відомим краулером, ботом або налагоджувальним інструментом.",
        "suspicious_ua": "User-Agent exhibits bot-like characteristics.",
        "ip2proxy_vpn_proxy": "IP2Proxy detected this as a VPN or proxy address.",
        "ip2proxy_datacenter": "IP2Proxy detected this as a datacenter IP.",
        "ip2proxy_residential": "IP2Proxy виявив це як residential proxy.",
        "ip2proxy_tor": "IP2Proxy виявив це як Tor exit node.",
        "asn_datacenter": "ASN збігся з відомим датацентром або хостинг-провайдером.",
        "webdriver": "Browser automation detected via JavaScript challenge.",
        "js_missing": "JavaScript did not execute.",
        "js_fingerprint": "Browser fingerprint challenge failed.",
        "bot_blocklist": "IP збігся з відомим чорним списком ботів.",
        "bot_isp": "ISP відвідувача знайдено у списку ботів/датацентрів.",
        "no_user_agent": "Запит без User-Agent — так не робить жоден реальний браузер.",
        "missing_accept_language": "Запит без заголовка Accept-Language — нетипово для реальних браузерів.",
        "ip2proxy_bot": "IP2Proxy позначив адресу як відомого краулера/павука (SES/AIC).",
        "ip2proxy_threat": "IP2Proxy позначив адресу як відому загрозу.",
        "ip2proxy_high_fraud": "Fraud-скор IP2Proxy для адреси — 80 і вище.",
        "datacenter_asn": "ASN належить відомому датацентру або хостингу.",
        "vpn_proxy_asn": "ASN належить відомому VPN або проксі-провайдеру.",
        "iprange_datacenter": "IP потрапляє в опублікований діапазон датацентру/краулера.",
        "hosting_isp": "У назві ISP/організації знайдено ключове слово хостингу."
    },
    "extCosts": {
        "title": "Dolphin / FBTool API",
        "description": "Приймання витрат із Dolphin та Fbtool.pro через Orbitra Automation API endpoint",
        "step1": "1. Створіть API-ключ із правами write: Користувачі → ваш профіль → згенерувати ключ (permissions: write)",
        "step2": "2. Скопіюйте в Dolphin (Налаштування → Експорт витрат → Tracker API) або Fbtool (Витрати → Tracker API):",
        "step3": "3. Endpoint, який викликають сервіси (сумісний із Keitaro Admin API v1):",
        "hint": "Фільтри матчаться за параметрами кліку: sub_id_4 = ad_id, sub_id_3 = adset_id (дефолти шаблону Facebook), або будь-яке ім’я параметра напряму (ad_id, adset_id, campaign_id...). Витрата ділиться порівну між кліками, що збіглися, за період; повторне надсилання перезаписує, а не сумує."
    },
    "tracking": {
        "saveFirst": "Спочатку збережіть кампанію — код підключення вбудовує токен кампанії.",
        "method": "Спосіб підключення",
        "groupSite": "Сайти",
        "groupBanners": "Банери",
        "groupAds": "Рекламні мережі",
        "groupDomains": "Домени та SSL",
        "groupMisc": "Інструменти",
        "trackingScript": "Скрипт відстеження",
        "bannerScript": "Блок банерів (script)",
        "bannerIframe": "Блок банерів (iframe)",
        "base64": "Base64 (сховати від блокувальників)",
        "hours": "Тривалість, годин",
        "offerUrl": "URL оффера",
        "trapUrl": "URL пастки",
        "heading": "Заголовок",
        "downloadKclient": "Завантажити kclient.php",
        "settings": "Налаштування та опції",
        "generatedCode": "Згенерований код інтеграції",
        "howToInstall": "Як встановити",
        "copyCode": "Копіювати код",
        "copied": "Скопійовано!",
        "previewWidget": "Передперегляд",
        "hidePreview": "Сховати передперегляд",
        "minutes": "Тривалість, хвилини",
        "headerText": "Текст заголовка",
        "ctaButtonText": "Текст кнопки",
        "theme": "Стиль",
        "themePurple": "Фіолетовий / Індиго",
        "themeEmerald": "Смарагдовий",
        "themeFire": "Вогняно-червоний",
        "themeDark": "Темний мінімалізм",
        "expireAction": "Після закінчення",
        "expireShowBadge": "Показати «EXPIRED»",
        "expireRedirect": "Редирект на резервний URL",
        "expireUrl": "Резервний URL (порожньо = URL оффера)",
        "logClick": "Реєструвати клік у трекері (sub1=back_button)",
        "activationDelay": "Затримка активації, сек",
        "description": "Опис",
        "popupButtonText": "Текст кнопки",
        "buttonColor": "Колір кнопки",
        "colorGreen": "Зелений",
        "colorBlue": "Синій",
        "colorCoral": "Кораловий",
        "showDelay": "Затримка показу, сек",
        "closeOnBackdrop": "Закривати при кліку повз вікно",
        "phpStreamNote": "Що отримає відвідувач — локальна сторінка, редирект чи біла сторінка — вирішують потоки кампанії, а не цей сніпет.",
        "phpSecondaryTitle": "Код для вторинних сторінок (необов'язково)",
        "phpSecondaryHint": "Для сторінок, куди лендинг веде відвідувача далі: клік і далі трекається замість реєстрації нового.",
        "phpOfferLinkTitle": "Як поставити посилання на оффер",
        "phpOfferLinkHint": "getOffer(42) бере конкретний оффер потоку замість того, який обрав трекер.",
        "phpAdvanced": "Отримати посилання на оффер у змінну",
        "phpAdvancedHint": "getOffer() віддає URL оффера, не даючи трекеру нічого зробити зі сторінкою — для кнопки, яку ви малюєте самі.",
        "phpDocs": "Tracking Client (PHP) — повна документація",
        "sendParams": "Пробрасовувати UTM / SubID з адресного рядка",
        "pixelType": "Тип пікселя",
        "pixelClick": "Кліки (email / банери)",
        "pixelConversion": "Конверсія (сторінка «Дякую»)",
        "convStatus": "Статус конверсії",
        "payoutValue": "Виплата (опціонально)",
        "subidParam": "Параметр SubID",
        "subidTemplate": "{subid} — підставляє ваша платформа розсилок/CRM",
        "clickidTemplate": "{clickid} — id кліку, що підставляється",
        "noOptions": "У цього методу немає додаткових опцій — код праворуч готовий до використання.",
        "instKclientJs": "Перший блок <script> додайте в <head> кожної сторінки сайту: kclient.js зареєструє клік і виконає правила потоку.",
        "instKclientPhp": "Вставте в найперші рядки index.php сайту, до будь-якого виводу. kclient.php завантажте і покладіть поруч з index.php.",
        "instTrackingScript": "Блок конфігурації — в <head> кожної сторінки; частину для конверсії — лише на сторінці «Дякую».",
        "instBanner": "Вставте там, де має з'являтися блок банерів.",
        "instCampaignUrl": "Використовуйте цей URL безпосередньо в налаштуваннях рекламної кампанії мережі.",
        "instAdSnippet": "Вставте сніпет в HTML креативу або лендінгу там, де потрібне посилання/фрейм.",
        "instPixel": "Піксель кліку — в HTML-код листа або банера. Піксель конверсії — на сторінці «Дякую» після форми.",
        "instWidgets": "Вставте код в HTML лендінгу перед закриваючим тегом </body>.",
        "instWordpress": "Встановіть плагін Orbitra і вставляйте шорткоди в записи, сторінки або сторінку підтвердження CF7."
    },
    "costSync": {
        "title": "Синк витрат",
        "noClicks": "Кліків за останні 7 днів немає — витрати поки не до чого привʼязувати.",
        "matchOk": "Останні 7 днів",
        "matchWarn": "У кліків за 7 днів немає параметрів ad_id / adset_id / campaign_id — імпортовані витрати не привʼяжуться. Виберіть джерело трафіку (наприклад Facebook) у налаштуваннях кампанії, щоб параметри URL дійшли до трекера.",
        "source": "Джерело",
        "noConnections": "Підключень витрат ще немає — створіть на сторінці Інтеграції (Facebook Costs / TikTok Ads) або в Агрегаторах.",
        "paused": "пауза",
        "lastSync": "синк",
        "syncNow": "Синк зараз",
        "synced": "Синхронізовано",
        "pushHint": "URL пушу витрат для цієї кампанії (API-ключ: сторінка Користувачі, права write)"
    },
    "cloudflare": {
        "accounts": "Акаунти",
        "addAccount": "Додати акаунт",
        "editAccount": "Редагувати акаунт",
        "accountLabel": "Назва акаунта",
        "accountLabelPlaceholder": "Buyer 1 — Main CF",
        "zonesCount": "зон",
        "importZones": "Import & Auto-DNS — підтягнути зони акаунта й припаркувати їх у трекер з A-записом",
        "importTitle": "Import & Auto-DNS",
        "importHint": "Зони акаунта Cloudflare. Відмічені додаються в трекер: A-запис на IP сервера прописується автоматично, проксовані отримують SSL на краї CF.",
        "inTracker": "вже в трекері",
        "noZones": "В акаунті немає зон",
        "selectedCount": "вибрано",
        "importBtn": "Імпортувати",
        "importing": "Імпортуємо…",
        "importedCount": "Доменів додано",
        "parkedOk": "A-записи прописані",
        "duplicatesSkipped": "вже були в трекері",
        "repoint": "Перепрописати A-записи доменів цього акаунта на IP сервера",
        "repointConfirm": "Переписати A-записи всіх доменів, чиї зони є в цьому акаунті Cloudflare, на поточний IP сервера?",
        "deleteConfirm": "Видалити акаунт із трекера? Домени залишаться, але їхні A-записи більше не оновлюватимуться через цей акаунт.",
        "noAccounts": "Немає підключених акаунтів — додайте перший, щоб паркувати зони в один клік.",
        "testAndSave": "Перевірити й зберегти",
        "title": "Cloudflare",
        "description": "Керування DNS доменів трекера через Cloudflare API: A-записи проставляються самі, SSL — краєм CF",
        "howTo": "Створіть токен: Cloudflare → My Profile → API Tokens → Create Token → шаблон «Edit zone DNS» → Permissions: Zone·DNS·Edit + Zone·Zone·Edit → Zone Resources: All zones. Домени, чия зона є в акаунті, при паркуванні отримують A-запис автоматично; при ввімкненому проксі SSL видається краєм Cloudflare миттєво.",
        "tokenSaved": "токен збережено — введіть новий, щоб замінити",
        "serverIp": "IP сервера (A-запис)",
        "sslMode": "SSL режим зони",
        "sslFlexible": "SSL одразу, сервер по HTTP",
        "sslFull": "потрібен сертифікат на сервері",
        "proxied": "помаранчова хмара (SSL від CF)",
        "saved": "Збережено",
        "test": "Перевірити",
        "syncAll": "Перепаркувати всі домени",
        "syncAllConfirm": "Переписати A-записи всіх доменів, чиї зони є в Cloudflare, на поточний IP сервера?",
        "syncedCount": "перепарковано",
        "zonesAvailable": "зон в акаунті",
        "managedDomains": "Доменів під керуванням Cloudflare"
    },
    "namecheap": {
        "whitelistError": "Помилка Namecheap API: IP сервера ({ip}) не додано до білого списку вашого акаунта Namecheap. Додайте його в Profile → Tools → Business & Dev Tools → Namecheap API Access → Whitelisted IPs.",
        "title": "Namecheap",
        "description": "Автопаркування DNS доменів з акаунта Namecheap, купівля нових доменів і SSL Let's Encrypt — прямо з трекера",
        "howTo": "Увімкніть API-доступ: Namecheap → Profile → Tools → Business & Dev Tools → Namecheap API Access → Manage → Toggle API Access і додайте вихідну IP сервера до Whitelisted IPs. Домени, зареєстровані в акаунті, при паркуванні отримують A-запис автоматично; сертифікат Let's Encrypt випускається, щойно DNS оновиться.",
        "username": "Username",
        "keySaved": "ключ збережено — введіть новий, щоб замінити",
        "whitelistIp": "IP сервера для Whitelisted IPs",
        "whitelistHint": "Додайте цю IP у Namecheap: Profile → Tools → Business & Dev Tools → Namecheap API Access → Manage → Whitelisted IPs. Якщо показана хибна адреса — не страшно: при першій спробі підключення Namecheap сам назве IP, з якої прийшли запити.",
        "addressBook": "Профіль контакту (Address Book) для нових доменів",
        "addressPlaceholder": "— не обрано —",
        "addressRefresh": "Оновити список з Address Book",
        "addressHint": "Профілі підтягуються з Namecheap Address Book — вони потрібні для реєстрації нових доменів.",
        "addressesLoaded": "профілі завантажено",
        "environment": "Середовище",
        "production": "Production",
        "sandbox": "Sandbox",
        "saved": "Збережено",
        "test": "Перевірити",
        "connected": "Підключено",
        "balance": "баланс",
        "errConnection": "Namecheap відхилив підключення",
        "registerBtn": "Register Domain",
        "registerHint": "Купити домен через баланс Namecheap і припаркувати його сюди одним кліком",
        "registerHintLong": "Домен реєструється через баланс вашого акаунта Namecheap: одразу після покупки A-запис вказує на цей сервер, а SSL Let's Encrypt випускається автоматично.",
        "registerTitle": "Register Domain",
        "checkBtn": "Check Availability",
        "available": "вільний",
        "taken": "зайнятий",
        "premium": "Premium-домен",
        "price": "Ціна",
        "priceEstimate": "Орієнтовна вартість реєстрації на 1 рік. Остаточну суму визначає Namecheap.",
        "priceUnavailable": "Ціна недоступна. Перед купівлею перевірте доступність ще раз.",
        "buyPark": "Buy & Park Domain",
        "buying": "Купуємо…",
        "buyConfirm": "Купити і припаркувати домен",
        "registeredOk": "Домен зареєстровано",
        "parkedOk": "DNS налаштовано автоматично через Namecheap API",
        "importBtn": "Import from Namecheap",
        "importBtn2": "Імпортувати",
        "importHint": "Обрати домени з акаунта Namecheap і додати їх у трекер",
        "importHintLong": "Усі домени акаунта Namecheap. Позначені додаються в трекер: A-запис і SSL налаштовуються автоматично.",
        "importTitle": "Import from Namecheap",
        "importing": "Імпортуємо…",
        "importedCount": "Доменів додано",
        "inTracker": "вже в трекері",
        "noDomains": "В акаунті немає доменів",
        "accounts": "Аккаунти Namecheap",
        "addAccount": "Додати аккаунт Namecheap",
        "editAccount": "Редагувати аккаунт",
        "accountLabel": "Назва аккаунта",
        "accountLabelPlaceholder": "Buyer 1 — основний",
        "testAndSave": "Перевірити й зберегти",
        "errUserKeyRequired": "Вкажіть username та API key",
        "noAccounts": "Аккаунтів ще немає — додайте перший, щоб паркувати й купувати домени прямо з трекера.",
        "domainsInAccount": "Доменів в аккаунті",
        "buyDomain": "Купити домен",
        "importPark": "Імпорт і парковка",
        "refreshBalance": "Оновити баланс",
        "chooseAccount": "Аккаунт Namecheap",
        "deleteConfirm": "Видалити аккаунт",
        "addressHintNew": "Профілі підтягуються з Address Book цього аккаунта — вони потрібні для реєстрації нових доменів."
    },
    "ipranges": {
        "title": "IP-діапазони датацентрів (клокінг)",
        "description": "Списки хмар і краулерів (AWS, Google, Meta, OpenAI…) оновлюються щодня; клокер позначає відвідувачів із цих IP як датацентрових.",
        "fresh": "Списки актуальні",
        "stale": "Спискам більше доби — оновляться фоном при першому клок-кліку або кнопкою",
        "notDownloaded": "Списки ще не завантажені — завантажаться фоном при першому клок-кліку або кнопкою",
        "updateNow": "Оновити зараз",
        "updated": "Діапазони оновлено"
    },
    "costImport": {
        "facebookAds": "Facebook Ads",
        "googleAds": "Google Ads",
        "tiktokAds": "TikTok Ads",
        "description": "Імпортує денну рекламну витрату і атрибутує її до кліків за рекламними ID, які вже ловлять шаблони джерел трафіку.",
        "fields": {
            "ttToken": "Access Token (Marketing API — права Ads Management / Reporting; токен з Events Manager не підійде)",
            "ttAdvertiser": "Advertiser ID (ID кабінету)",
            "ttAppId": "App ID (необов'язково, потрібен для авто-поновлення OAuth-токена)",
            "ttAppSecret": "App Secret (необов’язково)",
            "fbToken": "Access Token (long-lived / system user)",
            "fbAdAccount": "ID рекламного кабінету",
            "fbApiVersion": "Версія Facebook API",
            "fbAppId": "App ID (опційно, для деяких токенів)",
            "fbAppSecret": "App Secret (опційно)",
            "proxy": "Проксі (опційно) — scheme://user:pass@host:port",
            "gaLoginCustomerId": "Login Customer ID (MCC, опційно, без дефісів)"
        }
    },
    "tiktokCosts": {
        "modalTitle": "TikTok For Business",
        "oneClickTitle": "1-Click інтеграція з TikTok",
        "oneClickDesc": "Увійдіть в акаунт TikTok For Business — Orbitra автоматично знайде рекламні кабінети та пікселі, увімкне синхронізацію витрат та імпортує пікселі в Pixel Vault.",
        "loginWithTikTok": "Увійти через TikTok",
        "oauthConnecting": "Підключення до TikTok…",
        "selectAccounts": "Виберіть кабінети для підключення",
        "discoveredPixels": "Знайдені пікселі",
        "importPixels": "Імпортувати знайдені пікселі в Pixel Vault",
        "syncEvery": "Оновлювати витрати кожні",
        "connectSelected": "Зберегти та підключити",
        "connecting": "Підключення кабінетів…",
        "connectedAccounts": "Підключено кабінетів TikTok: {n}, імпортовано пікселів: {m}.",
        "oauthFailed": "Не вдалося підключити TikTok. Спробуйте ще раз.",
        "popupBlocked": "Вікно входу TikTok заблоковано. Дозвольте спливаючі вікна для Orbitra та спробуйте ще раз.",
        "noDiscoveredAccounts": "Вхід у TikTok виконано, але доступних рекламних кабінетів не знайдено.",
        "connectTikTok": "Підключити TikTok For Business",
        "connectTikTokHint": "Вхід через TikTok — авто-виявлення кабінетів і пікселів, імпорт у Pixel Vault та автоматична синхронізація витрат"
,
        "title": "TikTok Costs",
        "description": "Імпортує витрати з рекламних кабінетів TikTok і прив'язує їх до кліків за Campaign ID, Adset ID та Ad ID.",
        "addAccount": "Додати аккаунт TikTok",
        "editAccount": "Редагувати аккаунт",
        "findAccount": "Пошук за назвою або Advertiser ID",
        "name": "Назва аккаунта",
        "namePlaceholder": "Назва кабінету / аккаунта",
        "interval": "Оновлювати витрати кожні",
        "manualSection": "або підключіть вручну",
        "testConnection": "Перевірити підключення",
        "testing": "Перевіряємо…",
        "saved": "Аккаунт збережено",
        "cloned": "Аккаунт клоновано — копія на паузі, доки ви її не увімкнете",
        "deleted": "Аккаунт видалено",
        "confirmDelete": "Видалити цей аккаунт? Імпортовані витрати залишаться у звітах.",
        "syncNow": "Оновити витрати",
        "syncedOk": "Синхронізовано: отримано записів — {n}, прив’язано до кліків — {m}.",
        "syncedNoMatch": "Отримано записів: {n}, але прив’язати не вдалося жодного — перевірте, що посилання кампанії передає ad_id / adset_id / campaign_id.",
        "syncFailed": "Синхронізація не вдалася",
        "nextUpdate": "Наступне оновлення",
        "lastSync": "Остання синхронізація",
        "paused": "Зупинено",
        "pause": "Зупинити",
        "resume": "Відновити",
        "clone": "Клонувати",
        "onNextCron": "При наступному запуску cron",
        "due": "Час оновлювати",
        "error": "Помилка",
        "ok": "Працює",
        "neverSynced": "Ще не синхронізовано",
        "noAccounts": "Кабінети TikTok не підключено. Додайте аккаунт, щоб імпортувати витрати з TikTok.",
        "oauthNotConfiguredTitle": "1-Click OAuth потребує ключі застосунку",
        "oauthNotConfiguredHint": "Для авторизації через кнопку 1-Click необхідно вказати App ID та App Secret застосунку TikTok for Business. Якщо у вас немає застосунку, використовуйте пряме підключення за токеном нижче (без створення додатку).",
        "tokenHowTo": "Інструкція з підключення TikTok та ключів",
        "tokenHowToHint": "📌 Спосіб 1: Пряме підключення за токеном (Рекомендовано — без створення додатку):\n1. Увійдіть у TikTok for Business (ads.tiktok.com) → відкрийте потрібний рекламний кабінет.\n2. Скопіюйте Advertiser ID (числовий ID у верхньому меню перемикання акаунтів).\n3. У TikTok Business Center → розділ «Застосунки/Розробникам» або в Events Manager створіть Long-term Access Token зі скоупом Advertising Read / Reporting.\n4. Вставте Access Token та Advertiser ID у поля нижче і натисніть «Перевірити підключення» → «Зберегти».\n\n📌 Спосіб 2: 1-Click OAuth (автоматичний пошук усіх кабінетів і пікселів):\n1. У TikTok for Business Developer Portal (business-api.tiktok.com) створіть застосунок.\n2. У полі Redirect URI вкажіть точну адресу трекера (потрібен HTTPS):\n{redirect_uri}\n3. Вкажіть App ID та App Secret у формі нижче (або у змінних середовища ORBITRA_TIKTOK_APP_ID та ORBITRA_TIKTOK_APP_SECRET).\n4. Після збереження кнопка 1-Click зможе в один клік відкривати вікно входу та масово підключати кабінети.",
        "redirectUriLabel": "Authorized Redirect URI для TikTok Developer Portal:",
        "switchToManual": "Перейти до прямого введення токена (без додатку)",
        "switchTo1Click": "Перейти до 1-Click OAuth",
        "httpsNotice": "💡 Зверніть увагу: 1-Click OAuth вимагає HTTPS-домену. При роботі за прямою IP-адресою використовуйте пряме підключення за Access Token."
    },
    "googleAdsCosts": {
        "title": "Google Ads Costs",
        "description": "Імпорт витрат із Google Ads та клієнтських кабінетів MCC. Атрибуція витрат до кліків за ValueTrack {campaignid} та {creative}.",
        "oneClickTitle": "1-Click підключення Google Ads",
        "oneClickDesc": "Увійдіть через Google — Orbitra знайде прямі аккаунти та дочірні кабінети MCC і налаштує синхронізацію витрат.",
        "loginWithGoogle": "Увійти через Google",
        "oauthConnecting": "Підключаємося до Google…",
        "selectAccounts": "Виберіть рекламні аккаунти або групи MCC для підключення",
        "directAccounts": "Прямі аккаунти",
        "mccManager": "Керуючий аккаунт (MCC)",
        "selectAllInMcc": "Вибрати всі",
        "syncEvery": "Оновлювати витрати кожні",
        "connectSelected": "Підключити вибрані аккаунти",
        "connecting": "Підключаємо аккаунти…",
        "connectedAccounts": "Підключено аккаунтів Google Ads: {n}.",
        "oauthFailed": "Не вдалося підключити Google Ads. Спробуйте ще раз.",
        "popupBlocked": "Вікно входу Google заблоковано. Дозвольте спливаючі вікна для Orbitra і спробуйте ще раз.",
        "oauthNotConfiguredTitle": "1-Click OAuth потребує ключі Google Cloud та MCC",
        "oauthNotConfiguredHint": "Для роботи кнопки 1-Click потрібні Developer Token (з MCC) та OAuth Client ID + Secret (з Google Cloud Console). Якщо їх немає, ви можете підключитися напряму за готовим Refresh Token нижче.",
        "howTo": "Інструкція з ключів та налаштування Google Ads",
        "howToHint": "📌 Спосіб 1: Пряме підключення за токенами (Ручне введення):\n1. Customer ID: 10-значний номер вашого кабінету у правому верхньому кутку Google Ads (формат 123-456-7890).\n2. Developer Token: у керуючому акаунті Google Ads MCC → Інструменти та налаштування → Центр API (скопіюйте токен).\n3. Client ID та Client Secret: у Google Cloud Console (console.cloud.google.com) → APIs & Services → Credentials → Створити OAuth 2.0 (тип: Веб-застосунок).\n4. Refresh Token: отриманий OAuth-токен доступу (через OAuth Playground або майстер авторизації).\n5. Вставте ключі у поля нижче, натисніть «Перевірити підключення» та «Зберегти».\n\n📌 Спосіб 2: 1-Click OAuth (авто-пошук прямих та MCC-кабінетів):\n1. У Google Cloud Console при створенні OAuth Client ID додайте в Authorized Redirect URIs точну адресу (потрібен HTTPS):\n{redirect_uri}\n2. Збережіть Developer Token, Client ID та Client Secret на сервері (у .env або у формі нижче).\n3. Кнопка 1-Click стане активною і дозволить логінитися через Google в 1 клік з вибором будь-яких акаунтів.",
        "howToStep3": "💡 Підказка: При роботі трекера за прямою IP без SSL-сертифіката платформи Google та TikTok блокують OAuth-редиректи — використовуйте пряме підключення (Manual Token).",
        "redirectUriLabel": "Authorized Redirect URI для Google Cloud Console:",
        "switchToManual": "Перейти до прямого введення токена",
        "switchTo1Click": "Перейти до 1-Click OAuth",
        "httpsNotice": "💡 Зверніть увагу: 1-Click OAuth вимагає HTTPS-домену. При роботі за прямою IP-адресою використовуйте пряме підключення.",
        "setupGuideTitle": "🔧 Увімкніть 1-Click підключення Google Ads",
        "setupGuideSubtitle": "Налаштуйте OAuth-облікові дані на сервері для безшовного підключення в 1 клік. Або використовуйте пряме підключення за токенами нижче.",
        "setupStepsTitle": "📋 Швидке налаштування",
        "step1Title": "1. Створіть проект у Google Cloud",
        "step1Desc": "Перейдіть на console.cloud.google.com, створіть проект і увімкніть Google Ads API.",
        "step2Title": "2. Створіть облікові дані OAuth 2.0",
        "step2Desc": "APIs & Services → Облікові дані → Створити OAuth 2.0 Client ID (Веб-додаток)",
        "step3Title": "3. Додайте Redirect URI",
        "step3Desc": "Додайте цей URI в Authorized Redirect URIs:",
        "step4Title": "4. Отримайте Developer Token",
        "step4Desc": "Перейдіть в API Center Google Ads і подайте заявку на Developer Token.",
        "step5Title": "5. Налаштуйте змінні середовища",
        "step5Desc": "Додайте їх в оточення сервера або в .env файл:",
        "setupNote": "Після налаштування перезавантажте цю сторінку — кнопка «Увійти через Google» запрацює миттєво.",
        "useManualMode": "Використовувати ручний режим з токенами",
        "noDiscoveredAccounts": "Вхід у Google виконано, але доступних рекламних аккаунтів не знайдено.",
        "addAccount": "Додати аккаунт Google Ads",
        "editAccount": "Редагувати аккаунт",
        "findAccount": "Пошук за назвою або Customer ID",
        "name": "Назва аккаунта",
        "namePlaceholder": "Назва аккаунта",
        "interval": "Оновлювати витрати кожні",
        "manualSection": "або підключіть вручну (власний додаток)",
        "testConnection": "Перевірити підключення",
        "testing": "Перевіряємо…",
        "saved": "Аккаунт збережено",
        "cloned": "Аккаунт клоновано — копія на паузі, доки ви її не увімкнете",
        "deleted": "Аккаунт видалено",
        "confirmDelete": "Видалити цей аккаунт? Імпортовані витрати залишаться у звітах.",
        "syncNow": "Оновити витрати",
        "syncedOk": "Синхронізовано: отримано записів — {n}, прив’язано до кліків — {m}.",
        "syncedNoMatch": "Отримано записів: {n}, але прив’язати не вдалося жодного — перевірте, що посилання кампанії передає campaignid / creative.",
        "syncFailed": "Синхронізація не вдалася",
        "nextUpdate": "Наступне оновлення",
        "lastSync": "Остання синхронізація",
        "paused": "Зупинено",
        "pause": "Зупинити",
        "resume": "Відновити",
        "clone": "Клонувати",
        "onNextCron": "При наступному запуску cron",
        "due": "Час оновлювати",
        "error": "Помилка",
        "ok": "Працює",
        "neverSynced": "Ще не синхронізовано",
        "noAccounts": "Аккаунти Google Ads не підключено. Додайте аккаунт, щоб імпортувати витрати з Google Ads.",
        "mccSubAccount": "Дочірній кабінет MCC"
    },
    "fbCosts": {
        "title": "Facebook Costs",
        "description": "Імпортує витрати з рекламного кабінету Meta та прив’язує їх до кліків за ad, adset і campaign ID.",
        "oneClickTitle": "1-Click підключення Facebook",
        "oneClickDesc": "Увійдіть через профіль Facebook. Orbitra автоматично знайде всі доступні рекламні кабінети й налаштує синхронізацію витрат.",
        "loginWithFb": "Увійти через Facebook",
        "selectAccounts": "Оберіть рекламні кабінети для підключення",
        "connectSelected": "Підключити обрані кабінети",
        "oauthConnecting": "Підключення до Facebook…",
        "connectingAccounts": "Підключення кабінетів…",
        "oauthFailed": "Не вдалося підключитися до Facebook. Спробуйте ще раз.",
        "popupBlocked": "Вікно входу Facebook заблоковано. Дозвольте спливні вікна для Orbitra й повторіть спробу.",
        "noDiscoveredAccounts": "Вхід виконано, але доступних рекламних кабінетів не знайдено.",
        "connectedAccounts": "Підключено рекламних кабінетів Facebook: {n}.",
        "addAccount": "Додати акаунт",
        "editAccount": "Редагувати акаунт",
        "findAccount": "Знайти акаунт",
        "name": "Назва",
        "interval": "Оновлювати витрати кожні",
        "advanced": "Додатково: мапінг параметрів",
        "extendToken": "Продовжити на 60 днів",
        "extendTokenHint": "Обмінює токен на 60-денний довгоживучий через Graph API",
        "tokenExtended": "Токен продовжено на 60 днів",
        "customApp": "Власний Meta App (опційно)",
        "customAppHint": "Потрібно лише для продовження токена та рідкісних апп-специфічних налаштувань — спільному застосунку інстанса або готовому довгоживучому токену нічого не потрібно.",
        "oauthNotConfiguredTitle": "1-Click OAuth не налаштовано",
        "oauthNotConfiguredHint": "Задайте ORBITRA_META_APP_ID і ORBITRA_META_APP_SECRET на сервері або розкрийте «Власний Meta App» нижче та збережіть App ID / App Secret. А можна взагалі обійтися без 1-Click — підключіться вручну через Access Token у формі нижче, класичним способом.",
        "tokenHowTo": "Як отримати Access Token",
        "tokenHowToHint": "1. Відкрийте Meta for Developers → ваш застосунок або Business Settings → System Users.\n2. Створіть системного користувача з адмін-доступом до вашого Business Manager.\n3. Згенеруйте токен із правами ads_read і ads_management.\n4. Вставте його вище разом із ID рекламного кабінету (act_…).",
        "advancedHint": "Потрібно лише якщо трафік іде через застосунок, який перепаковує макроси Facebook у sub ID. Залиште порожнім, щоб використовувати стандартні ad_id / adset_id / campaign_id.",
        "ad_id_param": "Ad ID надходить у параметрі",
        "adset_id_param": "Adset ID надходить у параметрі",
        "campaign_id_param": "Campaign ID надходить у параметрі",
        "defaultVersion": "За замовчуванням (найновіша)",
        "testConnection": "Перевірити підключення",
        "testing": "Перевіряємо…",
        "saved": "Акаунт збережено",
        "cloned": "Акаунт склоновано — копія на паузі, доки ви її не увімкнете",
        "deleted": "Акаунт видалено",
        "confirmDelete": "Видалити цей акаунт? Імпортовані витрати залишаться у звітах.",
        "syncNow": "Оновити витрати",
        "syncedOk": "Синхронізовано: отримано записів — {n}, прив’язано до кліків — {m}.",
        "syncedNoMatch": "Отримано записів: {n}, але прив’язати не вдалося жодного — перевірте, що посилання кампанії передає ad_id / adset_id / campaign_id.",
        "syncFailed": "Синхронізація не вдалася",
        "nextUpdate": "Наступне оновлення",
        "lastSync": "Остання синхронізація",
        "paused": "Зупинено",
        "pause": "Зупинити",
        "resume": "Відновити",
        "clone": "Клонувати",
        "onNextCron": "На наступному запуску cron",
        "due": "Час оновити",
        "error": "Помилка",
        "ok": "Працює",
        "neverSynced": "Ще не синхронізовано",
        "noAccounts": "Рекламні кабінети не підключені. Додайте акаунт, щоб імпортувати витрати з Facebook."
    },
    "fbConv": {
        "selectPixelPrompt": "Оберіть збережений піксель...",
        "manageInVault": "Редагувати в Pixel Vault",
        "addNewPixel": "Додати новий піксель у базу...",
        "enterManually": "Ввести Pixel ID вручну...",
        "selectFromList": "Обрати зі збережених",
        "tokenAutoFilledHint": "Токен підставляється автоматично при виборі пікселя.",
        "title": "Facebook Conversions",
        "description": "Надсилає конверсії в Meta через Conversions API з сервера — події, заблоковані в браузері, все одно доходять до оптимізатора.",
        "addAccount": "Додати акаунт",
        "editAccount": "Редагувати акаунт",
        "findAccount": "Пошук за кампанією або пікселем",
        "campaign": "Кампанія",
        "pixelId": "Pixel ID",
        "token": "Токен конверсій",
        "tokenHint": "Без токена піксель залишається лише браузерним. Events Manager → ваш піксель → Settings → Згенерувати маркер доступу.",
        "mapping": "Статус → подія Meta",
        "doNotSend": "Не надсилати",
        "testEventCode": "Test event code",
        "proxy": "Проксі (опційно)",
        "sendTestEvent": "Надіслати тестову подію",
        "sending": "Надсилання…",
        "syntheticClick": "(свіжого кліку з fbclid немає — використано синтетичний)",
        "saved": "Інтеграцію збережено",
        "deleted": "Інтеграцію видалено",
        "confirmDelete": "Видалити цю інтеграцію?",
        "paused": "Зупинено",
        "pause": "Зупинити",
        "resume": "Відновити",
        "clone": "Клонувати",
        "editMapping": "Mapping",
        "serverSideOn": "Server-side увімкнено",
        "browserOnly": "Лише браузерний піксель",
        "defaultMapping": "за замовчуванням",
        "noAccounts": "Жодна кампанія поки не надсилає конверсії в Meta."
    },
    "reportCustomizer": {
        "columnsSelector": "Колонки",
        "restoreDefault": "Відновити стандартні",
        "resetColumnWidths": "Скинути ширину колонок",
        "dimCity": "Місто",
        "dimRegion": "Регіон",
        "dimHour": "Година",
        "dimCreative": "Креатив",
        "dimExternal": "External ID",
        "groupByHint": "До 5 рівнів групування; виміри «URL-параметр» беруться з того, що захоплює джерело трафіку.",
        "filtersHint": "Звужуйте звіт до групування — умови застосовуються до сирих кліків.",
        "addFilter": "Додати фільтр",
        "noFilters": "Фільтри не задані. Натисніть «+ Додати фільтр», щоб додати умову.",
        "fCity": "Місто",
        "opEq": "Дорівнює (=)",
        "opNeq": "Не дорівнює (!=)",
        "opContains": "Містить",
        "opNotContains": "Не містить",
        "fValue": "Значення…",
        "title": "Налаштування колонок звіту",
        "campaignColumnsTitle": "Налаштування колонок кампаній",
        "headerSubtitle": "Налаштуйте відображення колонок, оберіть шаблон або перетягніть для зміни порядку",
        "presets": "Пресети",
        "saveGroupTemplate": "Зберегти як шаблон",
        "groupTemplatePlaceholder": "наприклад, Країна → Місто → ISP",
        "presetCod": "COD (Оплата при отриманні)",
        "presetLanderToOffer": "Лендінг → Оффер (Lander to Offer)",
        "presetBest": "Кращі метрики (Best)",
        "presetFinance": "Фінанси та ROI (Finance)",
        "presetCustom": "Користувацький (Custom)",
        "presetAll": "Усі колонки",
        "searchMetrics": "Пошук колонок...",
        "selectAll": "Вибрати всі",
        "deselectAll": "Зняти всі",
        "selected": "Вибрано",
        "columns": "Колонки",
        "metrics": "Метрики",
        "filters": "Фільтри",
        "groupBy": "Групування",
        "addDimension": "+ Розріз",
        "addUrlParam": "URL Param",
        "urlParamPrompt": "Введіть ім'я URL параметра (наприклад, adset_id, utm_source, custom_id):",
        "dragReorderHint": "Перетягуйте елементи або заголовки таблиці для зміни порядку",
        "catTraffic": "Трафік",
        "catConversions": "Конверсії та Статуси",
        "catFinancial": "Фінанси",
        "catRates": "Коефіцієнти та Метрики",
        "apply": "Застосувати",
        "saveAsPreset": "Зберегти як пресет",
        "reset": "Скинути",
        "colClicks": "Кліки",
        "colUniqueClicks": "Унік. кліки",
        "colUcRate": "U/C (%)",
        "colPrelanderClicks": "Кліки на преленд",
        "colOfferClicks": "Кліки на оффер",
        "colLpCtr": "LP CTR (%)",
        "colConversions": "Конверсії (CV)",
        "colPurchases": "Продажі (Confirmed)",
        "colHolds": "Холд (Pending)",
        "colRejected": "Відхилено (Rejected)",
        "colTrash": "Треш (Trash)",
        "colCrAll": "CR (Всі) (%)",
        "colCrSales": "CR (Продажі) (%)",
        "colCrHolds": "CR (Холд) (%)",
        "colApproveRate": "Approve Rate (%)",
        "colApproveRateExclTrash": "Approve (без трешу) (%)",
        "colCost": "Витрати (Cost)",
        "colRevenue": "Дохід (Revenue)",
        "colRevenueConfirmed": "Дохід підтверджений",
        "colRevenueHold": "Дохід у холді",
        "colRevenueRejected": "Дохід відхилений",
        "colRevenueTrash": "Дохід треш",
        "colProfit": "Прибуток (Profit)",
        "colRoi": "ROI (%)",
        "colRealRevenue": "Real Revenue",
        "colRealProfit": "Real Profit",
        "colRealRoi": "Real ROI (%)",
        "colEpc": "EPC",
        "colUepc": "uEPC",
        "colCpc": "CPC",
        "colUcpc": "uCPC",
        "colCpa": "CPA",
        "colEarningsPerConv": "Дохід на конверсію",
        "presetTraffic": "Трафік",
        "saveTemplate": "Зберегти як шаблон",
        "templateNamePlaceholder": "напр. COD Nutra Split",
        "setAsDefault": "Зробити шаблоном за замовчуванням",
        "updateTemplate": "Зберегти зміни у \"{name}\"",
        "deleteTemplateConfirm": "Видалити шаблон \"{name}\"?",
        "renameTemplatePrompt": "Назва шаблону:",
        "makeDefault": "Зробити за замовчуванням",
        "defaultTemplateActive": "Шаблон за замовчуванням — натисніть, щоб прибрати",
        "makeDefaultGroup": "Зробити групуванням за замовчуванням",
        "defaultGroupActive": "Групування за замовчуванням — натисніть, щоб прибрати"
    },
    "dateRangePicker": {
        "today": "Сьогодні",
        "yesterday": "Вчора",
        "thisWeek": "Поточний тиждень",
        "last7Days": "Останні 7 днів",
        "thisMonth": "Поточний місяць",
        "last30Days": "Останні 30 днів",
        "previousMonth": "Минулий місяць",
        "custom": "Довільний період",
        "apply": "Застосувати",
        "cancel": "Скасувати",
        "timezone": "Часовий пояс",
        "savedTimezone": "Збережено як часовий пояс за замовчуванням",
        "startDate": "Дата початку",
        "endDate": "Дата кінця",
        "days": ["Нд", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"],
        "months": ["Січень", "Лютий", "Березень", "Квітень", "Травень", "Червень", "Липень", "Серпень", "Вересень", "Жовтень", "Листопад", "Грудень"]
    },
    "streamRefine": {
        "directMode": "Тип призначення",
        "trackerOffer": "Оффер з трекера",
        "directUrl": "Прямий URL",
        "directUrlPlaceholder": "https://affiliate-offer.com/?subid={subid}&clickid={clickid}",
        "directUrlHelp": "Трафік перенаправляється напряму на вказане посилання без необхідності створення оффера в каталозі. Підтримуються макроси {subid}, {clickid}, {country}, {ip}, {sub_id_1}..{sub_id_30}. Кожен перехід за цим посиланням рахується як клік у звітах.",
        "detectionLayers": "Шари детекції ботів",
        "safePageTitle": "Біла сторінка (Safe Page — для ботів і модераторів)",
        "moneyPageTitle": "Сторінка оффера (Money Page — для реальних відвідувачів)",
        "tabLanding": "Лендінг з трекера",
        "tabOffer": "Місцева пропозиція",
        "tabUrl": "Зовнішній URL",
        "tabHtml": "Вбудований HTML",
        "addCostConnection": "Підключити Cost Sync",
        "updateCostsManually": "Оновити витрати вручну",
        "costSyncTitle": "Синхронізація витрат (Cost Sync)",
        "manualCostTitle": "Ручне введення витрат",
        "passthroughParams": "Додати всі трекінг-параметри",
        "passthroughHint": "Додати всі параметри Facebook та UTM для передачі на цільову адресу",
        "passthroughHelp": "Автоматично додати макроси Facebook та UTM",
    },
    "suite": {
        "leadforge": "LeadForge",
        "tracker": "Tracker",
        "crm": "CRM"
    },
    "leadforge": {
        "title": "LeadForge",
        "subtitle": "Модуль пакетної підготовки офферів та лендінгів",
        "dropzoneTitle": "Перетягніть ZIP-архіви лендінгів сюди",
        "dropzoneSub": "Підтримується від 1 до 50+ архівів одночасно",
        "browseFiles": "Вибрати архіви",
        "clearQueue": "Очистити список",
        "selectedFiles": "Архівів у черзі",
        "networkApi": "Партнерська мережа (CPA Network)",
        "builtInNetworks": "CPA-мережі",
        "myNetworks": "Мої партнерки",
        "suggestAdapter": "Схоже, для цієї мережі краще підходить вбудований адаптер {name}, ніж універсальний custom-запит.",
        "apiKey": "API-ключ / Client Token",
        "offerId": "ID оффера / Flow Token",
        "targetGeo": "Цільове ГЕО / Країна",
        "geoEurope": "Європа",
        "geoAmericas": "Америки",
        "geoAsia": "Азія",
        "geoMena": "Близький Схід та Африка",
        "currency": "Валюта",
        "payout": "Виплата",
        "overridePayout": "Фіксована виплата",
        "autoFromPostback": "Авто (S2S постбек)",
        "phpBannerTitle": "PHP-лендінги вимкнено в налаштуваннях трекера",
        "phpBannerHint": "Компіляція order.php під CPA-мережі (Dr.Cash, Leadbit…) потребує увімкнених PHP-лендінгів — поки налаштування вимкнено, збірка кожного бандла падає з php_landings_disabled.",
        "phpEnableButton": "Увімкнути в 1 клік",
        "phpEnabledLog": "PHP-лендінги увімкнено — пересберіть бандли",
        "phpEnableDenied": "Не вдалося увімкнути PHP-лендінги: потрібна сесія адміністратора (Налаштування → General).",
        "phpDisabledHint": "PHP-лендінги вимкнено — увімкніть «Allow PHP landings» (банер вище або Налаштування → General) і пересберіть.",
        "landingGroup": "Група лендінгів",
        "destTitle": "Призначення в трекері",
        "destNone": "Не зберігати",
        "destNoneSub": "лише ZIP",
        "destLander": "Лендінг",
        "destLanderSub": "у Landings",
        "destOffer": "Лок. оффер",
        "destOfferSub": "у Offers",
        "destBoth": "Лендінг + оффер",
        "destBothSub": "зв'язка",
        "offerGroup": "Група офферів",
        "newGroup": "Нова група",
        "newGroupName": "Назва нової групи…",
        "createGroup": "Створити",
        "localOfferBadge": "Локальний оффер",
        "linkedOfferBadge": "Оффер",
        "forgeButton": "⚡ Forge & Save to Tracker",
        "forging": "⚡ Обробка та інтеграція...",
        "forgeSuccess": "Пакетну обробку лендінгів успішно завершено!",
        "optionsTitle": "Опції автопідготовки та ін'єкції",
        "optInjectMacro": "Вшити макрос {offer} у всі кнопки та посилання",
        "optInjectAdapter": "Впровадити JS-адаптер Orbitra та наскрізний ClickID",
        "optPhoneMask": "Додати маску телефону та валідатор для ГЕО",
        "optThankYou": "Згенерувати локалізовану сторінку «Дякуємо»",
        "optOrderPhp": "Згенерувати серверний order.php API-міст",
        "optSaveTracker": "Автоматично зберегти лендінги в трекер",
        "optCreateOffer": "Автоматично створити пов'язаний оффер у трекері",
        "consoleTitle": "Консоль процесу LeadForge",
        "downloadZip": "Завантажити готовий ZIP",
        "openInTracker": "Відкрити в трекері",
        "noArchives": "Архіви не вибрано. Завантажте ZIP-архіви вище для початку пакетної підготовки.",
        "subtitle2": "Компілятор лендінгів Auto / Cross / Raw із синком у CRM-сховище та Live Auto QA",
        "pendingFiles": "Очікують аналізу",
        "bundlesTitle": "Проаналізовані бандли",
        "selectAll": "Вибрати / зняти всі",
        "analyzeButton": "Analyze bundles",
        "analyzing": "Аналіз…",
        "buildButton": "Build all selected bundles",
        "building": "Збірка…",
        "errorBadge": "Помилка",
        "buildingBadge": "Збірка…",
        "notDetected": "Мережу не розпізнано",
        "formsWord": "форм(и)",
        "foreignScript": "Чужий скрипт — буде вирізано в Raw/Cross",
        "inputsLabel": "поля",
        "integrationTitle": "Інтеграція",
        "modeLabel": "Режим збірки",
        "modeAutoHint": "Detect + route: мережа-джерело визначається автоматично, бандл перезбирається під неї. Форми переносяться на order.php, впроваджується ClickID-міст.",
        "modeCrossHint": "Network swap: обробники старої мережі (order.php, send.php, api.php…) та жорстко вшиті ключі вирізаються, лендінг пересідає на обрану нижче цільову мережу.",
        "modeRawHint": "Clone patch: чужі лічильники (FB/TikTok/GA/Яндекс) та ворожі скрипти вирізаються, впроваджується ClickID-міст і макроси {offer} — серверний order.php не генерується.",
        "apiKeyPlaceholder": "Вставте API-ключ / токен CPA-мережі…",
        "apiKeyNote": "Зберігається у браузері окремо для кожної мережі.",
        "noGroup": "Без групи",
        "safetyTitle": "Захист лідів",
        "crmSync": "CRM sync",
        "crmSyncOn": "Увімк — кожен лід надсилається до CRM-сховища та локальний failsafe-лог (сировий телефон, запит/відповід мережі).",
        "crmSyncOff": "Вимк — лід іде лише до CPA-мережі; трекер отримує стандартний піксель конверсії.",
        "autoQa": "Auto QA",
        "autoQaOn": "Увімк — після збірки тестовий QA-лід проганяється по всьому ланцюжку (order.php → сховище → thank-you) і оцінюється 0–100%.",
        "autoQaOff": "Вимк — спочатку збірка, перевірка вручну.",
        "rerunQa": "Повторити Live QA",
    },
    "crm": {
        "title": "CRM — Облік лідів та замовлень",
        "subtitle": "Обробка, фільтрація та керування лідами в реальному часі",
        "totalLeads": "Всього лідів",
        "inProcess": "В обробці / Холд",
        "approved": "Підтверджені (Sale)",
        "rejected": "Відхилені",
        "trash": "Треш / Спам",
        "approvalRate": "Апрув (AR %)",
        "revenue": "Дохід",
        "searchPlaceholder": "Пошук за ім'ям, телефоном, SubID, ClickID...",
        "allStatuses": "Всі статуси",
        "newLead": "+ Створити замовлення",
        "exportCsv": "Експорт CSV",
        "leadDetails": "Деталі ліда",
        "customer": "Клієнт",
        "phone": "Телефон",
        "status": "Статус",
        "payout": "Виплата",
        "date": "Дата / Час",
        "actions": "Дії",
        "saveStatus": "Зберегти статус",
        "noLeads": "Ліди не знайдено за вибраними критеріями.",
        "title2": "CRM — замовлення та Anti-Shaving сховище",
        "subtitle2": "Кожен лід із точним номером, доставленим у мережу, сирим обміном з API та S2S-звіркою",
        "refresh": "Оновити ліди",
        "tabAll": "Усі",
        "tabQa": "QA-Test",
        "tabSuspect": "Підозра",
        "tabLost": "Втрачені",
        "suspects": "Підозри на шейв",
        "lostWord": "втрачено в дорозі",
        "qaCount": "QA-тести",
        "copySubid": "Копіювати SubID",
        "copyPhone": "Копіювати телефон",
        "copied": "Скопійовано",
        "networkCol": "Мережа",
        "phoneCol": "Телефон (сировий → E.164)",
        "inspect": "Інспекція доказів",
        "inspectorTitle": "Інспектор ліда",
        "tabRaw": "Сирі дані",
        "tabNetwork": "Транзакція мережі",
        "tabTracking": "Атрибуція трафіку",
        "fCustomer": "Ім'я клієнта",
        "fProduct": "Продукт",
        "fRawPhone": "Сирий номер (як увів клієнт)",
        "fCleanPhone": "E.164, доставлений мережі",
        "fPrice": "Ціна",
        "fPayout": "Виплата",
        "fGeo": "ГЕО",
        "fIp": "IP",
        "fCreatedAt": "Надіслано",
        "fLeadId": "Рядок сховища",
        "fUserAgent": "User Agent",
        "fNetRequest": "Запит у мережу (точнісінько що надіслали)",
        "fNetResponse": "Відповідь мережі (точнісінько що прийшло)",
        "evidenceTitle": "Пакет доказів шейву",
        "evidenceText": "Лід доставлено з валідним E.164 номером, мережа прийняла його з HTTP 200, але S2S-вердикт негативний або відсутній. Експортуйте цей рядок (CSV) і відкрийте тікет у сапорт мережі, вказавши SubID та network lead id.",
        "fSubParams": "Sub-параметри, передані кліком",
        "setStatus": "Встановити статус",
        "saving": "Збереження…",
        "campaign": "Кампанія",
        "noCampaign": "Без кампанії",
        "currency": "Валюта",
        "saveLead": "Зберегти лід",
        "loading": "Завантаження замовлень і лідів…",
        "allCampaigns": "Усі кампанії",
        "phoneNote": "Зберігається сирим і нормалізується в E.164.",
        "qaTest": "QA-Test",
        "suspectBadge": "Підозра на шейв",
        "suspectHint": "Відхилений при доведено валідному E.164 після відповіді мережі 200 — відкрийте тікет із доказами",
        "lostBadge": "Немає відповіді мережі",
        "lostHint2": "Мережа відповіла 200, але S2S-постбек не прийшов за 24 години",
        "duplicateBadge": "Дубль",
        "duplicateHint": "Той самий E.164 номер у тій самій мережі за 30 днів",
    },
    "proxy": {
        "singleString": "Одним рядком",
        "separateFields": "По полях",
        "hint": "Підтримує формати http://user:pass@ip:port та ip:port:user:pass",
        "host": "Хост / IP",
        "port": "Порт",
        "username": "Логін",
        "password": "Пароль"
    },
    "dimensions": {
        "campaign": "Кампанія",
        "stream": "Потік",
        "landing": "Лендінг",
        "offer": "Оффер",
        "source": "Джерело трафіку",
        "directNoLander": "Прямий перехід (Без лендінгу)",
        "defaultStream": "Потік за замовчуванням / Прямий",
        "deletedStream": "Видалений потік",
        "unnamedStream": "Потік без назви",
        "country": "Країна",
        "region": "Регіон",
        "city": "Місто",
        "deviceType": "Тип пристрою",
        "os": "Операційна система",
        "browser": "Браузер",
        "language": "Мова",
        "day": "Дата (День)",
        "hour": "Година",
        "lpTime": "До переходу на оффер (бакет)",
        "lpDwell": "Час на лендінгу (бакет)",
        "pwaEntryScreen": "PWA: перший екран",
        "pwaLastScreen": "PWA: останній екран",
        "adCampaignId": "ID кампанії",
        "adsetId": "ID групи оголошень",
        "adId": "ID оголошення",
        "keyword": "Ключове слово",
        "creativeId": "ID креативу",
        "externalId": "Зовнішній ID",
        "subId1": "Sub ID 1",
        "subId2": "Sub ID 2",
        "subId3": "Sub ID 3",
        "subId4": "Sub ID 4",
        "subId5": "Sub ID 5",
        "campaignName": "Назва кампанії",
        "adsetName": "Назва групи оголошень",
        "adName": "Назва оголошення",
        "utmPlacement": "UTM-майданчик",
        "paramSource": "Джерело",
        "isp": "Провайдер (ISP)",
        "asn": "ASN"
    },
    mobileCards: {
        "showMore": "Більше",
        "showLess": "Згорнути",
        "cardMetrics": "Метрики картки",
        "cardMetricsHint": "Показуються на кожній картці, у цьому порядку",
        "moveUp": "Вгору",
        "moveDown": "Вниз"
    },
    "media": {
        "title": "Галерея контенту",
        "subtitle": "Спільна бібліотека зображень — обирайте з неї в офертах, лендінгах та будь-якому діалозі вибору файлу.",
        "upload": "Завантажити зображення",
        "uploadHint": "Натисніть або перетягніть зображення сюди (webp, jpg, png, gif · до 10 МБ)",
        "uploadingCount": "Завантаження ({n})…",
        "allFiles": "Усі файли",
        "searchPlaceholder": "Пошук за іменем файлу…",
        "filterUser": "Користувач",
        "statusActive": "Активні",
        "statusInactive": "Неактивні (видалені)",
        "newFolder": "Нова папка",
        "folderNamePlaceholder": "Назва папки (до 50 символів)",
        "renameFolder": "Перейменувати папку",
        "deleteFolder": "Видалити папку",
        "deleteFolderConfirm": "Видалити папку «{name}»? Файли всередині залишаться в бібліотеці (у корені).",
        "selected": "Вибрано: {n}",
        "move": "Перемістити",
        "moveTitle": "Перемістити в папку",
        "noFolder": "Без папки (корінь)",
        "restore": "Відновити",
        "deselectHint": "Натисніть, щоб зняти виділення",
        "selectHint": "Натисніть, щоб виділити",
        "copyUrl": "Копіювати URL",
        "renameFile": "Перейменувати файл",
        "openFolder": "Відкрити папку",
        "folderMenu": "Дії з папкою",
        "purgeForever": "Видалити назавжди",
        "purgeConfirm": "Видалити назавжди файлів: {n}? Сторінки, де вони використовувались, покажуть биті картинки. Дію неможливо скасувати.",
        "empty": "Зображень ще немає — завантажте перше",
        "emptyInactive": "Видалених файлів немає",
        "totalFiles": "Файлів: {n}",
        "pageOf": "Сторінка {p} з {n}",
        "deniedNotice": "{n} файлів належать іншому користувачу та були пропущені.",
        "pickerTitle": "Виберіть зображення",
        "selectButton": "Вибрати",
        "selectCount": "Вибрати ({n})",
        "loadMore": "Показати ще",
        "cropButton": "Обрізати",
        "cropApply": "Застосувати та завантажити",
        "cropSkip": "Пропустити",
        "cropHint": "Перетягніть рамку, тягніть за кути — результат буде рівно {w}×{h}.",
        "exactSize": "Точний збіг",
        "croppable": "Можна обрізати до {w}×{h}",
        "tooSmall": "Занадто мале для {w}×{h}",
        "replaceFromGallery": "Замінити з галереї",
        "insertImage": "Вставити зображення",
        "err_extension": "Цей тип файлу не дозволений",
        "err_not_image": "Файл не є коректним зображенням",
        "err_too_large": "Файл занадто великий (максимум 10 МБ)",
        "err_upload": "Не вдалося завантажити",
        "err_store": "Не вдалося зберегти файл",
        "err_no_files": "Файли не отримано",
        "err_bad_request": "Некоректний запит",
        "err_folder_name": "Назва папки — від 1 до 50 символів",
        "err_folder_exists": "Папка з такою назвою вже існує"
    },
    pwa: {
        title: "PWA-лендінг",
        updateAvailable: "Доступне оновлення",
        reload: "Перезавантажити",
        loadFailed: "Не вдалося завантажити конфіг PWA",
        nameRequired: "Внутрішня назва та назва застосунку обов'язкові",
        saveFailed: "Помилка збереження PWA",
        saving: "Зберігаємо та генеруємо статику…",
        openPreview: "Відкрити прев'ю",
        copyLink: "Скопіювати посилання",
        linkServerAddress: "адреса сервера — для реклами прив'яжіть домен",
        stepGeneral: "Загальні",
        stepReviews: "Відгуки",
        preview: "Перегляд",
        presets: "Пресети",
        presetsHint: "Пресет заповнить оформлення та приклади контенту — назви залишаться як ввели",
        dragHint: "Перетягуйте відгуки за важіль, щоб змінювати порядок",
        funnelTitle: "Воронка візиту (екрани)",
        funnelHint: "Екрани браузерного візиту за порядком. Перший увімкнений крок — те, що відвідувач бачить першим; клік по кнопці кроку веде до наступного, на останньому спрацьовує встановлення. Перегляди кожного екрана пишуться автоматично.",
        funnelFirstBadge: "Перший екран",
        funnelOff: "вимк",
        funnelEdit: "Редагувати екран",
        funnelMoveUp: "Вище",
        funnelMoveDown: "Нижче",
        funnelEnabledTitle: "Бере участь у воронці",
        funnelAddScreen: "Додати екран",
        funnelStepJs: "Скрипт відстеження екрана (JS)",
        funnelStepJsHint: "Запускається при першому показі екрана — рекламні пікселі спрацьовують, лише якщо відвідувач дійшов до нього. Перегляди екрана фіксуються автоматично.",
        funnelStepStore: "Магазин (лістинг)",
        funnelStepInstructions: "Інструкція зі встановлення",
        funnelStepPush: "Пуш-картка",
        funnelStepScreen: "Власний екран",
        funnelCardTitle: "Воронка за екранами",
        funnelCardLoadFailed: "Не вдалося завантажити статистику воронки",
        funnelCardClicks: "Візити",
        funnelCardWithScreens: "Бачили екрани",
        funnelCardIntents: "Кліки «Встановити»",
        funnelCardInstalls: "Встановлення",
        funnelCardOfferClicks: "Переходи на оффер",
        funnelCardPushSubs: "Пуш-підписки",
        funnelCardNoData: "Переглядів екранів поки немає. Налаштуйте воронку в редакторі і лейте трафік.",
        funnelCardScreens: "Екрани",
        funnelCardScreen: "Екран",
        funnelCardViews: "Покази",
        funnelCardUniq: "Уніки",
        funnelCardEntry: "Вхід",
        funnelCardExits: "Відтік",
        funnelCardTransitions: "Переходи",
        back: "Назад",
        next: "Далі",
        sectionBasic: "Основне",
        internalName: "Внутрішня назва",
        internalNameHint: "Для ідентифікації в панелі; відвідувачам не показується",
        state: "Статус",
        appName: "Назва застосунку",
        developer: "Розробник",
        category: "Категорія",
        language: "Мова PWA",
        languageHint: "Будь-який код — pt, pt-BR, ar… Незнайомі мови відкатуються на англійську у вбудованих текстах (інструкції, підписка)",
        pushEnabled: "Push-повідомлення",
        pushEnabledHint: "Після встановлення застосунок один раз запропонує повідомлення — підписники дають безкоштовний ретаргетинг і зазвичай підвищують конверсію. Потрібні ключі зі сторінки Пуш-бази.",
        buttonText: "Текст кнопки",
        downloads: "Встановлення",
        adsLabel: "Позначка реклами",
        version: "Версія",
        sectionMedia: "Іконка та скриншоти",
        appAction: "Дія застосунку (після встановлення)",
        appActionHint: "Що бачить відвідувач під час відкриття встановленого застосунку з головного екрана — грошовий крок воронки",
        appActionStore: "Показувати магазин",
        appActionOffer: "Редирект на оффер",
        appActionCampaign: "Редирект на кампанію",
        appActionCampaignHint: "Відвідувач потрапляє в обрану кампанію через трекер — її стріми та ротація розподіляють трафік; {subid} зберігає атрибуцію конверсій",
        appActionScreen: "Кастомний екран застосунку",
        appScreenTitle: "Заголовок екрана",
        appScreenText: "Текст екрана",
        appScreenButton: "Текст кнопки",
        icon: "Іконка застосунку",
        iconHint: "Рекомендується квадрат 512×512 PNG/WebP",
        pickIcon: "Вибрати іконку з галереї",
        pickScreens: "Додати скриншоти з галереї",


        heroImage: "Фон екрана застосунку",        pickHero: "Вибрати фон екрана з галереї",
        iconDrop: "Завантажити іконку",
        iconDropHint: "Перетягніть зображення сюди або натисніть для вибору",
        iconDropReplace: "Натисніть або перетягніть інше зображення для заміни",
        screens: "Скриншоти",
        screensHint: "До 10 (PNG/JPEG/WebP)",
        addScreens: "Додати скриншоти",
        sectionContent: "Контент",
        description: "Опис",
        descriptionHint: "Макроси: {value} — назва застосунку, {value1}/{value2} — розробник",
        tags: "Теги",
        tagsHint: "Ключові слова через кому під описом",
        rating: "Розподіл рейтингу",
        ratingAvg: "середній",
        sectionComments: "Відгуки",
        commentName: "Ім'я",
        commentDate: "Дата",
        commentText: "Текст відгуку",
        commentReply: "Відповідь розробника",
        addComment: "Додати відгук",
        sectionDesign: "Оформлення",
        storeStyle: "Стиль магазину",
        storeStyleAuto: "Авто (iOS → App Store, Android → Google Play)",
        storeStyleGooglePlay: "Google Play (Android)",
        storeStyleAppStore: "Apple App Store (iOS)",
        themeMode: "Тема",
        themeLight: "Світла",
        themeDark: "Темна",
        colorScheme: "Колірна схема",
        verifiedBadge: "Бейдж «Перевірено»",
        preloader: "Прелоадер",
        showHeader: "Шапка з пошуком",
        bottomMenu: "Нижнє меню",
        showShare: "Кнопки «Поділитися»",
        sectionFunnel: "Воронка та редиректи",
        stepFlow: "Екрани",
        stepStore: "Магазин",
        funnelAllOffHint: "Усі кроки вимкнено — відвідувач побачить магазин (лістинг). Увімкніть крок або додайте власний екран.",
        buttonTextFlowHint: "Використовується як напис кнопки на кроці «Інструкція»",
        iosFlow: "iOS-флоу",
        iosFlowHint: "На iOS немає нативного промпта встановлення — покажемо інструкцію «На екран Домой»",
        iosFlowDefault: "Звичайний",
        iosFlowInstruction: "Інструкція",
        autoRedirect: "Авто-редирект після відкриття",
        declineRedirect: "Редирект при відмові від встановлення",
        installRedirect: "Редирект після встановлення",
        installRedirectHint: "0 = одразу після події встановлення",
        timerOff: "Вимк",
        timerImmediate: "Одразу",
        supportEnabled: "Блок підтримки",
        supportEmail: "Email підтримки",
        supportAddress: "Адреса підтримки",
        whatsNew: "Блок «Що нового»",
        whatsNewText: "Текст «Що нового»",
        previewStore: "Магазин",
        previewApp: "Екран додатка",
        actionTarget: "Цільова дія / редирект",
        actionTargetHint: "Куди перенаправляти користувача при кліку на кнопку, авторедиректі або запуску встановленого додатка",
        targetOffer: "Редирект на оффер (потік)",
        targetCampaign: "Редирект на кампанію",
        targetUrl: "Прямий URL",
        targetNotFound: "404 Not Found",
        targetCampaignLabel: "Цільова кампанія",
        targetCampaignPlaceholder: "-- Оберіть кампанію --",
        targetUrlLabel: "URL для переходу",
        targetUrlPlaceholder: "https://example.com/?subid={subid}",
        targetNotFoundHint: "Користувачеві буде показано сторінку помилки 404",
        appScreenType: "Тип екрана додатка",
        appScreenTypeHint: "Оберіть нативний дашборд, вбудовані інтерактивні міні-ігри або власний HTML5/JS код гри (рулетка, слоти, квіз)",
        screenTypeLobby: "Нативне лобі (Дашборд)",
        screenTypeSlot: "Інтерактивний 777 Слот-автомат",
        screenTypeWheel: "Колесо фортуни / Бонус-рулетка",
        screenTypeCustom: "Власний HTML & JavaScript код",
        customHtmlLabel: "Користувацький HTML код",
        customHtmlPlaceholder: "<div id=\"my-slot-game\">\n  <!-- Ваша верстка, canvas або iframe гри -->\n</div>",
        customJsLabel: "Користувацький JavaScript код",
        customJsPlaceholder: "// Викличте window.orbitraRedirect() при виграші або кліку:\nfunction onGameWin() {\n  window.orbitraRedirect();\n}",
        customJsTip: "Викликайте window.orbitraRedirect() або додавайте клас class=\"install-trigger\" на елементи для ініціації переходу на оффер/кампанію.",
        insertSlotBoilerplate: "Вставити шаблон слота",
        insertWheelBoilerplate: "Вставити шаблон рулетки",
        sectionCustomScripts: "Скрипти, стилі CSS та пікселі",
        customCssLabel: "Користувацькі стилі CSS та анімації",
        customCssHint: "Перевизначення будь-яких стилів, кольорів, градієнтів або додавання CSS @keyframes анімацій",
        customCssPlaceholder: "/* Користувацький CSS та keyframe анімації */\n.install-btn { transform: scale(1.02); }",
        customHeadLabel: "Код для <head> / Пікселі трекінгу",
        customHeadPlaceholder: "<!-- Пікселі FB, TikTok, Google Tag Manager, шрифти або зовнішні скрипти -->",
        customJsGlobalLabel: "Глобальний JavaScript",
        customJsGlobalPlaceholder: "// Користувацький JS перед закриваючим тегом </body>",
        animationGlow: "Неонове світіння та анімація кнопок (Буст CTR)",
        showLiveBadge: "Лічильник гравців онлайн (Соціальний доказ)",
        soundEnabled: "Звукові ефекти казино / ігор",
        vibrationEnabled: "Тактильна вібровіддача",
        domainLabel: "Домен",
        domainNone: "— не прив'язаний —",
        domainSaveFirst: "Збережіть PWA, щоб прив'язати домен",
        domainBindOnSave: "Домен можна вибрати одразу — він прив'яжеться до цієї PWA при збереженні",
        domainHint: "Візит на домен відкриє цей PWA з кореня домену; клік логується як органічний. Якщо в селекті нижче вибрано оффер, кнопка «Offer» веде на оффер просто з домена; без оффера буде чесна 404 — виберіть оффер у селекті нижче. Якщо домен уже прив'язаний до іншого PWA, під час збереження його буде перезаписано на цей.",
        domainSaving: "Збереження прив'язки…",
        domainError: "Не вдалося зберегти прив'язку домену",
        offerLabel: "Оффер",
        offerNone: "— не вибрано —",
        offerSaving: "Збереження оффера…",
        offerError: "Не вдалося зберегти прив'язку оффера",
    },
    push: {
        navLabel: "Пуш-база",
        noAccess: "Немає доступу",
        refresh: "Оновити",
        exportCsv: "Експорт CSV",
        vapidTitle: "VAPID-ключі",
vapidHint: "Генеруються один раз і підключаються автоматично — кожна PWA отримує публічний ключ під час віддачі сторінки. Нікуди вписувати не потрібно.",
        vapidMissing: "Ключі ще не згенеровано — екран підписки прихований, доки їх немає",
        rotateConfirm: "Заміна ключів зробить недійсними всі наявні підписки (браузери мають підписатися знову). Продовжити?",
        rotateKeys: "Замінити ключі",
        generateKeys: "Згенерувати ключі",
        filterAll: "Усі",
        filterActive: "Активні",
        filterDead: "Мертві",
        totalActive: "Активних",
        totalAll: "Всього",
        created: "Створена",
        country: "Країна",
        language: "Мова",
        clickId: "Click ID",
        endpoint: "Endpoint",
        status: "Статус",
        empty: "Підписників ще немає — встановіть PWA і дозвольте повідомлення",
        statusLive: "Живий",
        statusDead: "Мертвий",
        delete: "Видалити",
        deleteConfirm: "Видалити підписника безповоротно?",
        prev: "Назад",
        next: "Далі",
        loadFailed: "Не вдалося завантажити пуш-базу",
        tabSubscribers: "Підписники",
        tabMessages: "Повідомлення",
        newMessage: "Нове повідомлення",
        editMessage: "Редагування повідомлення",
        messagesEmpty: "Повідомлень ще немає — створіть перше та натисніть «Надіслати»",
        messageTitle: "Заголовок",
        messageTitlePlaceholder: "Ваш донат сьогодні подвоюється",
        messageText: "Текст",
        messageTextPlaceholder: "Текст сповіщення, до 400 символів",
        iconUrl: "URL іконки (необов'язково)",
        linkUrl: "Посилання",
        linkHint: "Вставте посилання трекер-кампанії — {subid} заміниться на click_id підписника, конверсії атрибуціються звичайними постбеками",
        segment: "Сегмент",
        segmentAll: "Усі підписники",
        segmentReg0: "Без реєстрацій",
        segmentReg1Dep0: "З реєстрацією, без депозитів",
        segmentReg1Dep1: "З реєстрацією та депозитами",
        kind: "Тип",
        kindManual: "Вручну",
        kindEvent: "За подією",
        event: "Подія",
        eventInstall: "Встановлення",
        eventLead: "Лід",
        eventSale: "Сейл",
        delay: "Затримка, секунди",
        delayUnit: "с",
        delayHint: "Пуш піде через стільки секунд після події",
        sent: "Надіслано",
        failed: "Помилок",
        queued: "У черзі",
        workerStalled: "Повідомлення стоять у черзі, але воркер доставки не запущено — нічого не піде. Додайте цей рядок у crontab користувача www-data:",
        contactLabel: "Контакт відправника (VAPID sub)",
        contactHint: "mailto: або https: — адреса, за якою пуш-сервіс може зв'язатися з вами. Apple відхиляє вбудовану заглушку і відповідає 403, тож перед надсиланням на iPhone вкажіть справжню.",
        contactInvalid: "Контакт має починатися з mailto: або https:",
        lastFailCode: "Остання помилка",
        failCodeNoReply: "немає відповіді",
        sendNow: "Надіслати",
        confirmSend: "Поставити повідомлення в чергу всім відповідним активним підписникам?",
        sendQueued: "У чергу поставлено:",
        messageDeleteConfirm: "Видалити повідомлення? Черга та журнал доставок залишаться.",
        keysExist: "Ключі вже існують — підтвердьте заміну, щоб ротувати їх",
        titleTextRequired: "Потрібні заголовок і текст",
        tooLong: "Заголовок — до 250 символів, текст — до 400",
        eventRequired: "Виберіть подію для повідомлення",
        saveFailed: "Не вдалося зберегти повідомлення",
        sendFailed: "Не вдалося поставити повідомлення в чергу",
        contactRequired: "Спочатку збережіть реальний контакт відправника (mailto: або https:) — Apple відхиляє заглушку з помилкою 403 на всіх iOS-надсиланнях",
        rotateAffects: "Це відв'яже {n} активних підписок; кожен пристрій перепідпишеться автоматично при наступному відкритті застосунку.",
        diagKeys: "Ключі",
        diagContact: "Контакт",
        diagWorker: "Воркер",
        diagWorkerNever: "не запускався",
        diagWorkerAgo: "{n} хв тому",
        fail403Hint: "403 — перевірте контакт відправника: Apple відхиляє адреси-заглушки",
        testSend: "Надіслати тестовий пуш цьому підписнику",
        testDelivered: "Тестовий пуш прийнято push-сервісом — перевірте пристрій",
        testDead: "Endpoint не існує (404/410) — підписка випаде з бази",
        testRejected: "Push-сервіс відхилив тест:",
    }
};
