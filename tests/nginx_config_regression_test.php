...
// Alternative: use nginx -T to get the full active config (better validation)
$nginxT = shell_exec('nginx -T 2>&1');
if ($nginxT !== null && strpos($nginxT, 'configuration file') !== false) {
    // Extract just the orbitra config from the full output
    preg_match('/# Auto-generated by Orbitra.*?(?=\n#|$|\n\s*$)/s', $nginxT, $matches);
    if (!empty($matches[0])) {
        $config = $matches[0];
        echo "Using nginx -T output (active config)\n\n";
    }
}

// ---- 3. Parse server blocks from config -----------------------------------
$serverBlocks = [];
preg_match_all('/server\s*\{([^}]+)\}/s', $config, $matches);
...
foreach ($serverBlocks as $block) {
    if ($block['server_name'] === '_' && in_array('443 ssl', array_map('trim', $block['listens']))) {
